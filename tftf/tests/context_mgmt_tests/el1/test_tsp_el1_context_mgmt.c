...
test_result_t test_tsp_el1_regs_ctx_mgmt(void)
{
	 ...
	 * This will be clean copy of registers which will be restored back at
	 * the end of the test.
	 */
	save_el1_sysregs_context(&el1_ctx_original);

	/**
	 * 2. Modify/Corrupt the EL1 registers with some dummy/random values.
	 * This will be essential for the test to ensure registers are accessed
	 * and modified in both the security states, which will be an ideal
	 * scenario in realtime. and
	 */
	modify_el1_context_sysregs(&el1_ctx_original);

	/**
	 * 3. Read the values of EL1 registers again after modification and save
	 * them into an other buffer before the world switch.
	...
	/**
	 * 5. Read the values of EL1 registers and save them into the
	...
	/**
	 * 7. Restore the EL1 registers to their original value in order to avoid
	 * any unintended crash for next tests.
	 */
	restore_el1_context_sysregs(&el1_ctx_original);

	/* 8. Standard SMC to TSP, to modify EL1 registers in secure world */
	tsp_svc_params.fid = TSP_STD_FID(TSP_MODIFY_EL1_CTX);
	tsp_svc_params.arg1 = TSP_RESTORE_WITH_DEFAULT;
	tsp_result = tftf_smc(&tsp_svc_params);

	/*
	 * Check the result of the SMC call
	 */
	if (tsp_result.ret0 != 0) {
		printf("TSP CHECK EL1_CTX returned wrong result: "
				     "got %d expected: 0 \n",
