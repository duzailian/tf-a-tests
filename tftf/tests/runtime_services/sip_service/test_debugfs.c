...
static const dir_expected_t dev_dir_expected[] = {
	{ "psci", 0x8005 }
};
...
static int open(const char *name, int flags)
{
	...
	strncpy(parms->open.fname, name, MAX_PATH_LEN);
...
static int mount(char *srv, char *where, char *spec)
{
	...
	strncpy(parms->mount.srv, srv, MAX_PATH_LEN);
	strncpy(parms->mount.where, where, MAX_PATH_LEN);
	strncpy(parms->mount.spec, spec, MAX_PATH_LEN);
...
static int stat(const char *name, dir_t *dir)
{
	...
	strncpy(parms->stat.path, name, MAX_PATH_LEN);

	args.fid  = ARM_SIP_SVC_DEBUGFS;
	args.arg1 = STAT;
	args.arg2 = (u_register_t) parms->stat.path;
	args.arg3 = (u_register_t) &parms->stat.dir;
...
static int bind(char *old, char *new)
{
	union debugfs_parms *parms = payload;
	smc_ret_values ret;
	smc_args args;

	strncpy(parms->bind.oldpath, old, MAX_PATH_LEN);
	strncpy(parms->bind.newpath, new, MAX_PATH_LEN);
...
test_result_t test_debugfs(void)
{
	...
	ret = mmap_add_dynamic_region(PLAT_ARM_DEBUGFS_BASE, PLAT_ARM_DEBUGFS_BASE,
				      PLAT_ARM_DEBUGFS_SIZE, MT_RW_DATA);
	if (ret) {
		tftf_testcase_printf("map failed ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}

	/* Initialize debugfs feature, this maps the NS shared buffer */
	...
	ret = read(fd, &dir, sizeof(dir));
	while (ret > 0) {
		if (compare_dir(dev_dir_expected, iteration++,
				&dir) == false) {
			dir_print(&dir);
			return TEST_RESULT_FAIL;
		}
	...
	/***************** PSCI operations **************/
	/* bind psci drvier to /dev/psci */
	ret = bind("#P", "/dev/psci");
	if (ret < 0) {
		tftf_testcase_printf("bind failed ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}

	/* open psci driver */
	fd = open("/dev/psci", O_READ);
	if (fd < 0) {
		tftf_testcase_printf("open failed fd=%d\n", fd);
		return TEST_RESULT_FAIL;
	}
	...
	/* Expect PSCI topology:                              */
	/* 1 (root), 2 (clusters), 4 (c0 cores), 4 (c1 cores) */
	if (read_buffer[0] != 0x04040201) {
		tftf_testcase_printf("PSCI topology 0x%x\n", read_buffer[0]);
		return TEST_RESULT_FAIL;
	}

	ret = mmap_remove_dynamic_region(PLAT_ARM_DEBUGFS_BASE,
					 PLAT_ARM_DEBUGFS_SIZE);
	if (ret) {
		tftf_testcase_printf("unmap failed ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
