...
test_result_t test_getc(void)
{
	...
	if(ret != -1) {
		tftf_testcase_printf("Read did not fail with negative length \
		       ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
	...
	if(ret != -1) {
		tftf_testcase_printf("Write did not fail with negative length \
		       ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
	...
	if(ret != -1) {
		tftf_testcase_printf("Read did not fail with a length greater \
		       than one page ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
	...
	if(ret != 0) {
		tftf_testcase_printf("Read for zero bytes did not return 0 \
		       ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
	...
	if(ret != 0) {
		tftf_testcase_printf("Write for zero bytes did not return 0 \
		       ret=%d\n", ret);
		return TEST_RESULT_FAIL;
	}
	...
	for(int i = 0; i < n_phrases; ++i) {
		...
		if(ret != len) {
			tftf_testcase_printf("read failed expected %d \
			ret=%d\n", len, ret);
			return TEST_RESULT_FAIL;
		}
		...
		if(strncmp(test_phrases[i], (char *)read_buffer, len) != 0) {
			...
			tftf_testcase_printf("Read returned wrong value. \
			Expected %s got: %s", test_phrases[i],
			(char *)read_buffer);
			return TEST_RESULT_FAIL;
		}
		...
		ret = write(fd, read_buffer, len);
		if(ret != len) {
			tftf_testcase_printf("write failed expected %d \
			ret=%d\n", len, ret);
			return TEST_RESULT_FAIL;
		}
	}

	/* Reading 4095 'a' characters and one '\n' character */
	/* FIXME: This experiences a hang after reading 18 characters
	 *
	len = 4096;
	ret = read(fd, read_buffer, len);
	if(ret != len) {
	...
	for(int i = 0; i < len - 1; ++i) {
		if(((char *)read_buffer)[i] != 'a') {
			tftf_testcase_printf("page read has wrong character \
			at index %d expected a got %d\n", i,
			((char *)read_buffer)[i]);
			return TEST_RESULT_FAIL;
		}
	}
