...
test_result_t test_getc(void)
{
	...
	// Open console so that we can read and write to it
	...
	*/
	...
	// This done message is sent to prevent the expect output of this test
	// from interleaving with the output of following tests
	...
	if(strncmp(done_msg, (char*)read_buffer, len) != 0) {
		((char*)read_buffer)[len] = '\0';
		tftf_testcase_printf("Read returned wrong value. Expected %s got: %s", done_msg, (char*)read_buffer);
		return TEST_RESULT_FAIL;
	}
	*/
