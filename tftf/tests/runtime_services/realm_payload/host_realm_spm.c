...
static test_result_t init_realm(void)
{
	...
	if (!host_create_realm_payload((u_register_t)REALM_IMAGE_BASE,
			...
			(u_register_t)PAGE_POOL_MAX_SIZE, 0UL, rec_flag, 1U, &realm)) {
...
{
	if (!host_enter_realm_execute(REALM_REQ_FPU_FILL_CMD, &realm, RMI_EXIT_HOST_CALL, 0U)) {
		ERROR("%s failed %d\n", __func__, __LINE__);
		return false;
	}
	return true;
...
{
	if (!host_enter_realm_execute(REALM_REQ_FPU_CMP_CMD, &realm, RMI_EXIT_HOST_CALL, 0U)) {
		ERROR("%s failed %d\n", __func__, __LINE__);
		return false;
	}
	return true;
...
test_result_t host_realm_sec_interrupt_can_preempt_rl(void)
{
	...
	host_enter_realm_execute(REALM_SLEEP_CMD, &realm, RMI_EXIT_FIQ, 0U);
