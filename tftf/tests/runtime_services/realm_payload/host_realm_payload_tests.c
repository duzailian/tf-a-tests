 ...
 * Host: changes RIPAS for IPA out of range, verifies error generated by RMM
...
test_result_t host_realm_set_ripas(void)
{
	...
	ret1 = host_enter_realm_execute(&realm, REALM_SLEEP_CMD, RMI_EXIT_HOST_CALL, 0U);
	base = (u_register_t)page_alloc(PAGE_SIZE * test_size);
	host_shared_data_set_host_val(&realm, 0U, HOST_ARG1_INDEX, base);
	host_shared_data_set_host_val(&realm, 0U, HOST_ARG2_INDEX, base + (PAGE_SIZE * test_size));

	for (unsigned int i = 0U; i < test_size; i++) {
	...
	/* Attemp to set ripas for IPA out of range, expect error */
	...
	while (run->exit.ripas_base <= base + (PAGE_SIZE * test_size)) {
 ...
 * when access page with PIAS=DESTOYED
 * Host maps a protected page, initial state is RIPAS=EMPTY HIPAS=ASSIGNED
 * Enters realm, realm requests page RIPAS change to RAM
 * Host changes RIPAS to RAM, then calls data_destroy
 * new state HIPAS=UNASSIGNED RIPAS=DESTROYED
 * Realm accesses the page
 * RMM should exit to host, Host verifies exit reason is instruction abort
 */
