...
/*
 * Helper to test initial state of page is unassigned empty
 * Call INIT_RIPAS to move state to unassigned ram
 * Realm is already created, Activate realm if requested
 * Call RTT_DESTROY
...
static test_result_t test_rtt_destroy_ram(struct realm *realm, bool activate)
{
	...
	/* Find any address not mapped in L3 */
	base = ALIGN_DOWN(PAGE_POOL_BASE, host_rtt_level_mapsize(2U));
	while (true) {
		ret = host_rmi_rtt_readentry(realm->rd, base, 3U, &rtt);
 ...
 * Call RTT_DESTROY
 * Check new state is unassigned destroyed
 * Realm is already created and can be in new or active state
 */
...
test_result_t host_realm_pas_transition_new(void)
{
	...
	/* Heap region 3MB is already mapped in RTT L2 Table as payload is carved from heap */
	ret = host_rmi_rtt_readentry(realm.rd, base, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.state != RMI_UNASSIGNED ||
			(rtt.ripas != RMI_EMPTY)) {
		ERROR("wrong initial state\n");
	...
	res = test_rtt_destroy_ram(&realm, false);
	goto destroy_realm;
...
test_result_t host_realm_pas_transition_active(void)
{
	...
	res = test_rtt_destroy_ram(&realm, true);
	if (res != TEST_RESULT_SUCCESS) {
...
test_result_t host_realm_sea_adr_fault(void)
{
	...
	/* Rec0 expect SEA in realm due to Data Access IPA outside Realm space */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_CMD,
			RMI_EXIT_HOST_CALL, 0U);
	if (!ret1) {
	...
	/* Rec1 expect SEA in realm due to Instruction access IPA outside Realm space */
	ret1 = host_enter_realm_execute(&realm, REALM_INSTR_FETCH_CMD,
		RMI_EXIT_HOST_CALL, 1U);
	if (!ret1) {
		ERROR("Rec1 did not fault\n");
 ...
 */
test_result_t host_test_rtt_fold_unfold_unassigned(void)
{
	...
	/* TFTF creates L1 entries for 512 GB range 0-1<<39 choose any adr excluding this */
	base = (u_register_t)(1UL << 40);
	ret = host_rmi_rtt_readentry(realm.rd, base, 3U, &rtt);
	...
	/* L3 entry should be created */
	ret = host_rmi_rtt_readentry(realm.rd, base, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.walk_level != 3U) {
		ERROR("host_rmi_create_rtt_levels failed ret=0x%lx\n", ret);
		goto destroy_realm;
	}
	INFO("rtt.state=0x%lx rtt.walk_level=0x%llx rtt.out_addr=0x%llx\n",
...
/*
 * Test aims to test RMI_RTT_FOLD on Unprotected IPA range
...
test_result_t host_test_rtt_fold_unfold_assigned_ns(void)
{
	...
	/* Any NS address not mapped at L1 0xc0000000 */
	base_pa = ALIGN(TFTF_BASE, host_rtt_level_mapsize(1U));
	...
	ret = host_rmi_create_rtt_levels(&realm, base, 1U, 2U);
	if (ret != RMI_SUCCESS) {
		ERROR("host_rmi_create_rtt_levels failed ret=0x%lx\n", ret);
	}
...
/*
 * Test aims to test RMI_RTT_FOLD on Protected IPA range
...
test_result_t host_test_rtt_fold_unfold_assigned_empty(void)
{
	...
	base = ALIGN_DOWN(PAGE_POOL_BASE + PAGE_POOL_MAX_SIZE, host_rtt_level_mapsize(2U));
 ...
 * Host verifies walk_level is 3
 * Host folds RTT_FOLD Level 2
 * Host verifies walk_level is 2 RIPAS is RAM
 */
test_result_t host_test_rtt_fold_unfold_assigned_ram(void)
{
	...
	/* Walk should terminate at L2 */
	ret = host_rmi_rtt_readentry(realm.rd, base, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.walk_level != 2U || rtt.ripas != RMI_RAM) {
		ERROR("host_rmi_fold_rtt failed ret=0x%lx rtt.state=0x%lx"
	...
	/* L3 entry should be created */
	ret = host_rmi_rtt_readentry(realm.rd, base, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.walk_level != 3U || rtt.ripas != RMI_RAM) {
		ERROR("host_rmi_create_rtt_levels failed ret=0x%lx\n", ret);
		goto destroy_realm;
	}
