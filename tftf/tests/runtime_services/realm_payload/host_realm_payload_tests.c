...
test_result_t host_test_realm_create_planes_enter_s2poe(void)
{
	...
	struct realm realm;
	u_register_t feature_flag0 = 0UL, feature_flag01;
	long sl = RTT_MIN_LEVEL;
	struct rmi_rec_run *run;
	...
	feature_flag01 = RMI_REALM_FLAGS1_RTT_S2AP_ENCODING_INDIRECT;

	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag0, feature_flag01, sl, &rec_flag, 1U, MAX_AUX_PLANE_COUNT)) {
		return TEST_RESULT_FAIL;
	}
...
test_result_t host_test_realm_create_enter(void)
{
	...
	u_register_t rec_flag[MAX_REC_COUNT];
	struct realm realm;
	u_register_t feature_flag0 = 0UL, feature_flag01 = 0UL;
	long sl = RTT_MIN_LEVEL;
	...
	for (unsigned int i = 0U; i < 5U; i++) {
		...
		if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
				feature_flag0, feature_flag01, sl, rec_flag, MAX_REC_COUNT, 0U)) {
			return TEST_RESULT_FAIL;
		}
...
static test_result_t host_test_realm_pmuv3(uint8_t cmd)
{
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag0, 0U, sl, rec_flag, 1U, num_aux_planes)) {
		return TEST_RESULT_FAIL;
	}
