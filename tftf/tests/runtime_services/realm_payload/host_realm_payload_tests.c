...
/*
 * Test aims to generate SEA in Realm by accessing
 * PAGE with IPA outside realm IPA space
 * Rec0 tries to create Data Abort to realm
 * Rec1 tries to create Instruction Abort to realm
...
test_result_t host_realm_sea_adr_fault(void)
{
	bool ret1, ret2;
	test_result_t res = TEST_RESULT_FAIL;
	u_register_t base_ipa, esr, feature_flag;
	struct realm realm;
	u_register_t rec_flag[2U] = {RMI_RUNNABLE, RMI_RUNNABLE}, base;
	struct rmi_rec_run *run;
