...
test_result_t host_test_realm_create_enter(void)
{
	...
	realm_shared_data_set_host_val(0U, HOST_ARG1_INDEX, SLEEP_TIME_MS);
...
{
	bool ret1, ret2;
	u_register_t ret, top_base, new_base, exit_reason;
	unsigned int host_call_result;
	struct realm *realm_ptr;
	...
	realm_shared_data_set_host_val(0U, HOST_ARG1_INDEX, 10U);
	ret1 = host_enter_realm_execute(REALM_SLEEP_CMD, &realm_ptr, RMI_EXIT_HOST_CALL, 0U);
	top_base = (u_register_t)page_alloc(PAGE_SIZE);
	ret = host_realm_map_protected_data(true, realm_ptr, top_base, PAGE_SIZE,
			top_base);
	if (ret != REALM_SUCCESS) {
		ERROR("host_realm_map_protected_data failed\n");
		goto destroy_realm;
	}
	realm_shared_data_set_host_val(0U, HOST_ARG1_INDEX, top_base);
	ret1 = host_enter_realm_execute(REALM_SET_RIPAS_CMD, &realm_ptr, RMI_EXIT_RIPAS_CHANGE, 0U);
	if (ret1) {
		run = (struct rmi_rec_run *)realm_ptr->run[0U];
		if (run->exit.ripas_base == top_base) {
			ret = host_rmi_rtt_set_ripas(realm_ptr->rd,
						     ...
						     &new_base);
			if (ret == RMI_SUCCESS) {
				run->entry.gprs[0] = RMI_SUCCESS;
				run->entry.gprs[1] = top_base + PAGE_SIZE;
				run->entry.gprs[2] = RSI_ACCEPT;
				ret = host_realm_rec_enter(realm_ptr,
					&exit_reason, &host_call_result, 0U);
				if (ret != RMI_SUCCESS && host_call_result != TEST_RESULT_SUCCESS) {
					ERROR("Rec re-enter failed\n");
				}
			} else {
				ERROR("host_rmi_rtt_set_ripas failed ret = 0x%lx\n", ret);
			}
		} else {
			ERROR("Wrong exit.ripas_base=0x%lx\n", run->exit.ripas_base);
		}
	} else {
		ERROR("Rec enter failed\n");
	}
	host_data_destroy_undelegate_range(realm_ptr,
			top_base,
			top_base,
			PAGE_SIZE);
