 ...
 * Enter Realm0 execute from page, and Realm1 to read the page
...
test_result_t host_realm_abort_unassigned_ram(void)
{
	...
	/* Realm0 expect rec exit due to Instr Abort unassigned ram page */
	ret1 = host_enter_realm_execute(&realm, REALM_INSTR_FETCH_CMD,
			RMI_EXIT_SYNC, 0U);
	...
	/* Realm1 expect rec exit due to Data Abort unassigned ram page */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_CMD,
			RMI_EXIT_SYNC, 1U);
 ...
 * Enter Realm0 and read the page, Enter Realm1 execute from the page
 * Realm should trigger an Instr/Data abort, and will exit to Host.
 * The Host verifies exit reason is Instr/Data abort
 */
test_result_t host_realm_abort_assigned_destroyed(void)
{
	...
	/* Realm0, expect rec exit due to Instr Abort assigned destroyed page */
	ret1 = host_enter_realm_execute(&realm, REALM_INSTR_FETCH_CMD,
		RMI_EXIT_SYNC, 0U);
	...
	/* Realm1  expect rec exit due to Data Abort  assigned destroyed page */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_CMD,
			RMI_EXIT_SYNC, 1U);
...
/*
 * Test aims to generate REALM Exit due to Abort
 * when access page with HIPAS=Unassigned_NS
 * Host maps a unprotected page
 * Enter Realm and access the page
 * Realm should trigger an Data abort, and will exit to Host.
 * The Host verifies exit reason is Data abort
 */
test_result_t host_realm_data_abort_unassigned_ns(void)
{
	bool ret1, ret2;
	test_result_t res = TEST_RESULT_FAIL;
	u_register_t ret, base_ipa;
	struct rtt_entry rtt;
	struct realm realm;
	struct rmi_rec_run *run;
	u_register_t rec_flag[2U] = {RMI_RUNNABLE, RMI_RUNNABLE}, base;
	...
	/* Any unprotected IPA can be used, TFTF_BASE used for convenience. */
	base = TFTF_BASE;
	...
	ret = host_rmi_rtt_readentry(realm.rd, base_ipa, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.state != RMI_UNASSIGNED) {
		ERROR("wrong initial state\n");
		goto destroy_realm;
	}

	/* Realm0  expect rec exit due to Data Abort  unassigned ns page */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_CMD,
			RMI_EXIT_SYNC, 0U);
	...
	if (!ret1 || ((run->exit.hpfar >> 4U) != (base_ipa >> PAGE_SIZE_SHIFT)
		|| (EC_BITS(run->exit.esr) != EC_DABORT_LOWER_EL)
		|| ((run->exit.esr & ISS_DFSC_MASK) != DFSC_L2_TRANS_FAULT)
		|| ((run->exit.esr & (1UL << ESR_ISS_EABORT_EA_BIT)) != 0U))) {
		ERROR("Rec did not fault exit=0x%lx ret1=%d HPFAR=0x%lx ESR=0x%lx\n",
 ...
 * Host creates and executed 4 recs to generate SEA
...
test_result_t host_realm_sea_unprotected(void)
{
	...
	/* Rec1 expect rec exit due to DA unprotected IPA page */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_SEA_CMD,
			RMI_EXIT_SYNC, 1U);
 ...
 * Rec0 DA
 * Rec1 IA
 * Realm exception handler returns ESR
 * Host validates ESR
 */
test_result_t host_realm_sea_adr_fault(void)
{
	...
	base_ipa = base | (1UL << 53U);
	host_shared_data_set_host_val(&realm, 0U, HOST_ARG1_INDEX, base_ipa);
	host_shared_data_set_host_val(&realm, 1U, HOST_ARG1_INDEX, base_ipa);

	INFO("base_ipa=0x%lx\n", base_ipa);

	/* Rec0 expect SEA in realm due to DA accessing IPA outside Realm space */
	ret1 = host_enter_realm_execute(&realm, REALM_DATA_ACCESS_SEA_CMD,
			RMI_EXIT_HOST_CALL, 0U);
	if (!ret1) {
		ERROR("Rec did not fault\n");
	...
	/* Rec1 expect SEA in realm due to IA accessing IPA outside Realm space */
	ret1 = host_enter_realm_execute(&realm, REALM_INSTR_FETCH_SEA_CMD,
		RMI_EXIT_HOST_CALL, 1U);
	if (!ret1) {
		ERROR("Rec did not fault\n");
	...
	/* get ESR set by Realm exception handler */
	esr = host_shared_data_get_realm_val(&realm, 1U, HOST_ARG2_INDEX);
	if (((esr & ISS_DFSC_MASK) != IFSC_L0_ADR_SIZE_FAULT)
 ...
 * Call INIT_RIPAS to move state to unassigned ram
 * Call RTT_DESTROY
 * Check new state is unassigned destroyed
 */
test_result_t test_rtt_destroy_ram(struct realm *realm)
{
	u_register_t ret, top, out_rtt, base;
	struct rtt_entry rtt;

	/* TFTF creates initial L1-L2 entries for range 0-1<<32 choose any adr excluding this */
	base = (u_register_t)(1UL << 34);
	ret = host_rmi_rtt_readentry(realm->rd, base, 3U, &rtt);
	...
	/* Destroy newly created rtt, there should be only 1 entry */
 ...
 * Check new state is unassigned destroyed
 */
test_result_t test_rtt_destroy_empty(struct realm *realm)
{
	u_register_t base, ret, top, out_rtt;
	struct rtt_entry rtt;

	/* TFTF creates initial L1-L2 entries for range 0-1<<32 choose any adr excluding this */
	base = (u_register_t)(1UL << 33);
	ret = host_rmi_rtt_readentry(realm->rd, base, 3U, &rtt);
	if (ret != RMI_SUCCESS || rtt.walk_level != 1U
			|| rtt.state != RMI_UNASSIGNED || (rtt.ripas != RMI_EMPTY)) {
		ERROR("Initial realm table creation changed\n");
	...
	/* Create RTT entries */
	ret = host_rmi_create_rtt_levels(realm, base, rtt.walk_level, 2U);
	if (ret != RMI_SUCCESS) {
		ERROR("host_rmi_create_rtt_levels failed ret=0x%lx\n", ret);
		return TEST_RESULT_FAIL;
	}
	ret = host_rmi_rtt_readentry(realm->rd, base, 3U, &rtt);
	if (rtt.walk_level != 2U) {
		ERROR("host_rmi_create_rtt_levels failed ret=0x%lx\n", ret);
		return TEST_RESULT_FAIL;
	}

	/* Destroy newly created rtt, there should be only 1 entry */
	ret = host_rmi_rtt_destroy(realm->rd, base, 2U, &out_rtt, &top);
	if (ret != RMI_SUCCESS) {
		ERROR("host_rmi_rtt_destroy failed ret=0x%lx\n", ret);
		return TEST_RESULT_FAIL;
	}
 ...
 * 6. Test RTT_DESTROY moves state unassigned ram to Unaasigned Destroyed
 *
 */
test_result_t host_realm_pas_transition_new(void)
{
	...
	INFO("Test 5\n");
	res = test_rtt_destroy_empty(&realm);
	if (res != TEST_RESULT_SUCCESS) {
		ERROR("Test 5 failed\n");
		goto destroy_realm;
	}

	/* 6. start with new page */
	res = test_rtt_destroy_ram(&realm);
	goto destroy_realm;
...
/*
 * Test aim is to test RTT_DESTROY
 * for active realm
 * Test initial state of page is unassigned empty
