...
test_result_t host_test_multiple_realm_create_enter(void)
{
	...
	ret1 = host_enter_realm_execute(REALM_SLEEP_CMD, &realm1, RMI_EXIT_HOST_CALL, 0U);
	if (!ret1) {
		goto destroy_realms;
	}
	host_shared_data_set_host_val(&realm2, 0U, HOST_ARG1_INDEX, SLEEP_TIME_MS);
	ret1 = host_enter_realm_execute(REALM_SLEEP_CMD, &realm2, RMI_EXIT_HOST_CALL, 0U);
