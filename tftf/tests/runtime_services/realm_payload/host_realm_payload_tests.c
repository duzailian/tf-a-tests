...
test_result_t host_test_realm_create_planes_register_rw(void)
{
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, sl, rec_flag, 1U, 1U)) {
...
test_result_t host_test_realm_create_planes_enter(void)
{
	...
	if (host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, RMI_REALM_FLAGS1_RTT_S2AP_ENCODING_INDIRECT,
			sl, rec_flag, 1U, MAX_AUX_PLANE_COUNT)) {
		return TEST_RESULT_FAIL;
	}

	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, sl, rec_flag, 1U, MAX_AUX_PLANE_COUNT)) {
		return TEST_RESULT_FAIL;
	}
 ...
 * Host will enter P0, P0 will enter aux plane
 */
test_result_t host_test_realm_create_planes_enter_s2poe(void)
{
	bool ret1, ret2;
	u_register_t rec_flag = RMI_RUNNABLE;
	struct realm realm;
	u_register_t feature_flag = 0UL, feature_flag1;
	long sl = RTT_MIN_LEVEL;
	struct rmi_rec_run *run;
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, feature_flag1, sl, &rec_flag, 1U, MAX_AUX_PLANE_COUNT)) {
		return TEST_RESULT_FAIL;
	}
...
test_result_t host_test_realm_create_enter(void)
{
	...
	u_register_t rec_flag[MAX_REC_COUNT];
	struct realm realm;
	u_register_t feature_flag = 0UL, feature_flag1 = 0UL;
	long sl = RTT_MIN_LEVEL;
	...
	for (unsigned int i = 0U; i < 5U; i++) {
		...
		if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
				feature_flag, feature_flag1, sl, rec_flag, MAX_REC_COUNT, 0U)) {
			return TEST_RESULT_FAIL;
		}
...
static test_result_t host_test_realm_pmuv3(uint8_t cmd)
{
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, sl, rec_flag, 1U, num_aux_planes)) {
		return TEST_RESULT_FAIL;
	}
	...
	if ((cmd != REALM_PMU_CYCLE_INTERRUPT) &&
			(cmd != REALM_PMU_EVENT_INTERRUPT)) {
		goto test_plane_n;
	}
...
test_result_t host_realm_abort_unassigned_destroyed(void)
{
	...
	if (!host_create_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, sl, rec_flag, 2U, 0U)) {
		return TEST_RESULT_FAIL;
	}
...
test_result_t host_realm_sea_empty(void)
{
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, sl, rec_flag, 4U, 0U)) {
		return TEST_RESULT_FAIL;
	}
...
test_result_t host_realm_sea_adr_fault(void)
{
	...
	if (!host_create_activate_realm_payload(&realm, (u_register_t)REALM_IMAGE_BASE,
			feature_flag, 0U, RTT_MIN_LEVEL, rec_flag, 4U, 0U)) {
		return TEST_RESULT_FAIL;
	}
