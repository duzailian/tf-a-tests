...
test_result_t host_realm_set_ripas(void)
{
	...
	top_base = (u_register_t)page_alloc(PAGE_SIZE * 3U);
	for (unsigned int i = 0U; i < 3U; i++) {
		ret = host_realm_map_protected_data(true, realm_ptr, top_base +
				(PAGE_SIZE * i), PAGE_SIZE, top_base + (PAGE_SIZE * i));
		if (ret != REALM_SUCCESS) {
			ERROR("host_realm_map_protected_data failed\n");
			goto destroy_realm;
		}
	}
	host_shared_data_set_host_val(0U, HOST_ARG1_INDEX, top_base);
	host_shared_data_set_host_val(0U, HOST_ARG2_INDEX, top_base + (PAGE_SIZE * 3U));
	...
	if (ret1) {
		...
		while (run->exit.ripas_base <= top_base + (PAGE_SIZE * 4U)) {
			INFO("host_rmi_rtt_set_ripas exit.ripas_base=0x%lx\n", run->exit.ripas_base);
