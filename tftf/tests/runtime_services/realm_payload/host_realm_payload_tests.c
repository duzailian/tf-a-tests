 ...
 */
test_result_t host_test_rtt_fold_unfold(void)
{
 ...
 * Host recreates L3, which should cause unfold operation
 */
test_result_t host_test_rtt_fold_unfold_assigned(void)
{

	bool ret1, ret2;
	test_result_t res = TEST_RESULT_FAIL;
	u_register_t ret, base, base_pa;
	struct realm realm;
	struct rtt_entry rtt;
	u_register_t rec_flag[] = {RMI_RUNNABLE};
	...
	for (unsigned int i = 0U; i < (512); i++) {
		ret = host_realm_map_unprotected(&realm, base_pa + (RTT_L2_BLOCK_SIZE * i), RTT_L2_BLOCK_SIZE);
		if (ret != RMI_SUCCESS) {
			ERROR("host_realm_map_unprotected failed base=0x%lx ret=0x%lx\n", base, ret);
			goto destroy_realm;
		}
	}
	...
	u_register_t  out_rtt, top;
	for (unsigned int i = 1U; i < (512); i++) {	
		ret = host_rmi_rtt_destroy(realm.rd, base + (PAGE_SIZE * i), 3U, &out_rtt, &top);
		if (ret != RMI_SUCCESS) {
			ERROR("host_rmi_rtt_destroy failed base=0x%lx\n", base + (PAGE_SIZE * i));
			break;
		}
		ret = host_rmi_granule_undelegate(out_rtt);
	}
	for (unsigned int i = 1U; i < (512); i++) {
		ret = host_rmi_rtt_destroy(realm.rd, base + (RTT_L2_BLOCK_SIZE * i), 2U, &out_rtt, &top);
		if (ret != RMI_SUCCESS) {
