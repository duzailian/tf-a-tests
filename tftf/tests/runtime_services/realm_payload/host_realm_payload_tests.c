...
void cpu_on_handler(void)
{
	...
	ret = host_enter_realm_execute(REALM_MULTIPLE_REC_CMD, NULL, RMI_EXIT_HOST_CALL, 1U);
	if (!ret) {
...
test_result_t host_realm_multi_rec(void)
{
	...
	INFO("Entering REC 0\n");
	ret1 = host_enter_realm_execute(REALM_MULTIPLE_REC_CMD, &realm_ptr, RMI_EXIT_HOST_CALL, 0U);
	run = (struct rmi_rec_run *)realm_ptr->run[0];
	if ((run->exit.exit_reason == RMI_EXIT_PSCI) &&
			(run->exit.gprs[0] == SMC_PSCI_CPU_ON_AARCH64)) {
		ret1 = host_rmi_psci_complete(realm_ptr->rec[0], realm_ptr->rec[1]);
		if (ret1 == 0) {
			...
			if (ret != PSCI_E_SUCCESS) {
				ERROR("Powering on %lx failed ret = %d \n", other_mpidr, ret1);
				return TEST_RESULT_FAIL;
			}
