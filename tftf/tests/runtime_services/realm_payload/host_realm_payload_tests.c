...
test_result_t host_realm_inst_fetch_unassigned(void)
{
	...
	if (ret != RMI_SUCCESS) {
		ERROR("host_rmi_data_destroy error\n");
		goto destroy_realm;
	}
	ret = host_rmi_rtt_readentry(realm_ptr->rd, base, 3U, &rtt);
	if (rtt.state != RMI_UNASSIGNED &&
			(rtt.ripas == RMI_DESTROYED)) {
		ERROR("host_rmi_data_destroy invalid state\n");
		goto destroy_realm;
	}
	host_shared_data_set_host_val(0U, HOST_ARG1_INDEX, base);
	...
	if (ret1) {
		...
		ret = host_rmi_rtt_set_ripas(realm_ptr->rd,
						     ...
						     run->exit.ripas_base + PAGE_SIZE,
						     &new_base);
		// ?? should have failed RSI_NO_CHANGE_DESTROYED  ??
		if (ret == RMI_SUCCESS) {
			INFO("set ripas success base=0x%lx, new_base=0x%lx\n", base, new_base);
			ret = host_realm_rec_enter(realm_ptr, &exit_reason, &host_call_result, 0U);
		}
