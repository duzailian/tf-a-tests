...
static fpu_reg_state_t fpu_state_send, fpu_state_receive;
...
test_result_t fpu_concurrent_in_rl_ns_se(void)
{
	...
	/*
	 * SIMD_NS_VALUE is just a template value to be distinguished from the
	 * value in the secure world.
	 */
	...
	for (uint32_t i = 0; i < 1000; i++)
	{
		...
		switch (cmd)
		{
			...
			case CMD_SIMD_NS_CMP:
				...
				if (!fpu_state_cmp(&fpu_state_send, &fpu_state_receive)) {
					ERROR("%s failed %d\n",__func__, __LINE__);
					fpu_state_print(&fpu_state_send);
					goto destroy_realm;
				}
				break;
