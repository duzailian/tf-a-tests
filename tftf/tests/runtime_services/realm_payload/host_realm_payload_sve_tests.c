...
#define CHECK_SVE_SUPPORT_IN_HW_AND_IN_RMI(_reg0)				\
	do {									\
		...
		if (EXTRACT(RMI_FEATURE_REGISTER_0_SVE_EN, _reg0) == 0UL) {	\
			ERROR("SVE not in RMI features, failing\n");		\
			return TEST_RESULT_FAIL;				\
		}								\
	} while (false)
...
test_result_t host_check_rmi_reports_proper_sve_vl(void)
{
	...
	if (rmi_sve_vq != ns_sve_vq) {
		ERROR("RMI max SVE VL %d bits doesn't matches NS max "
...
test_result_t host_sve_realm_cmd_rdvl(void)
{
	...
	if (sve_vq == rl_max_sve_vq) {
		...
		ERROR("Realm created with max VL: %d bits, but Realm reported "
...
test_result_t host_sve_realm_check_vectors_preserved(void)
{
	...
	val = rand();
	for (i = 0; i < NS_SVE_OP_ARRAYSIZE; i++) {
		ns_sve_op_1[i] = val - i;
		ns_sve_op_2[i] = 1;
	}
	...
	for (i = 0; i < SVE_TEST_ITERATIONS; i++) {
		...
		sve_subtract_arrays_interleaved(ns_sve_op_1, ns_sve_op_1,
						ns_sve_op_2, NS_SVE_OP_ARRAYSIZE,
	...
	for (i = 0; i < NS_SVE_OP_ARRAYSIZE; i++) {
		if (ns_sve_op_1[i] != (val - i - SVE_TEST_ITERATIONS)) {
			ERROR("SVE op failed at idx: %d, expected: 0x%x "
			      "received: 0x%x\n", i,
