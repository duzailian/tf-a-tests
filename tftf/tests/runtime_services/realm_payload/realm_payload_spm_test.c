...
static void waite_for_secondary_cores_to_turn_off(void)
{
 ...
 *Non secure word tftf can access secure word SP at S-EL1.
 */
...
test_result_t test_ffa_secondary_core_direct_realm_msg(void)
{
	...
	waite_for_secondary_cores_to_turn_off();
 ...
 *Non secure word tftf can access TRP(R-EL2) through  RMI and RMMD(EL3).
 */
 ...
 */
test_result_t serial_spm_rmi_msg_singel_core(void)
{
	if (get_armv9_2_feat_rme_support() == 0U) {
		return TEST_RESULT_SKIPPED;
	}
 ...
 */
test_result_t serial_spm_rmi_msg_singel_core_on_off(void)
{
	if (get_armv9_2_feat_rme_support() == 0U) {
		return TEST_RESULT_SKIPPED;
	}
