...
static test_result_t non_secure_call_realm(void)
{
	...
	unsigned int core_pos = platform_get_core_pos(mpid);
	test_result_t ret = TEST_RESULT_SUCCESS;
	ret =  realm_multi_cpu_payload_del_undel();
	/* Tell the lead CPU that the calling CPU has completed the test */
	tftf_send_event(&cpu_booted[core_pos]);
	return ret;
}
...
test_result_t serial_spm_rmi_msg(void)
{
	...
	/*
	 * Clear events state for the next usage.
	 */
	for (unsigned int i = 0U; i < PLATFORM_CORE_COUNT; i++) {
		tftf_init_event(&cpu_booted[i]);
	}
	...
	for (size_t i = 0; i < MAX_REPEATED_TEST; i++) {
		/* SPM FF-A direct message call */
		if (TEST_RESULT_SUCCESS != run_on_cpu(mpidr, (uintptr_t)non_secure_call_secure)) {
			return TEST_RESULT_FAIL;
		}
		VERBOSE("Wait for the target CPU to finish the test execution ...\n");
		/* Wait for the target CPU to finish the test execution */
		tftf_wait_for_event(&cpu_booted[platform_get_core_pos(mpidr)]);

		/* RMI call */
		if (TEST_RESULT_SUCCESS != run_on_cpu(mpidr, (uintptr_t)non_secure_call_realm)) {
			return TEST_RESULT_FAIL;
		}
		VERBOSE("Wait for the target CPU to finish the test execution ...\n");
		/* Wait for the target CPU to finish the test execution */
	...
	for (int i = 0; i < MAX_REPEATED_TEST; i++) {
		...
		for_each_cpu(cpu_node) {
			...
			if (spm_rmi_test(mpidr) == 1) {
				if (TEST_RESULT_SUCCESS != run_on_cpu(mpidr, (uintptr_t)non_secure_call_secure_multi_cpu_sync)) {
					return TEST_RESULT_FAIL;
				}
			} else {
				if (TEST_RESULT_SUCCESS != run_on_cpu(mpidr, (uintptr_t)non_secure_call_realm_multi_cpu_sync)) {
					return TEST_RESULT_FAIL;
				}
			}
		}
