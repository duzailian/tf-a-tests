...
static int cpu_cnt_mpidr[PLATFORM_CORE_COUNT];
static char bufferstate[NUM_GRANULES * PLATFORM_CORE_COUNT];
static int cpu_test_spm_rmi[PLATFORM_CORE_COUNT];

/*
 * The following test conducts SPM(direct messaging0 tests on a subset of selected CPU'S while
 ...
 */
int spm_rmi_test(unsigned int mpidr)
{
	return cpu_test_spm_rmi[get_cpu_node(mpidr)];
}
 ...
 */
test_result_t init_buffer_del_spm_rmi(void)
{
	u_register_t retrmm;
...
/*
 * SPM functions for the directe messaging
 */
...
test_result_t test_ffa_secondary_core_direct_realm_msg(void)
{
	...
	unsigned int cpu_node, mpidr, target_mpid;
	int32_t ret;
	int cpucnt;
	...
	/*
	 * Assign the mpidr to a CPU number
	 */
	cpucnt = 0;
	for_each_cpu(cpu_node) {
		target_mpid = tftf_get_mpidr_from_node(cpu_node) & MPID_MASK;
		cpu_cnt_mpidr[cpucnt] = target_mpid;
		cpucnt++;
	}

	/*
	 * Randomize the assignmet of the CPU's to either SPM or RMI
	 */
	rand_cpu_spm_rmi();
