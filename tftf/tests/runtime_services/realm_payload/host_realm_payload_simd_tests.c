...
static test_result_t ns_sve_read_and_compare(void)
{
	...
	bit_map = sve_p_regs_compare(&ns_sve_p_regs_write, &ns_sve_p_regs_read);
	if (bit_map != 0) {
...
static simd_test_t rl_simd_write_rand(bool rl_sve_en)
{
	...
	rc = host_enter_realm_execute(rl_fill_cmd, NULL, RMI_EXIT_HOST_CALL);
	assert(rc);
...
/*
 * Thie test case verifies whether various SIMD related registers like Q[0-31],
