...
test_result_t host_sve_realm_cmd_rdvl(void)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SVE_RDVL, &realm,
...
static test_result_t _host_sve_realm_check_id_registers(bool sve_en)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SVE_ID_REGISTERS, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
		rc = TEST_RESULT_FAIL;
		goto rm_realm;
...
test_result_t host_sve_realm_cmd_probe_vl(void)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SVE_PROBE_VL, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
		rc = TEST_RESULT_FAIL;
		goto rm_realm;
...
static bool callback_realm_do_sve(void)
{
	...
	return !host_enter_realm_execute(REALM_SVE_OPS, &realm,
					 RMI_EXIT_HOST_CALL, 0U);
}
...
{
	return !host_enter_realm_execute(REALM_REQ_FPU_FILL_CMD, &realm,
					 RMI_EXIT_HOST_CALL, 0U);
}
...
test_result_t host_sve_realm_check_vectors_leaked(void)
{
	...
	/* 4. Call Realm to fill in Z registers */
	realm_rc = host_enter_realm_execute(REALM_SVE_FILL_REGS, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
		rc = TEST_RESULT_FAIL;
		goto rm_realm;
...
test_result_t host_non_sve_realm_check_undef_abort(void)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SVE_UNDEF_ABORT, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
...
static simd_test_t rl_simd_write_rand(bool rl_sve_en)
{
	...
	rc = host_enter_realm_execute(rl_fill_cmd, &realm, RMI_EXIT_HOST_CALL, 0U);
	assert(rc);
...
static bool rl_simd_read_and_compare(simd_test_t type)
{
	...
	return host_enter_realm_execute(rl_cmp_cmd, &realm, RMI_EXIT_HOST_CALL,
					0U);
}
...
test_result_t host_realm_check_sme_id_registers(void)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SME_ID_REGISTERS, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
		rc = TEST_RESULT_FAIL;
		goto rm_realm;
...
test_result_t host_realm_check_sme_undef_abort(void)
{
	...
	realm_rc = host_enter_realm_execute(REALM_SME_UNDEF_ABORT, &realm,
					    RMI_EXIT_HOST_CALL, 0U);
	if (!realm_rc) {
		ERROR("Realm didn't receive undefined abort\n");
		rc = TEST_RESULT_FAIL;
