...
test_result_t host_realm_multi_rec_single_cpu(void)
{
	...
	if (!host_create_realm_payload((u_register_t)REALM_IMAGE_BASE,
			...
			(u_register_t)PAGE_POOL_MAX_SIZE,
			0UL, rec_flag, MAX_REC_COUNT, &realm)) {
	...
	for (unsigned int i = 0; i < MAX_REC_COUNT; i++) {
		...
		ret1 = host_enter_realm_execute(REALM_SLEEP_CMD, &realm,
...
test_result_t host_realm_multi_rec_psci_denied(void)
{
	...
	ret1 = host_enter_realm_execute(REALM_MULTIPLE_REC_PSCI_DENIED_CMD, &realm,
			RMI_EXIT_PSCI, 0U);
...
static test_result_t cpu_on_handler2(void)
{
	...
	ret = host_enter_realm_execute(REALM_LOOP_CMD, &realm,
