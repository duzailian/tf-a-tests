 ...
 * Test realm creation with 0 PMU counter than available
 * expect failure if FEAT_HPMN0 is not supported
 * expect success if FEAT_HPMN0 is supported
 * Create 2 Realms first with MAX PMU counters
 * second with lesser PMU counter than available
...
test_result_t host_realm_pmuv3_mul_rec(void)
{
	...
	/* Test 2 realms first with max PMU counters */
	feature_flag = RMI_FEATURE_REGISTER_0_PMU_EN |
			INPLACE(FEATURE_PMU_NUM_CTRS, num_cnts);

	/* Prepare realm0, create realm0-recs after creating realm1 */
	...
	/* Second realm with lesser num of PMU counters */
	feature_flag = RMI_FEATURE_REGISTER_0_PMU_EN |
			INPLACE(FEATURE_PMU_NUM_CTRS, num_cnts - 1U);
	...
	for (unsigned int i = 0U; i < MAX_REC_COUNT; i++) {
		host_shared_data_set_host_val(&realm, i, HOST_ARG1_INDEX, num_cnts);
		host_shared_data_set_host_val(&realm1, i, HOST_ARG1_INDEX, num_cnts - 4U);
	}
	...
	/* Test if Realm entering/exiting preserves PMU state */
	ret1 = host_enter_realm_execute(&realm, REALM_PMU_PRESERVE, RMI_EXIT_HOST_CALL, i);
	if (!ret1) {
		goto test_exit2;
	}
