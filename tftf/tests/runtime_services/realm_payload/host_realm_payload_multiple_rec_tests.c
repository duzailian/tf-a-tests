...
static test_result_t cpu_on_handler(void)
{
	...
	host_enter_realm_execute(REALM_MULTIPLE_REC_MULTIPLE_CPU_CMD, &realm_ptr, RMI_EXIT_PSCI, i);
...
test_result_t host_realm_multi_rec_multiple_cpu(void)
{
		...
		if (run->exit.gprs[0] == SMC_PSCI_CPU_ON_AARCH64) {
			...
			if (ret == RMI_SUCCESS) {
				do {
					...
					ret = tftf_cpu_on(other_mpidr, (uintptr_t)cpu_on_handler,
							0);
	...
	/*Try to run CPU OFF Rec, expect error */
	ret1 = host_realm_rec_enter(realm_ptr, &exit_reason,
			&host_call_result, 3U);
