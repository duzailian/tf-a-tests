...
static test_result_t plane_cpu_on_handler(void)
{
	...
	/* CMD for Plane 1 */
	host_shared_data_set_realm_cmd(&realm, REALM_MULTIPLE_REC_MULTIPLE_CPU_CMD, 1U, i);
...
test_result_t host_realm_multi_planes_multi_rec_multiple_cpu(void)
{
	...
	u_register_t exit_reason;
	unsigned int cpu_node, rec_count, num_aux_planes = 0U;
	u_register_t feature_flag = 0U;
	long sl = RTT_MIN_LEVEL;
	...
	/* CMD for Plane 1 */
	host_shared_data_set_realm_cmd(&realm, REALM_MULTIPLE_REC_MULTIPLE_CPU_CMD, 1U, 0U);
