...
test_result_t host_test_realm_no_lpa2_invalid_sl(void)
{
	...
	if (!host_create_activate_realm_payload(
		    ...
		    (u_register_t)PAGE_POOL_MAX_SIZE, 0UL, RTT_MIN_LEVEL_LPA2,
		    rec_flag, 1U)) {
...
test_result_t host_test_realm_no_lpa2_invalid_s2sz(void)
{
	...
	if (!host_create_activate_realm_payload(
		    ...
		    INPLACE(RMI_FEATURE_REGISTER_0_S2SZ, 50UL), RTT_MIN_LEVEL,
		    rec_flag, 1U)) {
		return TEST_RESULT_SUCCESS;
	}
...
test_result_t host_test_non_lpa2_realm_on_lpa2plat(void)
{
	...
	if (!host_create_activate_realm_payload(
		    ...
		    (u_register_t)PAGE_POOL_BASE,
		    (u_register_t)PAGE_POOL_MAX_SIZE,
		    INPLACE(RMI_FEATURE_REGISTER_0_S2SZ, 48UL), RTT_MIN_LEVEL,
		    rec_flag, 1U)) {
		return TEST_RESULT_FAIL;
	}
