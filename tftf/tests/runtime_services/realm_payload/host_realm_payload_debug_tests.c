 ...
 * BRPs [15:12] - no of bp's supported - 1
 * WRPs [23:20] - no of wp's supported - 1
 * The value 0b0000 is reserved. Implies that min bp's/wp's = 2
 * If the above fields read 0b1111 => read ID_AA64DFR1_EL1.
 * ID_AA64DFR1_EL1 indicates the no of bp's or wp's supported.
 * BRPs [15:8] ->  no of bp's supported - 1
 * WRPs [23:16] -> no of wp's supported - 1
 */
 ...
 * BP's or WP's lesser than that supported by hardware.
 * ID_AA64DFR0_BRPs - 0x0001 - 0x1111 (No of BP's - 1)
 * ID_AA64DFR1_BRPs - max supported 64BP's
 *
 */
test_result_t host_request_lesser_num_bps_wps(void)
{
	...
	INFO("BP's and WP's requested NS host for realm is %ld and %ld\n", num_bps+1, num_wps+1);
