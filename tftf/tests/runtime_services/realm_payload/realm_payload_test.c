...
static char bufferdelegate[NUM_GRANULES * GRANULE_SIZE] __aligned(GRANULE_SIZE);
...
/*
 * Select all CPU's to randomly deleate/undelegate
...
/*
 * Random Granule selection with 50% probability of
 * delegate versus undelegate for the multi CPU testing
 */
test_result_t realm_multi_cpu_payload_del_undel(void)
{
	u_register_t retrmm;

	for (int i = 0; i < (rand() % NUM_RANDOM_ITERATIONS); i++) {
		if ((rand() % 2) == 0) {
			retrmm = realm_granule_delegate((u_register_t)&bufferdelegate[(rand() % NUM_GRANULES) * GRANULE_SIZE]);
		} else {
			retrmm = realm_granule_undelegate((u_register_t)&bufferdelegate[(rand() % NUM_GRANULES) * GRANULE_SIZE]);
		}
		if (retrmm != 0UL) {
			tftf_testcase_printf("Delegate operation returns fail, %lx\n", retrmm);
			return TEST_RESULT_FAIL;
