...
test_result_t realm_multi_cpu_payload_del_undel(void)
{
	...
	for (int i = 0; i < NUM_GRANULES; i++) {
		if (bufferstate[((cpu_node * NUM_GRANULES) + i)] == B_UNDELEGATED) {
			retrmm = realm_granule_delegate((u_register_t)&bufferdelegate[((cpu_node * NUM_GRANULES) + i) * GRANULE_SIZE]);
			bufferstate[((cpu_node * NUM_GRANULES) + i)] = B_DELEGATED;
		} else {
			retrmm = realm_granule_undelegate((u_register_t)&bufferdelegate[((cpu_node * NUM_GRANULES) + i) * GRANULE_SIZE]);
...
test_result_t realm_fail_del(void)
{
	...
	if (retrmm == 0UL) {
		tftf_testcase_printf("Delegate operation does not fail as expected for double delegation, %lx\n", retrmm);
		return TEST_RESULT_FAIL;
	}
	...
	if (retrmm == 0UL) {
		tftf_testcase_printf("Delegate operation does not return fail for misaligned address, %lx\n", retrmm);
		return TEST_RESULT_FAIL;
	}
