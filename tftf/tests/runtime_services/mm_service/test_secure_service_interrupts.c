...
test_result_t test_secure_partition_interrupt_by_ns(void)
{
	...
	sps_request = create_sps_request(SPS_TIMER_SLEEP, &timer_delay,
					 sizeof(timer_delay));
	smc_args mm_communicate_smc = {MM_COMMUNICATE_AARCH64,
				       0, /* cookie, MBZ */
				       (uintptr_t)sps_request, 0};

	smc_ret_values smc_ret = tftf_smc(&mm_communicate_smc);

	INFO("Returned from Cactus, MM_COMMUNICATE_AARCH64 handling "
	     "complete\n");
	...
	if (cactus_response != CACTUS_FAST_REQUEST_SUCCESS) {
		tftf_testcase_printf(
			"Error code from the timer secure service: 0x%x\n",
			cactus_response);
		goto exit_test;
	}
