...
static test_result_t test_smccc_entrypoint(void)
{
	...
	case NOT_REQUIRED_DONOT_INVOKE:
		...
		tftf_testcase_printf(
			"SMCCC_ARCH_WORKAROUND_2 is not required\n");
		return TEST_RESULT_SKIPPED;
	...
	case NOT_SUPPORTED:
		 ...
		 */
		tftf_testcase_printf(
			"SMCCC_ARCH_WORKAROUND_2 is not supported\n");
		return TEST_RESULT_SKIPPED;
	...
	if (i == ARRAY_SIZE(entries)) {
		tftf_testcase_printf(
			"TFTF workaround table out of sync with TF\n");
		return TEST_RESULT_FAIL;
	}
...
test_result_t test_smccc_arch_workaround_2(void)
{
	...
	for_each_cpu(cpu_node)
	{
		...
		if (lead_mpid == target_mpid)
			continue;
		ret = tftf_cpu_on(target_mpid, (uintptr_t)test_smccc_entrypoint,
				  0);
