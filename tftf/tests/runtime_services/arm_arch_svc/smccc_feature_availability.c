...
static inline bool always_present()
{
	return true;
}

/* feat func should be is_feat_xyz_present(), but get_feat_xyz_support() will
 * also work
 * Macro will allow for a feature to be reported in the bitmask, without it being
 * in the system
 */
	...
	do {									\
		if (feat_func() && !(reg & bit)) {				\
			tftf_testcase_printf(					\
				#feat_func					\
				" says feature was supported but "		\
				#bit " was not set!\n");			\
...
test_result_t test_smccc_arch_feature_availability(void)
{
	...
	CHECK_BIT_SET(is_feat_d128_present,			SCR_D128En_BIT);
	CHECK_BIT_SET(is_feat_s1pie_present,			SCR_PIEN_BIT);
	CHECK_BIT_SET(is_feat_sctlr2_present,			SCR_SCTLR2En_BIT);
	CHECK_BIT_SET(is_feat_tcr2_present,			SCR_TCR2EN_BIT);
	...
	CHECK_BIT_SET(is_feat_trf_present,			MDCR_TTRF_BIT);
	...
	CHECK_BIT_SET(is_feat_mpam_present,		MPAM3_EL3_TRAPLOWER_BIT);
