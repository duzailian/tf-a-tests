 ...
 * to argument passed */
static smc_ret_values get_soc_id_params(u_register_t arg1)
{
	smc_args args;
	smc_ret_values ret;
...
test_result_t test_smccc_arch_soc_id(void)
{
	...
	/* If the call returns 0, if SMCCC_ARCH_SOC_ID is implemented */
	if ((int)ret.ret0 == SMC_OK) {
		soc_id_implemented = true;
	} else {
		ERROR("Invalid error during SMCCC_ARCH_FEATURES call\n");
		return TEST_RESULT_FAIL;
	}

	if (soc_id_implemented == true) {
		ret = get_soc_id_params(SMC_GET_SOC_REVISION);
		...
		if ((int)ret.ret0 == SMC_ARCH_CALL_INVAL_PARAM) {
			...
			tftf_testcase_printf("SOC Rev=%x\n", (int)ret.ret0);
		}

		ret = get_soc_id_params(SMC_GET_SOC_VERSION);
		...
		if((int)ret.ret0 == SMC_ARCH_CALL_INVAL_PARAM) {
			...
			tftf_testcase_printf("SOC Ver=%x\n", (int)ret.ret0);
		}
