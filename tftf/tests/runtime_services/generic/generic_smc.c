			    ...
			    const bool do_check[4], const bool allow_zeros[4])
{
	...
	if ((do_check[0] && (ret.ret0 != expect->ret0 && !(allow_zeros[0] && ret.ret0 == 0))) ||
	    (do_check[1] && (ret.ret1 != expect->ret1 && !(allow_zeros[1] && ret.ret1 == 0))) ||
	    (do_check[2] && (ret.ret2 != expect->ret2 && !(allow_zeros[2] && ret.ret2 == 0))) ||
	    (do_check[3] && (ret.ret3 != expect->ret3 && !(allow_zeros[3] && ret.ret3 == 0)))) {
		/*
		 * Build an error message where unchecked SMC return values are
		 * displayed as '*'.
		 */
		char expect_str[4][28];
#define BUILD_STR(_buf, _do_check, _allow_zero, _expect)			\
		if (_do_check) {						\
			if (_allow_zero) {					\
				snprintf(_buf, 20, "0x%lx or zero", _expect);	\
			} else {						\
		...
		BUILD_STR(expect_str[0], do_check[0], allow_zeros[0], expect->ret0);
		BUILD_STR(expect_str[1], do_check[1], allow_zeros[1], expect->ret1);
		BUILD_STR(expect_str[2], do_check[2], allow_zeros[2], expect->ret2);
		BUILD_STR(expect_str[3], do_check[3], allow_zeros[3], expect->ret3);
#undef BUILD_STR
		tftf_testcase_printf(
...
test_result_t smc64_yielding(void)
{
	...
	if (is_trusted_os_present(NULL)) {
		 ...
		 * should at least preserve or fill by zeroes the values of x1-x3.
		 */
