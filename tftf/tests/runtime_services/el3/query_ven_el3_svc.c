 ...
 *
 * This test targets the implementation of the vendor-specific el3 in the
 * Trusted Firmware. If it is interfaced with a different implementation then
 * this test will most likely fail because the values returned by the service
 * won't be the ones expected.
 *
...
test_result_t test_query_ven_el3_svc(void)
{
	...
	make_uuid_from_4words(&ven_el3_svc_uuid, ret.ret0, ret.ret1, ret.ret2,
			      ret.ret3);
	...
	if (ret.ret0 != SMC_UNKNOWN) {
		tftf_testcase_printf(
			"Querying Vendor-Specific el3 service call count"
	...
	if ((ret.ret0 != VEN_EL3_SVC_VERSION_MAJOR) ||
		...
		tftf_testcase_printf(
			"Vendor Specific El3 service reported wrong version: "
			"expected {%u.%u}, got {%llu.%llu}\n",
