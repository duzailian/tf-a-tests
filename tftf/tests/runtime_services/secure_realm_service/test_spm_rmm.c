...
test_result_t init_test(void)
{
	 ...
	 */
	if (!host_create_realm_payload((u_register_t)PLATFORM_REALM_IMAGE_BASE,
			(u_register_t)PLATFORM_MEMORY_POOL_BASE,
			(u_register_t)PLATFORM_MEMORY_POOL_SIZE,
			(u_register_t)PLATFORM_HEAP_REGION_BASE,
			(u_register_t)PLATFORM_HEAP_REGION_SIZE)) {
	 ...
	 */
	if (!host_create_shared_mem(PLATFORM_SHARED_REGION_BASE,
...
test_result_t test_sec_interrupt_can_preempt_rl(void)
{
	...
	/*
	 * Sleep for REALM_TIME_SLEEP ms. This ensures secure wdog timer triggers during this
	 * time.
	 */
	if(!host_enter_realm_and_sleep(REALM_TIME_SLEEP, REALM_SLEEP_CMD)) {
