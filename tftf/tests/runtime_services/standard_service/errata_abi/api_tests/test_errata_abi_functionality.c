...
#define EXTRACT_REV_VAR(x)    \
	(x & MIDR_REV_MASK) | \
		((x >> (MIDR_VAR_SHIFT - MIDR_REV_BITS)) & MIDR_VAR_MASK)
...
em_cpu_t cortex_A15_errata_list = {
	.cpu_pn = 0xC0F,
	.cpu_errata = {{816470, 0x30, 0xFF}, {827671, 0x30, 0xFF}, {-1}},
};

em_cpu_t cortex_A17_errata_list = {
	.cpu_pn = 0xC0E,
	.cpu_errata = {{852421, 0x00, 0x12}, {852423, 0x00, 0x12}, {-1}},
};

em_cpu_t cortex_A9_errata_list = {
	.cpu_pn = 0xC09,
	.cpu_errata = {{790473, 0x00, 0xFF}, {-1}},
};

em_cpu_t cortex_A35_errata_list = {
	.cpu_pn = 0xD04,
	.cpu_errata = {{855472, 0x00, 0x00}, {1234567, 0x00, 0x00}, {-1}},
};

em_cpu_t cortex_A53_errata_list = {
	.cpu_pn = 0xD03,
	.cpu_errata = {{819472, 0x00, 0x01},
		       {824069, 0x00, 0x02},
		       {826319, 0x00, 0x02},
		       {827319, 0x00, 0x02},
		       {835769, 0x00, 0x04},
...
em_cpu_t cortex_A73_errata_list = {
	.cpu_pn = 0xD09,
	.cpu_errata = {{852427, 0x00, 0x00}, {855423, 0x00, 0x01}, {-1}},
};

em_cpu_t cortex_A75_errata_list = {
	.cpu_pn = 0xD0A,
	.cpu_errata = {{764081, 0x00, 0x00}, {790748, 0x00, 0x00}, {-1}},
};

em_cpu_t cortex_A76_errata_list = {
	.cpu_pn = 0xD0B,
	.cpu_errata = {{1073348, 0x00, 0x10},
		       {1130799, 0x00, 0x20},
		       {1165522, 0x00, 0xFF},
		       {1220197, 0x00, 0x20},
		       {1257314, 0x00, 0x30},
...
em_cpu_t cortex_X4_errata_list = {
	.cpu_pn = 0xD82,
	.cpu_errata = {{2701112, 0x00, 0x00}, {-1}},
};

em_cpu_t cortex_A715_errata_list = {
	.cpu_pn = 0xD4D,
	.cpu_errata = {{2561034, 0x10, 0x10}, {-1}},
};

em_cpu_t neoverse_V2_errata_list = {
	.cpu_pn = 0xD4F,
	.cpu_errata = {{2331132, 0x00, 0x02},
		       {2618597, 0x00, 0x01},
		       {2662553, 0x00, 0x01},
		       {2719103, 0x00, 0x01},
		       {2719103, 0x00, 0x01},
...
em_cpu_t cortex_A520_errata_list = {
	.cpu_pn = 0xD80,
	.cpu_errata = {{2630792, 0x00, 0x01}, {2858100, 0x00, 0x01}, {-1}},
};
...
test_result_t test_em_cpu_features(void)
{
	...
	for (int i = 0;
	     i < ERRATA_COUNT && cpu_ptr->cpu_errata[i].em_errata_id != -1;
	     i++) {
		ret_val = tftf_em_abi_cpu_feature_implemented(
			cpu_ptr->cpu_errata[i].em_errata_id, FORWARD_FLAG_EL1);

		switch (ret_val.ret0) {
		case EM_NOT_AFFECTED: {
			return_val =
				(RXPX_RANGE(rxpx_val_extracted,
					    cpu_ptr->cpu_errata[i].rxpx_low,
		...
		default: {
			ERROR("Return value did not match the expected "
			      "returns\n");
...
test_result_t test_errata_abi_features(void)
{
	...
	for_each_cpu(cpu_node)
	{
		...
		psci_ret = tftf_cpu_on(cpu_mpid,
				       (uintptr_t)test_em_cpu_features, 0);
