...
/*
 * @Test_Aim@ RND32 Meets the Zero-fill requirements of the spec
 */
test_result_t test_trng_rnd32(void)
{
	int32_t version = tftf_trng_version();
	...
	if (!tftf_trng_feature_implemented(SMC_TRNG_RND_AARCH32)) {
		return TEST_RESULT_SKIPPED;
	}
	rnd_out = tftf_trng_rnd32(U(0));
	if (rnd_out.ret0 != TRNG_E_INVALID_PARAMS) {
		ERROR("RND 0 returned 0x%x\n", (uint32_t) rnd_out.ret0);
		return TEST_RESULT_FAIL;
	}
	rnd_out = tftf_trng_rnd32(U(1024));
	if (rnd_out.ret0 != TRNG_E_INVALID_PARAMS) {
		ERROR("RND 1024 returned 0x%x\n", (uint32_t) rnd_out.ret0);
		return TEST_RESULT_FAIL;
	}
	rnd_out = tftf_trng_rnd32(U(1));
	 ...
	 * significant bit to be random by passing a "1" to the RND32 function
	 */
...
/*
 * @Test_Aim@ RND64 Meets the Zero-fill requirements of the spec
 */
test_result_t test_trng_rnd64(void)
{
	int32_t version = tftf_trng_version();
	smc_ret_values rnd_out;
	...
	if (!tftf_trng_feature_implemented(SMC_TRNG_RND_AARCH64)) {
		return TEST_RESULT_SKIPPED;
	}
	rnd_out = tftf_trng_rnd64(U(0));
	if (rnd_out.ret0 != TRNG_E_INVALID_PARAMS) {
		ERROR("RND 0 returned 0x%lx\n", rnd_out.ret0);
		return TEST_RESULT_FAIL;
	}
	rnd_out = tftf_trng_rnd64(U(1024));
	if (rnd_out.ret0 != TRNG_E_INVALID_PARAMS) {
		ERROR("RND 1024 returned 0x%lx\n", rnd_out.ret0);
		return TEST_RESULT_FAIL;
	}
	rnd_out = tftf_trng_rnd64(U(1));
	if (rnd_out.ret0 == TRNG_E_NO_ENTOPY) {
		WARN("There is not a single bit of entropy\n");
		return TEST_RESULT_SKIPPED;
	}
	...
	/* Note: there's an "E" suffix here  ^, because we asked for the least
	 * significant bit to be random by passing a "1" to the RND64 function
	 */
		ERROR("non-zero x3 value 0x%lx\n", rnd_out.ret3);
		return TEST_RESULT_FAIL;
	}
	return TEST_RESULT_SUCCESS;
}
