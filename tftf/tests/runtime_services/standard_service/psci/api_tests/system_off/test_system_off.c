...
{
	unsigned int curr_mpid = read_mpidr_el1() & MPID_MASK;
	unsigned int cpu_node, mpid;
        smc_args args = { SMC_PSCI_SYSTEM_OFF };

	/* Wait for the lead CPU to turn OFF.
	 * All CPU's other than lead are OFF at the start
	 * of the test
	 */
...
test_result_t test_system_off_cpu_other_than_lead(void)
{
	...
	INFO("Lead CPU to be turned off MPID: 0x%x\n", lead_mpid);
	tftf_cpu_off();
