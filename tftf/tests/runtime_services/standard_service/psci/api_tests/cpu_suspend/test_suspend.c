...
test_result_t test_psci_suspend_invalid(void)
{
	...
	/*
	 * This test requires at least two clusters. Also, it requires the state
	 * id encoding to not be null, because otherwise the suspend requests
	 * are always accepted regardless of which CPUs are still running.
	 */
	if (
		tftf_get_total_aff_count(MPIDR_AFFLVL1) < 2
		|| tftf_is_psci_state_id_null()
	) {
		return TEST_RESULT_SKIPPED;
	}
	...
	rc = test_init(MPIDR_AFFLVL0, PSTATE_TYPE_POWERDOWN, true);
	if (rc != TEST_RESULT_SUCCESS)
		return rc;
