		...
		uint64_t *count_diff, unsigned int *cpu_on_hits_on_target)
{
	...
	} while ((ret == PSCI_E_ALREADY_ON) && (read_cntpct_el0() < (start_time + read_cntfrq_el0())));
	...
	*count_diff = (uint64_t)(read_cntpct_el0() - start_time);
...
test_result_t psci_trigger_peer_cluster_cache_coh(void)
{
	...
	if (diff_test > (diff_baseline + (diff_baseline / BASELINE_VARIANCE)))
		tftf_testcase_printf("Variance of %d per-cent from baseline detected. Possibility of CPU ON issue\n",
				100 / BASELINE_VARIANCE);
