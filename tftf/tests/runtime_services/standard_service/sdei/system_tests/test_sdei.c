...
static inline test_result_t sdei_event_signal_self(bool enable, bool unmask)
{
	long long ret;
...
test_result_t test_sdei_event_signal_state(void)
{
	...
	if (sdei_event_signal_self(true, true) == TEST_RESULT_FAIL) {
		tftf_testcase_printf("SDEI event signaling failed when "
	...
	if (sdei_event_signal_self(false, true) == TEST_RESULT_SUCCESS) {
		tftf_testcase_printf("SDEI event signaling succeeded when "
