...
static test_result_t sdei_event_check_pstate(void)
{
	...
	if (daif != all_interrupts_masked) {
		tftf_testcase_printf("Interrupts were not correctly masked during SDEI event signal\n" \
				     "Expected DAIF: 0x%lx, Actual DAIF: 0x%lx\n", all_interrupts_masked, daif);
		ret = -1;
		goto err1;
	}
	u_register_t use_sp_elx = 0x1;
	if (sp != use_sp_elx) {
		tftf_testcase_printf("The SPSel PSTATE Bit was not set correctly during SDEI event signal\n" \
				     "Expected SPSel: 0x%lx, Actual SPSel: 0x%lx\n", use_sp_elx, sp);
		ret = -1;
		goto err1;
	}
	...
	if (is_armv8_1_pan_present()) {
		...
		/* Check that when the SPAN bit is 0 the PAN PSTATE bit is maintained */
		/* When PAN bit is 0 */
		...
		sdei_handler_done();
		if (pan != expected_pan) {
			tftf_testcase_printf("PAN PSTATE bit not mainted during SDEI event signal\n" \
					     "Expected PAN: 0x%lx, Actual PAN: 0x%lx\n", expected_pan, pan);
			ret = -1;
			goto err1;
		}
		...
		expected_pan = 0;
		/* Check that when the HCR_EL2.{E2H, TGE} != {1, 1} the PAN bit is maintained */
		ret = sdei_event_signal(read_mpidr_el1());
		if (ret < 0) {
			tftf_testcase_printf("SDEI event signal failed: 0x%llx\n", ret);
			goto err2;
		...
		sdei_handler_done();
		if (pan != expected_pan) {
			tftf_testcase_printf("PAN PSTATE bit not mainted during SDEI event signal when the \n" \
					     "SPAN bit is set and HCR_EL2.{E2H, TGE} != {1, 1}\n" \
					     "Expected PAN: 0x%lx, Actual PAN: 0x%lx\n", expected_pan, pan);
			ret = -1;
			goto err1;
		}
		...
		/* Check that when the HCR_EL2.{E2H, TGE} = {1, 1} PAN bit is forced to 1 */
		/* Set E2H Bit */
		...
		sdei_handler_done();
		if (pan != PAN_BIT) {
			tftf_testcase_printf("PAN PSTATE bit was not forced to 1 during SDEI event signal\n" \
					     "when the SPAN bit is set and HCR_EL2.{E2H, TGE} = {1, 1}\n");
			ret = -1;
			goto err1;
		}
	...
	/* Check that the DIT PSTATE bit is maintained during the event signal */
	if(is_armv8_4_dit_present()){
		...
		sdei_handler_done();
		if (dit != expected_dit) {
			tftf_testcase_printf("DIT PSTATE bit not mainted during SDEI event signal\n" \
					     "Expected DIT: 0x%lx, Actual DIT: 0x%lx\n", expected_dit, dit);
			ret = -1;
			goto err1;
		}
