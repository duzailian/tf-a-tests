...
/*
 * Test that the sdei_interrupt_bind function doesn't crash when too many events
 * are bound, but returns an error instead.
 */
test_result_t test_sdei_bind_failure(void)
{
	...
	for (intr = 16; intr < 20; intr++) {
		...
		if (bind_failed != last_call) {
			printf(
				"%d: SDEI interrupt bind; ret=%lld; Bind "
				"should have %s\n",
				__LINE__,
				ev,
