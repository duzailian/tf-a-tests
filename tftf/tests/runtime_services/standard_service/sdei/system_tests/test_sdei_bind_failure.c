 ...
 */
#define NUM_EVENTS_ALLOWED_IN_PPI_RANGE		U(4)
...
test_result_t test_sdei_bind_failure(void)
{
	...
	for (intr = MIN_PPI_ID; intr < MIN_PPI_ID + NUM_EVENTS_ALLOWED_IN_PPI_RANGE; intr++) {
		 ...
		 */
		bind_failed = ev < 0;
		last_call = intr == MIN_PPI_ID + NUM_EVENTS_ALLOWED_IN_PPI_RANGE - 1;
