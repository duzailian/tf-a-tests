...
CASSERT((MAX_NS_IMAGE_SIZE > TFTF_MAX_IMAGE_SIZE),\
		error_MAX_NS_IMAGE_SIZE_less_then_TFTF_MAX_IMAGE_SIZE);
...
static bool host_enter_realm(u_register_t *exit_reason, unsigned int *test_result)
{
	...
	if (!realm_payload_created) {
		ERROR("host_enter_realm failed, Realm not created\n");
		return false;
	}
	if (!shared_mem_created) {
		ERROR("host_enter_realm failed, shared memory not created\n");
		return false;
	}
		...
		u_register_t realm_pages_size)
{
	...
	if (page_pool_init(plat_mem_pool_adr, realm_pages_size) != HEAP_INIT_SUCCESS) {
	...
	if (realm_init_ipa_state(&realm, 0U, 0U, 1ULL << 32) != RMI_SUCCESS) {
...
bool host_enter_realm_and_sleep(unsigned int time_ms, uint8_t cmd)
{
	exit_reason = RMI_EXIT_INVALID;
	test_result = TEST_RESULT_FAIL;

	realm_shared_data_clear_realm_val();
	realm_shared_data_set_host_val(1, time_ms);
	...
	/* Re-enter the Realm payload to continue executing the spinning loop */
		return host_enter_realm(&exit_reason, &test_result);
	}
	ERROR("host_enter_realm_and_sleep exit_reason:[0x%lx] not RMI_EXIT_FIQ,"
		"test_result:[0x%x]\n",
		exit_reason,
		test_result);
	return false;
