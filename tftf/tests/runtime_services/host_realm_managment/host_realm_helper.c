				...
				u_register_t feature_flag)
{
	...
	s2sz = (uint8_t)EXTRACT(RMM_FEATURE_REGISTER_0_S2SZ, feature_flag);
	if (s2sz == RMM_OVERWRITE_S2SL) {
		/*
		 * We use an invalid value on S2SZ to indicate that we want to
		 * overwrite the stage 2 starting level.
		 *
		 * The starting level is overriden in the following way:
		 *	* New value == 0 if current value == -1
		 *	* New value == -1 if current value == 0
		 *	* Ignored otherwise.
		 */
		if (realm.s2sl == -1L) {
			realm.s2sl = 0L;
		...
		realm.rmm_feat_reg0 &= ~MASK(RMM_FEATURE_REGISTER_0_S2SZ);
		realm.rmm_feat_reg0 |=
			INPLACE(RMM_FEATURE_REGISTER_0_S2SZ, s2sz);
	}
