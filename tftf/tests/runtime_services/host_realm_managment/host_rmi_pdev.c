...
static int host_tdi_pdev_setup(struct host_tdi *tdi)
{
	...
	if (ret != RMI_SUCCESS) {
		ERROR("PDEV delegate failed 0x%x\n", ret);
		return -1;
	}
	...
	if (ret != RMI_SUCCESS) {
		ERROR("host_rmi_pdev_aux_count() failed 0x%x\n", ret);
		return -1;
	}
	...
	for (i = 0; i < tdi->pdev_aux_num; i++) {
		...
		if (ret != RMI_SUCCESS) {
			ERROR("Aux granule delegate failed 0x%x\n", ret);
			return -1;
		}
	}
...
static int host_tdi_pdev_reclaim(struct host_tdi *tdi)
{
	...
	/* TODO : Add undelegate for all the delegated pages. */
