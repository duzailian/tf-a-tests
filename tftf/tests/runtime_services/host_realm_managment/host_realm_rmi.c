...
{
	return (host_rmi_handler(&(smc_args){RMI_PSCI_COMPLETE, calling_rec, target_rec}, 3U)).ret0;
}
...
static u_register_t host_realm_alloc_rec_aux(struct realm *realm,
						struct rmi_rec_params *params, u_register_t i)
{
	u_register_t ret;
	unsigned int j;
	...
	for (j = 0U; j < realm->num_aux; j++) {
		...
		realm->aux_pages[j + (i * realm->num_aux)] = params->aux[j];
	}
	return RMI_SUCCESS;
err_free_mem:
