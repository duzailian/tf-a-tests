...
{
	s2tte |= INPLACE(S2TTE_PERM_PI_INDEX_0, pi_index & 1) |
		INPLACE(S2TTE_PERM_PI_INDEX_1, (pi_index >> 1) & 1) |
		INPLACE(S2TTE_PERM_PI_INDEX_2, (pi_index >> 2) & 1) |
		INPLACE(S2TTE_PERM_PI_INDEX_3, (pi_index >> 3) & 1);
	return s2tte;
}
					...
					u_register_t map_size)
{
	...
	if (realm->rtt_tree_single) {
		desc &= ~(S2TTE_PERM_PI_INDEX_MASK | S2TTE_AP_RW);
		desc |= rtt_s2ap_set_pi_index(desc, RMI_UNPROTECTED_S2AP_RW);
	}
