 ...
 *
 * SPDX-License-Identifier: BSD-3UL-Clause
 *
 */
...
static inline u_register_t rmi_data_create(u_register_t data, u_register_t rd,
	u_register_t map_addr, u_register_t src)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_DATA_CREATE,
		data, rd, map_addr, src, 0UL, 0UL, 0UL))).ret0;
}

static inline u_register_t rmi_data_create_unknown(u_register_t data,
	u_register_t rd, u_register_t map_addr)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_DATA_CREATE_UNKNOWN,
		data, rd, map_addr, 0UL, 0UL, 0UL, 0UL))).ret0;
}

static inline u_register_t rmi_realm_activate(u_register_t rd)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_REALM_ACTIVATE,
		rd, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL))).ret0;
}

u_register_t rmi_realm_create(u_register_t rd, u_register_t params_ptr)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_REALM_CREATE,
		rd, params_ptr, 0UL, 0UL, 0UL, 0UL, 0UL))).ret0;
}

u_register_t rmi_realm_destroy(u_register_t rd)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_REALM_DESTROY,
		rd, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL))).ret0;
}
...
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_DATA_DESTROY,
...
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_REC_CREATE,
			rec, rd, params_ptr, 0UL, 0UL, 0UL, 0UL))).ret0;
}

static inline u_register_t rmi_rec_destroy(u_register_t rec)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_REC_DESTROY,
...
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_CREATE,
...
static inline u_register_t rmi_rtt_destroy(u_register_t rtt, u_register_t rd,
	u_register_t map_addr, u_register_t level)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_DESTROY,
		rtt, rd, map_addr, level, 0UL, 0UL, 0UL))).ret0;
}
...
{
	smc_ret_values rets;
	rets = asm_tftf_smc64(RMI_FEATURES, index, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL);
	*features = rets.ret1;
	return rets.ret0;
}
...
static inline u_register_t rmi_rtt_init_ripas(u_register_t rd,
	...
	u_register_t level)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_INIT_RIPAS,
...
static inline u_register_t rmi_rtt_fold(u_register_t rtt,u_register_t rd,
	u_register_t map_addr, u_register_t level)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_FOLD,
		rtt, rd, map_addr, level, 0UL, 0UL, 0UL))).ret0;
}
...
{
	smc_ret_values rets;
	rets = asm_tftf_smc64(RMI_REC_AUX_COUNT, rd, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL);
	*aux_count = rets.ret1;
	return rets.ret0;
}
...
static inline u_register_t rmi_rtt_set_ripas(u_register_t rd, u_register_t rec,
	...
	u_register_t ripas)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_SET_RIPAS,
...
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_MAP_UNPROTECTED,
	...
	u_register_t level, struct rtt_entry *rtt)
{
	...
	rets = asm_tftf_smc64(RMI_RTT_READ_ENTRY,
...
static inline u_register_t rmi_rtt_unmap_unprotected(u_register_t rd,
	...
	u_register_t level, u_register_t ns_pa)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_RTT_UNMAP_UNPROTECTED,
		rd, map_addr, level, ns_pa, 0UL, 0UL, 0UL))).ret0;
}
...
static u_register_t realm_map_protected_data(struct realm *realm,
	...
	u_register_t src_pa)
{
	...
	for (size = 0UL; size < map_size; size += PAGE_SIZE) {
		...
		ret = rmi_data_create(phys, rd, map_addr, src_pa);
	...
	u_register_t map_size)
{
	...
	for (size = 0UL; size < map_size; size += PAGE_SIZE) {
		ret = rmi_granule_delegate(phys);
		if (ret != RMI_SUCCESS) {
			ERROR("Granule delegation failed, PA=0x%lx\n", phys);
			return REALM_ERROR;
		}

		ret = rmi_data_create_unknown(phys, rd, map_addr);
		...
		if (ret != RMI_SUCCESS) {
			ERROR("rmi_data_create_unknown failed, ret=0x%lx\n", ret);
			goto err;
		}
...
err:
	while (size >= PAGE_SIZE) {
		...
		if (ret != RMI_SUCCESS) {
			/* Page can't be returned to NS world so is lost */
			ERROR("rmi_granule_undelegate failed, ret=0x%lx\n", ret);
		}
		phys -= PAGE_SIZE;
		size -= PAGE_SIZE;
		map_addr -= PAGE_SIZE;
	...
	u_register_t map_size)
{
	...
	u_register_t desc = phys | S2TTE_ATTRS;
...
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_GRANULE_DELEGATE,
			addr, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL))).ret0;
}

u_register_t rmi_granule_undelegate(u_register_t addr)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_GRANULE_UNDELEGATE,
			addr, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL))).ret0;
}

u_register_t rmi_version(void)
{
	return ((smc_ret_values)(asm_tftf_smc64(RMI_VERSION,
...
u_register_t realm_create(struct realm *realm)
{
	...
	realm->par_size = PLATFORM_REALM_IMAGE_SIZE;
	...
	if (realm->par_base == HEAP_INVALID_ADDR) {
	...
	if (realm->rtt_addr == HEAP_INVALID_ADDR) {
	...
	u_register_t realm_payload_adr)
{
	...
	while (i < (realm->par_size / PAGE_SIZE)) {
		ret =	realm_map_protected_data(realm,
		...
		struct rmi_rec_params * params)
{
	...
	for (i = 0; i < realm->num_aux; i++) {
		...
		if (params->aux[i] == HEAP_INVALID_ADDR) {
...
{
	struct rmi_rec_params *rec_params = HEAP_INVALID_ADDR;
	u_register_t ret;

	/* Allocate memory for run object */
	realm->run = (u_register_t)page_alloc(PAGE_SIZE);
	if (realm->run == HEAP_INVALID_ADDR) {
	...
	if (realm->rec == HEAP_INVALID_ADDR) {
		...
		unsigned int *test_result)
{
	...
	do {
		re_enter_rec = false;
		ret = ((smc_ret_values)(asm_tftf_smc64(RMI_REC_ENTER,
				realm->rec, realm->run,
