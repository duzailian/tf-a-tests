...
u_register_t host_realm_create(struct realm *realm)
{
	 ...
	 * default MECID (0) first.
	 */
	if (is_feat_mec_supported() &&
			(host_rmi_features(1UL, &realm->rmm_feat_reg1)
				== REALM_SUCCESS) &&
			(realm->rmm_feat_reg1 > DEFAULT_MECID)) {
...
u_register_t host_realm_destroy(struct realm *realm)
{
	 ...
	 * default MECID (0) first.
	 */
	if (is_feat_mec_supported() && (realm->rmm_feat_reg1 > DEFAULT_MECID)) {
		mecid_count--;
	}
