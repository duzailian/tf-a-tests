...
{
	return host_rmi_handler(&(smc_args) {SMC_RMI_GRANULE_DEV_DELEGATE, addr}, 2U).ret0;
}

u_register_t host_rmi_granule_dev_undelegate(u_register_t addr)
{
	return host_rmi_handler(&(smc_args) {SMC_RMI_GRANULE_DEV_UNDELEGATE, addr}, 2U).ret0;
}
