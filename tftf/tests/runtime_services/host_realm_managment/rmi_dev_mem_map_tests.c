...
#define DEV_BLOCK_LEVEL		2L

#define NUM_INFO_TESTS		2U
...
const uintptr_t dev_mem_buffer = (const uintptr_t)PCIE_MEM_2_BASE;

u_register_t host_realm_dev_mem_map(struct realm *realm,
				    u_register_t dev_pa,
				    long map_level,
...
test_result_t host_realm_dev_mem_map_unmap(void)
{
	...
	struct dev_mem_info mem_info[NUM_INFO_TESTS] = {
		{0UL, RTT_MAX_LEVEL, RTT_MAP_SIZE(RTT_MAX_LEVEL), NUM_L3_REGIONS},
		{0UL, DEV_BLOCK_LEVEL, RTT_MAP_SIZE(DEV_BLOCK_LEVEL), NUM_L2_REGIONS}
	};
	unsigned int num[NUM_INFO_TESTS] = {0U, 0U};
	...
	mem_info[0].base_pa = dev_mem_buffer + offset * GRANULE_SIZE;
	...
	mem_info[1].base_pa = dev_mem_buffer + offset * RTT_L2_BLOCK_SIZE;

	/* Delegate device granules */
	for (i = 0U; i < NUM_INFO_TESTS; i++) {
		/* Number of granules in mapping region */
		unsigned int num_granules = mem_info[i].map_size / GRANULE_SIZE;

		for (; num[i] < num_granules * mem_info[i].num_regions; num[i]++) {
			res = host_rmi_granule_delegate(mem_info[i].base_pa +
							num[i] * GRANULE_SIZE);
			if (res != RMI_SUCCESS) {
				tftf_testcase_printf(
					"Delegate operation for 0x%lx failed, 0x%lx\n",
	...
	for (i = 0U; i < NUM_INFO_TESTS; i++) {
		for (j = 0U; j < mem_info[i].num_regions; j++) {
			res = host_realm_dev_mem_map(&realm,
		...
		for (j = 0U; j < mem_info[i].num_regions; j++) {
			...
			if ((rtt.state != RMI_ASSIGNED_DEV) ||
				...
				tftf_testcase_printf("%s() for 0x%lx failed, "
					...
					"host_rmi_rtt_readentry",
					map_addr[i][j], RMI_ASSIGNED_DEV, RMI_EMPTY,
					rtt.state, rtt.ripas);
				goto undelegate_granules;
			}
		}
...
undelegate_granules:
	for (i = 0U; i < NUM_INFO_TESTS; i++) {
		for (j = 0U; j < num[i]; j++) {
			...
			if (res != RMI_SUCCESS) {
				tftf_testcase_printf("Udelegate operation for 0x%lx failed, 0x%lx\n",
							(mem_info[i].base_pa + j * GRANULE_SIZE), res);
