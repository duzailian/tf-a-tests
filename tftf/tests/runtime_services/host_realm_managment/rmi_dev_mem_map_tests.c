...
test_result_t host_realm_dev_mem_map_unmap(void)
{
	...
	struct dev_mem_info mem_info[NUM_INFO_TESTS] = {
		...
		{PCIE_MEM_1_BASE, RTT_MAX_LEVEL,
			RTT_MAP_SIZE(RTT_MAX_LEVEL), NUM_L3_REGIONS},
		/* PCIe memory region 2 */
		{PCIE_MEM_2_BASE, RTT_MAX_LEVEL,
			RTT_MAP_SIZE(RTT_MAX_LEVEL), NUM_L3_REGIONS},
		/* PCIe memory region 2 */
		{PCIE_MEM_2_BASE, RTT_MIN_DEV_BLOCK_LEVEL,
	...
	assert(dev_size >= mem_info[0].num_regions * mem_info[0].map_size);
	...
	assert(dev_size >= mem_info[1].num_regions * mem_info[1].map_size);
	assert(dev_size >= mem_info[2].num_regions * mem_info[2].map_size);
