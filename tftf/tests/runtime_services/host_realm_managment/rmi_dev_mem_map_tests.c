...
struct dev_mem_info {
	...
	size_t max_size;		/* Dev memory region size required */
...
test_result_t host_realm_dev_mem_map_unmap(void)
{
	...
	if (num_reg == 1U) {
		...
		if (mem_region[0].size < mem_info[0].max_size) {
			INFO("PCIe memory region 0 too small\n");
			return TEST_RESULT_SKIPPED;
		}

		if (mem_region[0].size <
		    mem_info[1].max_size + mem_info[2].max_size) {
			/* Setup test region 1 */
		...
		if (mem_region[reg_b].size < mem_info[0].max_size) {
			INFO("PCIe memory regions too small\n");
			return TEST_RESULT_SKIPPED;
		}

		if (mem_region[reg_s].size >= mem_info[0].max_size) {
