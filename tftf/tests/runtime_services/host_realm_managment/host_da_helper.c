...
static int host_pdev_notify(struct host_pdev *h_pdev, unsigned long event)
{
	u_register_t ret;

	ret = host_rmi_pdev_notify((u_register_t)h_pdev->pdev,
				   (u_register_t)event);
	if (ret != RMI_SUCCESS) {
		return -1;
	}
				...
				unsigned char to_state)
{
	...
	case RMI_PDEV_STATE_COMMUNICATING:
		rc = host_pdev_notify(h_pdev, RMI_PDEV_EVENT_IDE_KEY_REFRESH);
		break;
...
int tsm_disconnect_device(struct host_pdev *h_pdev)
{
	int rc;
	u_register_t state;
	unsigned char tsm_disconnect_flow[] = {
...
{
	int rc;
	unsigned char tsm_connect_flow[] = {
		RMI_PDEV_STATE_NEW,
		RMI_PDEV_STATE_NEEDS_KEY,
