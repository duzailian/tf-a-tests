...
test_result_t test_spci_direct_messaging(void)
{
	 ...
	 */
	if (ret_values.ret3 != (DIRECT_MSG_TEST_PATTERN1 | 1)) {
		return TEST_RESULT_FAIL;
	}
	...
	/* Send a message to SP2 through direct messaging */
	ret_values = spci_msg_send_direct_req(HYP_ID, SP_ID(2),
					      DIRECT_MSG_TEST_PATTERN2);
	 ...
	 * into the lower 16 bits of initial message.
	 */
	if (ret_values.ret3 != (DIRECT_MSG_TEST_PATTERN2 | 2)) {
		return TEST_RESULT_FAIL;
	}
	...
	if (ret_values.ret3 != (DIRECT_MSG_TEST_PATTERN3 | 1)) {
		return TEST_RESULT_FAIL;
	}
