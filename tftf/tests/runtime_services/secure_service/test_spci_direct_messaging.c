...
test_result_t test_spci_direct_messaging(void)
{
	...
	/* Send a message to SP1 through direct messaging */
	ret_values = spci_msg_send_direct_req(HYP_ID, SP_ID(1), 0xaaaa0000);
	 ...
	 * on boot up so we explicitelly run the SP once by invoking SPCI_RUN so
	 * it reaches spci_msg_wait in the message loop function.
	 */

	/* Run SP2 on VCPU0 */
	ret_values = spci_run(2 /*SP_ID(2)*/, 0);
	...
	/* Send a message to SP2 through direct messaging */
	ret_values = spci_msg_send_direct_req(HYP_ID, SP_ID(2), 0xbbbb0000);
	...
	/* Send a second message to SP1 through direct messaging */
	ret_values = spci_msg_send_direct_req(HYP_ID, SP_ID(1), 0xcccc0000);
