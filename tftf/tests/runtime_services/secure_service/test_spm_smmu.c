...
test_result_t test_smmu_spm(void)
{
	...
	ret = cactus_send_dma_cmd(HYP_ID, SP_ID(1), PLAT_CACTUS_MEMCPY_BASE);
...
test_result_t test_spm_smmu_invalid_access(void)
{
	...
	retmm = realm_granule_delegate((u_register_t)0x82000000);
	if (retmm != 0UL) {
		ERROR("Granule delegate failed!\n");
		return TEST_RESULT_FAIL;
	}

	ret = cactus_send_dma_cmd(HYP_ID, SP_ID(1), 0x82000000);

	retmm = realm_granule_undelegate((u_register_t)0x82000000);
	if (retmm != 0UL) {
