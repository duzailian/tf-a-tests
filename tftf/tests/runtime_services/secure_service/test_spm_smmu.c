...
test_result_t test_smmu_spm(void)
{
	 ...
	 */
	ret = cactus_send_dma_cmd(HYP_ID, SP_ID(1), TEST_DMA_ENGINE_RAND48,
	 ...
	 */
	ret = cactus_send_dma_cmd(HYP_ID, SP_ID(1), TEST_DMA_ENGINE_MEMCPY,
				  PLAT_CACTUS_MEMCPY_BASE,
...
test_result_t test_smmu_spm_invalid_access(void)
{
	...
	retmm = host_rmi_granule_delegate(
		(u_register_t)PLAT_CACTUS_NS_MEMCPY_BASE);
	if (retmm != 0UL) {
	...
	retmm = host_rmi_granule_undelegate(
		(u_register_t)PLAT_CACTUS_NS_MEMCPY_BASE);
	if (retmm != 0UL) {
