...
test_result_t test_mem_share_to_sp_clear_memory(void)
{
	...
	ret = memory_send(mb.send, FFA_MEM_LEND_SMC32, constituents,
			  ...
			  fragment_length, total_length, NULL);
	handle = ffa_mem_success_handle(ret);
						      ...
						      bool fragmented)
{
	...
	static struct ffa_memory_region_constituent
		fragments_out[FRAGMENTED_SHARE_PAGE_COUNT];
...
#if false
	/* Iterate up to `sent_constituents_count - 1` because the first
	 * constituent is not sent as a fragment */
	for (uint32_t i = 0; i < sent_constituents_count - 1; i++) {
		if (fragments_out[i].page_count != 1) {
			ERROR("page_count of page %d is %d, expected 1\n", i,
			      fragments_out[i].page_count);
			return TEST_RESULT_FAIL;
		}
	}
#endif
