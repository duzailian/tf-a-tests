 ...
 * SPMC.
 */
static bool verify_retrieve_response(struct ffa_memory_region *region1,
				     struct ffa_memory_region *region2)
{
...
static test_result_t hypervisor_retrieve_request_test_helper(uint32_t mem_func)
{
	...
	if (!verify_retrieve_response(
		    ...
		    &((struct ffa_memory_region){
			    ...
			    .receiver_count = 1,
			    .receivers_offset = offsetof(
				    struct ffa_memory_region, receivers),
		    })))
	{
		return TEST_RESULT_FAIL;
	}
