...
test_result_t test_hypervisor_memory_retrieve(void) {
         ...
         */
        memcpy(&received_region, mb.send, sizeof(received_region));
        INFO("recieved_region:\n");
        print_region(&received_region);

        assert(received_region.sender == sent_region.sender);
        assert(received_region.flags == sent_region.flags);
        ...
        assert(received_region.receiver_count == 0);
        ...
        for (uint32_t i = 0; i < received_region.receiver_count; i++) {
                ...
                assert(sent_access.receiver_permissions.receiver ==
                       received_access.receiver_permissions.receiver);
                assert(sent_access.receiver_permissions.permissions ==
                       received_access.receiver_permissions.permissions);
                assert(sent_access.receiver_permissions.flags ==
                       received_access.receiver_permissions.flags);
                     ...
                     j < received_composite_region->constituent_count; j++) {
                        struct ffa_memory_region_constituent
                            received_constituent =
                                received_composite_region->constituents[j];
