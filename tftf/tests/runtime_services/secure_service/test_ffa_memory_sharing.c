			   ...
			   struct ffa_memory_region *sent_region)
{
	...
	// EXPECT_EQ(received_region->receiver_count,
	// sent_region->receiver_count);
					...
					bool multiple_receivers)
{
	...
	{
		struct ffa_memory_region_attributes received_permissions = {
			.permissions = permissions,
			.receiver = RECEIVER,
			.flags = 0,
		};
		struct ffa_memory_access sent_access = sent_receivers[0];

		TRY(compare_ffa_memory_region_attributes(
			&received_permissions, &sent_access.receiver_permissions));

		if (sent_access.composite_memory_region_offset == 0) {
