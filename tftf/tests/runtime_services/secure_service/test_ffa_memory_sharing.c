					 ...
					 size_t constituents_count)
{
	...
	unsigned int rme_supported = validate_rme
					? get_armv9_2_feat_rme_support()
					: 0U;
#else
	 ...
	 */
	if (rme_supported != 0U && mem_func != FFA_MEM_SHARE_SMC32) {
		*ptr = 0xBEEF;
	}

	if (mem_func != FFA_MEM_DONATE_SMC32) {

		/* Reclaim memory interily before checking its state. */
