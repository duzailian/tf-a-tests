					   ...
					   ffa_id_t borrower)
{
	...
	struct ffa_memory_access receiver =
		init_receiver_from_mem_func(borrower, mem_func);
...
test_result_t test_mem_share_to_sp_clear_memory(void)
{
	...
	struct ffa_memory_access receiver =
		init_receiver_from_mem_func(RECEIVER, FFA_MEM_LEND_SMC32);
...
static bool verify_composite(struct ffa_composite_memory_region *composite,
	...
	if (composite->reserved_0 != 0) {
		ERROR("composite.reserved_0=%d, expected 0\n",
 ...
 */
static test_result_t hypervisor_retrieve_request_test_helper( uint32_t mem_func, bool multiple_receivers, bool fragmented) {
	...
	struct ffa_memory_access receivers[2] = {
		init_receiver_from_mem_func(SP_ID(1), mem_func),
		init_receiver_from_mem_func(SP_ID(2), mem_func),
	};
	...
	/*
	 * `rx_release()` is no longer necessary because `hypervisor_retrieve()`
	 * does it for us.
	 */
