...
void print_region(struct ffa_memory_region *region) {
        INFO("\n{\n"
             ...
             region->tag, region->memory_access_desc_size,
...
void print_composite_region(struct ffa_composite_memory_region *region) {
        INFO("\n{\n"
             ...
             "\treserved_0 = %llu,\n"
             "}\n",
             region->page_count, region->constituent_count, region->reserved_0);
}
...
test_result_t test_hypervisor_memory_retrieve(void) {
        ...
        struct ffa_memory_region recieved_region;
        ...
        /* 2. Send Hypervisor Retrieve request according to section 16.4.3 of
        ...
        /* 2.5. Verify the recieved `FFA_MEM_RETRIEVE_RESP` aligns with
         * transaction description sent above
         */
        memcpy(&recieved_region, mb.send, sizeof(recieved_region));
        INFO("recieved_region:\n");
        print_region(&recieved_region);

        assert(recieved_region.sender == sent_region.sender);
        assert(recieved_region.flags == sent_region.flags);
        assert(recieved_region.handle == sent_region.handle);
        assert(recieved_region.tag == sent_region.tag);
        assert(recieved_region.memory_access_desc_size ==
               sent_region.memory_access_desc_size);
        assert(recieved_region.receiver_count == 0);
        assert(recieved_region.receivers_offset ==
               sent_region.receivers_offset);
        for (uint32_t i = 0; i < recieved_region.receiver_count; i++) {
                struct ffa_memory_access recieved_access =
                    recieved_region.receivers[i];
                INFO("recieved_access:\n");
                print_memory_access(&recieved_access);

                struct ffa_memory_access sent_access = sent_region.receivers[i];
                INFO("sent_access:\n");
                print_memory_access(&sent_access);

                assert(sent_access.receiver_permissions.receiver ==
                       recieved_access.receiver_permissions.receiver);
                assert(sent_access.receiver_permissions.permissions ==
                       recieved_access.receiver_permissions.permissions);
                assert(sent_access.receiver_permissions.flags ==
                       recieved_access.receiver_permissions.flags);

                struct ffa_composite_memory_region *recieved_composite_region =
                    ffa_memory_region_get_composite(&recieved_region, i);
                INFO("recieved_composite_region:\n");
                print_composite_region(recieved_composite_region);

                struct ffa_composite_memory_region *sent_composite_region =
                    ffa_memory_region_get_composite(&sent_region, i);
                INFO("sent_composite_region:\n");
                print_composite_region(sent_composite_region);

                assert(recieved_composite_region->page_count ==
                       sent_composite_region->page_count);
                assert(recieved_composite_region->constituent_count ==
                       sent_composite_region->constituent_count);

                for (uint32_t j = 0;
                     j < recieved_composite_region->constituent_count; j++) {
                        struct ffa_memory_region_constituent
                            recieved_constituent =
                                recieved_composite_region->constituents[j];
                        struct ffa_memory_region_constituent sent_constituent =
                            sent_composite_region->constituents[j];

                        assert(sent_constituent.address ==
                               recieved_constituent.address);
                        assert(sent_constituent.page_count ==
                               recieved_constituent.page_count);
                        assert(sent_constituent.reserved ==
                               recieved_constituent.reserved);
                }
        }

        /* 3. Reclaim for the SPMC to deallocate any data related to the handle.
         */
        ret = ffa_mem_reclaim(handle, 0);
        if (is_ffa_call_error(ret)) {
