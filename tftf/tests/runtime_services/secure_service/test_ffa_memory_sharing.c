 ...
 * cactus SP, at which point will reclaim the access to memory region and
 * validate contents in memory region have been used by receiver SP.
 *
 * Accessing memory before memory reclaim operation should only be possible
 * on the context of a memory share operation.
 ...
 * Donate the access is relinquished permanently.
 ...
 * policy for a donate and lend operations within SPMC.
