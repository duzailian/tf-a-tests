							     ...
							     size_t receivers_count)
{
	...
	handle = memory_init_and_send(mb->send, MAILBOX_SIZE, SENDER, receivers,
				      ...
				      constituents_count, FFA_MEM_SHARE_SMC32, &ret);
	return handle;
}
...
test_result_t test_ffa_memory_retrieve_request_from_vm(void)
{
	...
	struct ffa_memory_access receivers[2] = {
		ffa_memory_access_init_permissions_from_mem_func(1,
								 FFA_MEM_SHARE_SMC32),
		ffa_memory_access_init_permissions_from_mem_func(SP_ID(2),
								 FFA_MEM_SHARE_SMC32),
	};
...
/**
 * Test that an hypervisor retrieve request would fail if the TX buffer
 * was in realm PAS.
 * The hypervisor retrieve request normally happens during an FFA_MEM_RECLAIM.
 * This validates that the SPMC is able to recoevr from a GPF from accessing the
 * RX buffer when reading the hypervisor retrieve request message.
 */
...
/**
 * Test that an hypervisor retrieve request would fail if the RX buffer
 * was in realm PAS.
 * The hypervisor retrieve request normally happens during an FFA_MEM_RECLAIM.
 * This validates the SPMC is able to recover from a GPF from accessing the TX
