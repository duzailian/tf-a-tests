...
test_result_t test_hypervisor_memory_retrieve(void)
{
	struct ffa_memory_region_constituent sent_constituents[] = {
            {(void *)share_page, 1, 0}};
        const uint32_t sent_constituents_count = sizeof(sent_constituents) /
                                                 sizeof(struct ffa_memory_region_constituent);
	struct mailbox_buffers mb;
	ffa_memory_handle_t handle;
	struct ffa_value ret;
	...
	handle = memory_init_and_send(
            mb.send, MAILBOX_SIZE, hypervisor, sp, sent_constituents,
            sent_constituents_count, FFA_MEM_SHARE_SMC32, &ret);
	...
	assert(received_region->flags == FFA_MEMORY_REGION_TRANSACTION_TYPE_SHARE);
	...
	for (uint32_t i = 0; i < received_region->receiver_count; i++) {
		...
		assert(sent_access.receiver_permissions.permissions == FFA_DATA_ACCESS_RW);
		...
		assert(received_composite_region->constituent_count ==
			sent_composite_region->constituent_count);
                ...
                for (uint32_t j = 0; j < received_composite_region->constituent_count; j++) {
