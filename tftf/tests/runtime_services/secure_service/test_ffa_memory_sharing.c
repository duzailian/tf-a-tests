					   ...
					   ffa_id_t borrower)
{
	...
	ret = memory_init_and_send(mb.send, MAILBOX_SIZE, SENDER, &receiver, 1,
	...
	uint32_t mem_func, bool multiple_receivers, bool fragmented)
{
	...
	/* Share */
	ret = memory_init_and_send(mb.send, MAILBOX_SIZE, SENDER, receivers,
	     ...
	     i++)
	{
		...
		for (uint32_t j = 0; j < composite->constituent_count; j++) {
			...
			if (!verify_constituent(&constituent, j > 0 ? 0 : 1)) {
				return TEST_RESULT_FAIL;
			}
		}
	}
