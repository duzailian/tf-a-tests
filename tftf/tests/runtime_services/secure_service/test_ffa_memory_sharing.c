...
static test_result_t test_memory_send_sp(uint32_t mem_func)
{
	...
	/**********************************************************************
	 * Verify that cactus SP is deployed in the system. If not, skip test.
	 **********************************************************************/
	SKIP_TEST_IF_FFA_ENDPOINT_NOT_DEPLOYED(mb, CACTUS_UUID);
	...
	VERBOSE("TFTF - Handle: %llx\nTFTF - Address: %llx\n",
					handle, (uint64_t)constituents[0].address);
