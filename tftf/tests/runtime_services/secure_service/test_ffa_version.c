...
test_result_t test_ffa_version(uint32_t input_version, uint32_t expected_return)
{
    ...
    return success ? TEST_RESULT_SUCCESS : TEST_RESULT_FAIL;;
}
...
{
    return test_ffa_version(FFA_VERSION_BIT31_MASK | SPM_VERSION, FFA_ERROR_INVALID_PARAMETER);
}
...
{
    return test_ffa_version(MAKE_FFA_VERSION(2,0), SPM_VERSION);
}
