...
static __aligned(PAGE_SIZE) uint8_t vm_rx_buffer[PAGE_SIZE];
static __aligned(PAGE_SIZE) uint8_t vm_tx_buffer[PAGE_SIZE];

static __aligned(PAGE_SIZE) uint8_t vm_rx_buffer2[PAGE_SIZE];
static __aligned(PAGE_SIZE) uint8_t vm_tx_buffer2[PAGE_SIZE];
 ...
 * memory outside the regions specified in the SPMC manifest nodes.
 */
...
test_result_t test_ffa_rxtx_unmap_nonexistent_vm_id_fail(void)
{
	...
	ret = ffa_rxtx_map((uintptr_t)vm_tx_buffer, (uintptr_t)vm_rx_buffer, 1);
	if (is_ffa_call_error(ret)) {
		return TEST_RESULT_FAIL;
	}
...
test_result_t test_ffa_rxtx_map_forward_success(void)
{
	...
	const ffa_id_t endpoint_id = VM_ID(1);
	void *rx_address = vm_rx_buffer;
	void *tx_address = vm_tx_buffer;
	...
	ffa_endpoint_rx_tx_descriptor_init(mb.send, endpoint_id, rx_address,
					   tx_address);
...
test_result_t test_ffa_rxtx_map_forward_consecutive_success(void)
{
	...
	const ffa_id_t endpoint_id = VM_ID(2);
	void *rx_address = vm_rx_buffer2;
	void *tx_address = vm_tx_buffer2;
...
/**
 * Test to verify that RXTX_MAP forwarding fails if trying to forward buffers
