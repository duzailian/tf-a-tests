...
static __aligned(PAGE_SIZE) uint8_t rx_buffer[PAGE_SIZE];
static __aligned(PAGE_SIZE) uint8_t tx_buffer[PAGE_SIZE];
...
test_result_t test_ffa_rxtx_map_forward_success(void)
{
	...
	void *rx_address = rx_buffer;
	void *tx_address = tx_buffer;
...
/**
 * Test to verify that RXTX_MAP forwarding fails if RXTX buffers are not mapped.
 */
