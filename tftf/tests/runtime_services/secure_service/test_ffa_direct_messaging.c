 ...
 * otherwise.
 * For the CACTUS_SUCCESS response, the test return TEST_RESULT_SUCCESS.
 */
static test_result_t send_cactus_req_echo_cmd(ffa_vm_id_t dest,
					      ffa_vm_id_t echo_dest,
					      uint64_t value)
{
	smc_ret_values ret;

	ret = CACTUS_REQ_ECHO_SEND_CMD(HYP_ID, dest, echo_dest, value);
