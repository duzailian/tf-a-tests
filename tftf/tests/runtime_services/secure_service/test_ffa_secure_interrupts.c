 ...
 *
 * 2. Send a comand to SP to sleep by executing a busy loop.
 *
 * 3. While SP is running the busy loop, Secure interrupt should trigger during
 *    this time.
 *
 * 4. The interrupt will be trapped to SPM as IRQ. SPM will delegate the
 *    interrupt to first SP through vIRQ conduit and perform eret to resume
 *    execution in SP.
 *
 * 5. Execution traps to irq hadler of Cactus SP. It will handle the secure
 ...
 *    the requested value.
 *
 * 9. For robustness of state transition checks, we send an echo command using
 *    a direct request message.
 *
 * 10. Test finishes successfully once the SP returns with a success value
 *    through a direct response message.
 *
 */
test_result_t test_ffa_sec_interrupt_sp_running(void)
{
	...
	ret_values = cactus_interrupt_cmd(SENDER, RECEIVER, IRQ_TWDOG_INTID,
	...
	if (cactus_get_response(ret_values) != CACTUS_SUCCESS) {
		ERROR("Failed to enable Trusted Watchdog interrupt\n");
		return TEST_RESULT_FAIL;
	}
 ...
 *
 * 5. The interrupt is trapped to BL31(TF-A) as FIQ and later synchronously
 *    delivered to SPM.
 *
 * 6. SPM delegates the interrupt handling to first Cactus Secure Partition.
 *
 ...
 *    the requested value.
 *
 * 10. For robustness of state transition checks, we send an echo command using
 *    a direct request message.
 *
 * 11. Test finishes successfully once the SP returns with a success value
 *    through a direct response message.
 *
 */
test_result_t test_ffa_sec_interrupt_sp_waiting(void)
{
	...
	if(time_lapsed < NS_TIME_SLEEP) {
		ERROR("Time elapsed less than expected value: %llu\n", time_lapsed);
	}
 ...
 *    interrupt triggered by the trusted watchdog timer.
 *
 * 6. Cactus SP will perform End-Of-Interrupt and invoke FFA_RUN to resume
 *    second SP in the busy loop.
 *
...
test_result_t test_ffa_sec_interrupt_sp_blocked(void)
{
	...
	VERBOSE("Secure interrupt has preempted SP in BLOCKED state\n");
 ...
 *    execution in first SP.
 *
 * 5. Execution traps to irq hadler of the first Cactus SP. It will handle the
 *    secure interrupt triggered by the trusted watchdog timer.
 *
 * 6. Cactus SP will perform End-Of-Interrupt and invoke FFA_MSG_WAIT ABI to
 *    perform interrupt signal completion.
 *
...
test_result_t test_ffa_sec_interrupt_sp1_waiting_sp2_running(void)
{
	 ...
	 */
	if (!is_ffa_direct_response(ret_values)) {
		ERROR("Unexpected ff-a return status\n");
		return TEST_RESULT_FAIL;
	}
