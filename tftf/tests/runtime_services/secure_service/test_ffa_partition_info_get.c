			       ...
			       const uint16_t expected_size)
{
	...
	if (ret.ret0 == FFA_SUCCESS_SMC32) {
		if (ret.ret2 != expected_size) {
			tftf_testcase_printf(
				"Unexpected number of partitions %ld\n",
				ret.ret2);
			return false;
		}
		...
		for (; i < expected_size; i++) {
			...
			if (info[i].exec_context != expected[i].exec_context) {
				tftf_testcase_printf(
					"Wrong context. Expected %d, got %d\n",
			...
			if (info[i].properties != expected[i].properties) {
				tftf_testcase_printf(
					"Wrong properties. Expected %d, got %d\n",
	...
	ret = ffa_rx_release();
	if (ret.ret0 != FFA_SUCCESS_SMC32) {
		tftf_testcase_printf("Failed to release RX buffer\n");
		result = false;
	}
