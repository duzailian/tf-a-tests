...
/*
 * @Test_Aim@ This test exercises the following scenario: Managed exit is
 * supported by first SP but not by the second SP in a call chain. A non secure
 * interrupt triggers while second SP is processing a direct request message
 * sent by the first SP. We choose SP(1) as first SP and SP(2) as second SP.
 *
 * 1. Enable managed exit interrupt by sending interrupt_enable command to
 ...
 *    its interrupt handler for managed exit.
 *
 * 8. TFTF checks the return value in the direct message response from first SP
 *    and ensures it is managed signal interrupt ID.
 *
 * 9. Check whether the pending non-secure timer interrupt successfully got
 *    handled in the normal world by TFTF.
 *
 * 10. Send a dummy direct message request command to resume first SP's execution.
 *
 * 11. First SP direct message request returns with FFA_INTERRUPT status. It
 *     then resumes second SP's execution using FFA_RUN ABI.
 *
