 ...
 *    handled in TFTF.
 *
 * 8. Send a new sleep command to Cactus SP. An error response must be sent
 *    back by the Cactus SP with CACTUS_ERROR_TEST as the error code.
 *
 * 9. Send a command asking the SP to resume after managed exit. SP resumes in
...
test_result_t test_ffa_ns_interrupt_signaled(void)
{
	 ...
	 */
	ret_values = cactus_echo_send_cmd(SENDER, RECEIVER_2, ECHO_VAL);
