 ...
 *    should be pre-empted by non-secure interrupt.
 *
...
test_result_t test_ffa_ns_interrupt_signaled(void)
{
	...
	/* Send request to secondary Cactus to sleep for 200ms */
 ...
 * SP. We choose SP(1) asthe first SP and SP(3) as the second SP.
 *
 ...
 *
 * 7. SPMC proactively injects managed exit signal tothe first SP through vFIQ
 ...
 *
 * 14. The first SP checks if time lapsed is greater than sleep time and if
 ...
 *     response tothe first SP.
 *
 * 13. The first SP checks if time lapsed is greater than sleep time and if
 *     successful, sends direct message response to the TFTF.
 *
 ...
 *    resume execution in the second SP.
 *
 * 6. The second SP sends the managed exit direct response to The first SP through its
