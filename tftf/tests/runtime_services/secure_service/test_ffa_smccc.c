...
#define EXPECTED_PARTITIONS_COUNT	(4UL)
...
test_result_t test_smccc_callee_preserved(void)
{
	...
	/* Omit partition info get if SPMC is S-EL1 OP-TEE. */
	...
	expect_eq(args.fid, FFA_SUCCESS_SMC32);
	expect_eq(args.arg1, 0);
	expect_eq(args.arg2, EXPECTED_PARTITIONS_COUNT);
...
test_result_t test_smccc_ext_callee_preserved(void)
{
	...
	/* Skip test if SPMC is S-EL1 OP-TEE. */
	if (check_spmc_execution_level()) {
	...
	expect_eq(args_ext.arg2 & 0xffff, EXPECTED_PARTITIONS_COUNT - 1);
