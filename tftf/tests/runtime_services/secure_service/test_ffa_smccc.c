...
bool test_ffa_smc(struct ffa_value *);
bool test_ffa_smc_ext(struct ffa_value *);
...
test_result_t test_smccc_callee_preserved(void)
{
	...
	args.arg1 = 0x10001;
	expect_eq(test_ffa_smc(&args), true);
...
test_result_t test_smccc_ext_callee_preserved(void)
{
	...
	memset(&args, 0, sizeof(struct ffa_value));
	args.fid  = 0xC400008B; /* FFA_PARTITION_INFO_GET_REGS */
	expect_eq(test_ffa_smc_ext(&args), true);
	expect_eq(args.fid, FFA_SUCCESS_SMC64);
	expect_eq(args.arg1, 0);
	expect_eq(args.arg2 >> 48, sizeof(struct ffa_partition_info));
	expect_eq(args.arg2 & 0xffff, ARRAY_SIZE(ffa_expected_partition_info) - 1);
