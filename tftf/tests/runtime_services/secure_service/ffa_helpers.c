...
/**
 * Copy the memory region at `src` to `dst`, including the trailing
 * `ffa_memory_access`s
 */
void ffa_memory_region_copy(struct ffa_memory_region *dst,
			    struct ffa_memory_region *src)
{
	memcpy(dst, src,
	       sizeof(struct ffa_memory_region) +
		       src->receiver_count * sizeof(struct ffa_memory_access));
}
...
{
	/* all fields except `handle` must be zero to indicate a hypervisor
	 * retrieve */
				       ...
				       enum ffa_instruction_access instruction_access)
{
	...
	access.receiver_permissions.flags = 0;
	access.receiver_permissions.receiver = receiver;
						     ...
						     uint32_t mem_func)
{
	...
	return init_receiver(receiver, data_access, instruction_access);
}
