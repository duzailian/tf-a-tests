...
test_result_t test_simd_vectors_preserved(void)
{
	...
	fpu_temp = fpu_state_write_template(SIMD_NS_VALUE);
	...
	return fpu_state_compare_template(&fpu_temp) ? TEST_RESULT_SUCCESS :
		TEST_RESULT_FAIL;
}
...
test_result_t test_sve_vectors_preserved(void)
{
	...
	sve = sve_state_write_template(SVE_Z_NS_VALUE,
			SVE_P_NS_VALUE,
			vl,
			vl,
			SVE_FFR_NS_VALUE);
	...
	return sve_state_compare_template(&sve) ? TEST_RESULT_SUCCESS : TEST_RESULT_FAIL;
#else
