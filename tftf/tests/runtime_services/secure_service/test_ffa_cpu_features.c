...
/*
 * Tests that SIMD vectors are preserved during the context switches between the
...
test_result_t test_ffa_simd_preserved(void)
{
	...
	ffa_fill_simd_vector_regs(simd_vectors_send);
	...
	ffa_dump_simd_vector_regs(simd_vectors_receive);
