...
static test_result_t simd_vector_compare(simd_vector_t *a, simd_vector_t *b, unsigned int length)
{
	for (unsigned int num = 0U; num < length; num++) {
		if (memcmp(a[num], b[num], sizeof(simd_vector_t)) != 0U) {
...
/*
 * Tests that SIMD vectors are preserved during the context switches between SW
 * and nSw.
...
test_result_t test_ffa_simd_preserved(void)
{
	...
	return simd_vector_compare(simd_vectors, simd_vectors_compare,
				 SIMD_NUM_VECTORS);
}
