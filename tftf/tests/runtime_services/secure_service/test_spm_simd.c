...
static test_result_t test_sve_vectors_preserved_impl(uint64_t vq)
{
	uint8_t *sve_vector;
	uint64_t vl;
...
test_result_t test_sve_vectors_preserved(void)
{
	...
	for (bitmap = vl_bitmap; bitmap != 0U; bitmap >>= 1) {
		if ((bitmap & 1) != 0) {
			...
			ret = test_sve_vectors_preserved_impl(vq);
 ...
 * value.
 */
static test_result_t __test_sme(test_result_t (*func)(uint8_t svq))
{
	uint32_t svl_bitmap;
	uint8_t svq;
	test_result_t ret;
 ...
 */
static test_result_t do_test_sme_za(uint8_t svq)
{
	struct ffa_value ret;
