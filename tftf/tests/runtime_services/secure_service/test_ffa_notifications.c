...
static bool request_notification_get(
	ffa_vm_id_t cmd_dest, ffa_vm_id_t receiver, uint32_t vcpu_id,
	...
	bool more_pending)
{
	...
	for (unsigned int i = 0; i < lists_count; i++) {
		uint32_t cur_size =
				ffa_notifications_info_get_list_size(*ret, i);
	...
	if (memcmp(&ret->ret3, ids, sizeof(ret->ret3) * 5) != 0) {
 ...
 */
static bool request_notification_bind_and_set(ffa_vm_id_t sender,
	ffa_vm_id_t receiver, uint64_t notifications)
{
	smc_ret_values ret;
	...
	ret = ffa_notification_set(sender, receiver, 0, notifications);
 ...
 */
static bool request_notification_get_and_validate(ffa_vm_id_t sender,
	ffa_vm_id_t receiver, uint64_t notification, uint32_t flags)
{
	smc_ret_values ret;

	return request_notification_get(receiver, receiver, 0, flags, &ret) &&
	       is_notifications_get_as_expected(&ret, 0, notification, receiver);
}
...
{
	CHECK_SPMC_TESTING_SETUP(1, 0, expected_sp_uuids);
	const ffa_vm_id_t sender = 1;
	const ffa_vm_id_t receiver = SP_ID(1);
	uint64_t notification = FFA_NOTIFICATION(1) | FFA_NOTIFICATION(60);
	const uint32_t flags = FFA_NOTIFICATIONS_FLAG_BITMAP_VM;
	...
	if (!request_notification_bind_and_set(sender,
					       receiver,
					       notification)) {
		return TEST_RESULT_FAIL;
	}
	...
	if(!request_notification_get_and_validate(sender, receiver, notification,
						 flags)) {
		result = TEST_RESULT_FAIL;
	}
...
test_result_t test_ffa_notifications_sp_signals_sp(void)
{
	...
	const ffa_vm_id_t receiver = SP_ID(2);
