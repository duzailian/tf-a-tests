					...
					ffa_vcpu_count_t vcpu_count)
{
	VERBOSE("Creating bitmap for VM %x; cpu count: %u.\n",
		vm_id, vcpu_count);
	ffa_value ret = ffa_notification_bitmap_create(vm_id, vcpu_count);
...
static bool notifications_bitmap_destroy(ffa_id_t vm_id)
{
	...
	ffa_value ret = ffa_notification_bitmap_destroy(vm_id);
...
test_result_t test_ffa_notifications_destroy_not_created(void)
{
	...
	ffa_value ret = ffa_notification_bitmap_destroy(VM_ID(1));
...
static bool request_notification_get(
	...
	bool check_npi_handled, ffa_value *response)
{
...
static bool is_notifications_get_as_expected(
	ffa_value *ret, uint64_t exp_from_sp, uint64_t exp_from_vm,
	ffa_id_t receiver)
{
	uint64_t from_sp;
	uint64_t from_vm;
...
static bool is_notifications_info_get_as_expected(
	ffa_value *ret, uint16_t *ids, uint32_t *lists_sizes,
						     ...
						     bool enable)
{
	...
	ffa_value ret = cactus_interrupt_cmd(
