...
static const struct feature_test test_target[] = {
	...
	{"FFA_RXTX_MAP_32 check", FFA_RXTX_MAP_SMC32, FFA_SUCCESS_SMC32},
...
test_result_t test_ffa_features(void)
{
	...
	uint16_t i, test_target_size = sizeof(test_target) / sizeof(struct feature_test);

	for (i = 0; i < test_target_size; i++) {
		...
		if ((test_target[i].expected_ret == FFA_ERROR) &&
		    (ffa_ret.ret2 != FFA_ERROR_NOT_SUPPORTED)) {
			tftf_testcase_printf("%s failed for the wrong reason: "
					     "returned %lx, expected %x\n",
					     test_target[i].test_name,
					     ffa_ret.ret2,
