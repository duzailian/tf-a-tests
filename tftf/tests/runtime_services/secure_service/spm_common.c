			 ...
			 uint32_t out_size)
{
	 ...
	 * message.
	 */
	ffa_memory_region_copy(region_out, mb->recv);
...
{
	/*
	 * These variables are marked `__unused` because they are only used in
	 * `assert` calls, so the static checker will complain when compiling
	 * with assertions disabled.
	 */
	__unused uint64_t handle;
	__unused uint64_t expected_handle_mask =
