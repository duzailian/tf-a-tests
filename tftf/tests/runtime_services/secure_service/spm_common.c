...
#define fill_sve_helper(num) "ldr z"#num", [%0]; add %0, %0, #"STR(SVE_VECTOR_LEN_BYTES)";"
#define read_sve_helper(num) "str z"#num", [%0]; add %0, %0, #"STR(SVE_VECTOR_LEN_BYTES)";"
#define fill_simd_helper(num1, num2) "ldp q"#num1", q"#num2", [%0], #"STR(2 * SIMD_VECTOR_LEN_BYTES)";"
#define read_simd_helper(num1, num2) "stp q"#num1", q"#num2", [%0], #"STR(2 * SIMD_VECTOR_LEN_BYTES)";"
...
void fill_sve_vector_regs(const sve_vector_t v[SVE_NUM_VECTORS])
{
	...
	__asm__ volatile(
		...
		"sub %0, %0, #" STR(SVE_NUM_VECTORS * SVE_VECTOR_LEN_BYTES) ";"
		: : "r" (v));
#endif
}
