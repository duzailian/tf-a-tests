...
bool hypervisor_retrieve(struct mailbox_buffers *mb, uint64_t handle,
			 struct ffa_memory_access *receivers,
			 uint32_t receiver_count, void *out, uint32_t out_size)
{
	struct ffa_value ret;
	uint32_t total_size;
	uint32_t fragment_size;
	...
	// all fields must be zero to indicate a hypervisor retrieve
	ffa_memory_retrieve_request_init(
		mb->send, handle, 0, receivers, receiver_count, 0, 0,
		FFA_MEMORY_NOT_SPECIFIED_MEM, FFA_MEMORY_CACHE_RESERVED,
	...
	if (ffa_func_id(ret) != FFA_MEM_RETRIEVE_RESP) {
		ERROR("Couldn't retrieve the memory page. Error: %d\n",
		      ffa_error_code(ret));
		return false;
	}
	...
	if (total_size > out_size) {
		ERROR("out is not large enough to store all "
		      "fragments (total_size=%d, max_size=%d)\n",
		      total_size, out_size);
		return false;
	}

	memcpy(out, mb->recv, sizeof(struct ffa_memory_region));
	memcpy(((struct ffa_memory_region *)out)->receivers,
	       ((struct ffa_memory_region *)mb->recv)->receivers,
	       receiver_count * sizeof(struct ffa_memory_region));

	if (((struct ffa_memory_region *)out)->receiver_count >
	    MAX_MEM_SHARE_RECIPIENTS)
	{
		VERBOSE("SPMC memory sharing operations support max of %u "
			"receivers!\n",
	...
	while (fragment_offset < total_size) {
		...
		if (ret.fid != FFA_MEM_FRAG_TX) {
			ERROR("ffa_mem_frag_rx() returned wrong function id: "
			      "%lu\n",
			      ret.fid);
			return false;
		}

		assert(ffa_frag_handle(ret) == handle);
		/* Sender MBZ at virtual instance. */
		assert(ffa_frag_sender(ret) == 0);
		fragment_size = ret.arg3;
		assert(fragment_size > 0);

		assert(fragment_offset + fragment_size <= out_size);
	...
	assert(fragment_offset == total_size);
...
struct ffa_value send_fragmented_memory_region(
	struct mailbox_buffers *mb,
	struct ffa_memory_region_constituent constituents[],
	uint32_t constituent_count, uint32_t remaining_constituent_count,
	uint32_t sent_length, uint32_t total_length, ffa_memory_handle_t handle,
	uint64_t allocator_mask, struct ffa_value ret)
{
	...
	while (remaining_constituent_count != 0) {
		...
		assert(ret.fid == FFA_MEM_FRAG_RX);
		if (fragment_handle == FFA_MEMORY_HANDLE_INVALID) {
			fragment_handle = ffa_frag_handle(ret);
		} else {
			assert(ffa_frag_handle(ret) == fragment_handle);
		}
		assert(ret.arg3 == sent_length);
	...
	assert(sent_length == total_length);
	assert(ret.fid == FFA_SUCCESS_SMC32);

	// TODO: assertion fails
	// assert((handle & FFA_MEMORY_HANDLE_ALLOCATOR_MASK) ==
	// allocator_mask);

	if (fragment_handle != FFA_MEMORY_HANDLE_INVALID) {
		assert(handle == fragment_handle);
	}
 ...
 */
struct ffa_value memory_send(struct mailbox_buffers *mb, uint32_t mem_func,
			     struct ffa_memory_region_constituent *constituents,
			     ...
			     uint32_t fragment_length, uint32_t total_length)
{
	...
	if (remaining_constituent_count == 0 && fragment_length != total_length)
	{
		...
		return ffa_error(FFA_ERROR_INVALID_PARAMETER);
	}
	...
	ret = send_fragmented_memory_region(
		mb, constituents, constituent_count,
		remaining_constituent_count, fragment_length, total_length,
		handle, FFA_MEMORY_HANDLE_ALLOCATOR_HYPERVISOR, ret);
 ...
 */
struct ffa_value memory_init_and_send(struct mailbox_buffers *mb,
		...
		uint32_t constituents_count, uint32_t mem_func)
{
	...
	remaining_constituent_count = ffa_memory_region_init(
		mb->send, memory_region_max_size, sender, receivers,
		receiver_count, constituents, constituents_count, 0, 0, type,
		FFA_MEMORY_CACHE_WRITE_BACK, FFA_MEMORY_INNER_SHAREABLE,
		&total_length, &fragment_length);

	return memory_send(mb, mem_func, constituents, constituents_count,
			   remaining_constituent_count, fragment_length,
			   total_length);
}
