	...
	struct ffa_partition_info *partition_info)
{
	...
	partition_info->id = info & 0xFFFF;
	partition_info->exec_context = (info >> 16) & 0xFFFF;
...
static bool ffa_compare_partition_info(const struct ffa_uuid uuid,
 ...
 * returned in registers in the output parameters. Comparison against
 * expected results is expected to be done by the caller outside the function.
		       ...
		       const uint16_t expected_size)
{
	 ...
	 */
	if (expected_size > 5) {
		ERROR("info_regs_helper only supports information received in"
