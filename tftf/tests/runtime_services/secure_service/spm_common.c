			 ...
			 uint32_t receiver_count)
{
	...
	memcpy(retrieved, mb->recv, sizeof(struct ffa_memory_region));
	...
	while (fragment_offset < total_size) {
		VERBOSE("Calling again. frag offset: %x; total: %x\n",
		...
		// TODO: fails
		assert(fragment_offset + fragment_size <= PAGE_SIZE);

		memcpy((uint8_t *)retrieved + fragment_offset, mb->recv,
		       fragment_size);
