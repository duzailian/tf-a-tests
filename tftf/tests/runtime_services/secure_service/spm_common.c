 ...
 */
bool is_ffa_call_error(ffa_value ret)
{
...
bool is_expected_ffa_error(ffa_value ret, int32_t error_code)
{
	if (ffa_func_id(ret) == FFA_ERROR &&
 ...
 * to an SP.
 */
bool is_ffa_direct_response(ffa_value ret)
{
 ...
 */
bool is_expected_ffa_return(ffa_value ret, uint32_t func_id)
{
...
bool is_expected_cactus_response(ffa_value ret, uint32_t expected_resp,
				 uint32_t arg)
{
...
void dump_ffa_value(ffa_value ret)
{
...
ffa_memory_handle_t memory_send(
	...
	uint32_t fragment_length, uint32_t total_length, ffa_value *ret)
{
...
ffa_memory_handle_t memory_init_and_send(
	...
	uint32_t constituents_count, uint32_t mem_func, ffa_value *ret)
{
...
{
	bool result = true;
	ffa_value ret = ffa_partition_info_get(uuid);
