...
test_result_t test_spci_handle_open(void)
{
	...
	if (ret != SPCI_NOT_PRESENT) {
		tftf_testcase_printf("%d: SPM should have returned SPCI_NOT_PRESENT. Returned: 0x%x\n",
	...
	if (ret != SPCI_INVALID_PARAMETER) {
		tftf_testcase_printf("%d: SPM didn't fail to close the handle. Returned: 0x%x\n",
				     __LINE__, (uint32_t)ret);
		return TEST_RESULT_FAIL;
	}
