...
test_result_t test_ffa_indirect_message_sp_to_vm(void)
{
	...
	if (header_sender != sender) {
		ERROR("Unexpected endpoints. Sender: %x Exp: %x\n", header_sender,
		      sender);
		return TEST_RESULT_FAIL;
	}
...
test_result_t test_ffa_indirect_message_sp_to_vm_rx_realm_fail(void)
{
	...
	/*
	 * Create bitmap only to then demonstrate that the operation fail, as
	 * there are no pending notifications to the VM.
	 */
	ret = ffa_notification_bitmap_create(vm_id, PLATFORM_CORE_COUNT);
...
/**
 * Test message sent from VM to SP, when its TX is realm, the operation fails
 * smoothly.
 */
test_result_t test_ffa_indirect_message_vm_to_sp_tx_realm_fail(void)
{
	...
	if (!is_expected_ffa_return(ret, FFA_SUCCESS_SMC32)) {
		ERROR("Failed to map buffers RX %llx TX %llx for VM %x\n",
		      (uint64_t)vm1_tx_buffer, (uint64_t)vm1_tx_buffer, vm_id);
		return TEST_RESULT_FAIL;
	}
