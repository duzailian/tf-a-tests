...
{
	/*
	 * Setting the P bit tells the counter NOT to increment at EL1.
	 * Setting the NSK bit to be different from the P bit further tells the
	 * counter NOT to increment at non-secure EL1.
	 * Disabling the NSH bit tells the counter NOT to increment at EL2.
	 * We want to profile just the secure world for maximum accuracy.
	 * Setting the M bit to be equal to P bit tells the counter to increment
	 * at EL3.
	 */
	write_pmevtyper0_el0(
		(read_pmevtyper0_el0() | PMEVTYPER_EL0_P_BIT |
		 PMEVTYPER_EL0_M_BIT) &
		~(PMEVTYPER_EL0_NSK_BIT | PMEVTYPER_EL0_NSH_BIT));
...
static inline void configure_pmu_cycle_cntr(void)
{
	 ...
	 */
	write_pmccfiltr_el0(
		(read_pmccfiltr_el0() | PMCCFILTR_EL0_P_BIT |
		 PMCCFILTR_EL0_M_BIT) &
		~(PMCCFILTR_EL0_NSK_BIT & PMCCFILTR_EL0_NSH_BIT));
