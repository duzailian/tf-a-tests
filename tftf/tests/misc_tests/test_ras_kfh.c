 ...
 *
 * Register a custom serror handler in tftf, inject a RAS error and wait until
 * custom serror handler is called and updates the serror_triggered flag.
 */
test_result_t test_ras_kfh(void)
{
	...
	/* Wait for some reasonable time for SError to be triggered */
	waitms(500);
