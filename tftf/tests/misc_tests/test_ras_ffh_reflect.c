...
/*
 * Test to verify reflection of lower EL SErrors in RAS FFH mode.
 *
 * This test exercises the path of EL3 reflection of SError back to lower
 * EL, which gets triggered as part of error synchronization during EL3
 * entry. This test works in conjunction with "ras_ffh_reflection.patch"
 ...
 *  7. Make SMC call to get ther version
 *  8. On entering EL3, sync_exception_vector entry, will find that SError is
 *     pending, So it will call "reflect_pending_serror_to_lower_el".
 *  9. TF-A will eret back from EL3(without handling SMC call) and during ERET
 *     change SCR_EL3.EA back to 1 along with unmasking SError for TFTF.
 *     SPSR.PSTATE.A = 0.
 *  10.At TFTF entry it will see an SError pending which will be routed to
 *     EL3 as we are in FFH mode.
 *  11.After handling the SError, EL3 will notify it to tftf using SDEI handler
 *     registered earlier.
 *  12.After retruning back from EL3 the original SMC request will be handled.
 *
 * Checks:
 *  1. Ensure that SDEI notification was not received before SMC call, it
 *     verifies reflection path.
 *  2. Ensure that 2nd SMC call indeed has happened.
 *  3. Ensure that we did recieve SDEI notification
 *
 */
