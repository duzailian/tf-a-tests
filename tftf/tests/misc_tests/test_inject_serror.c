 ...
 *
 * This test maps an non-existent memory as Device memory and access it. Memory
 * is mapped as device memory to go past MMU and cause an error on bus and
 * trap as an SError.
...
test_result_t test_inject_serror(void)
{
	...
	/* Try to to access invalid address */
	rc = mmio_read_32(test_address);

	/* Never comes here, so return rc to avoid compiler optimizing read operation above */
	return rc;
}
