...
struct ras_aux_data {
	/* point to null-terminated ras_error array to convert error code to
	 * msg. */
	const struct ras_error_rec *error_records;
	/* function to return a value which needs to be programmed into
	 * ERXCTLR_EL1 to enable all specified RAS errors for current node.
	 */
