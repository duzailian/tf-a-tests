...
static void test_ras_inject_error(uint32_t errselr_el1,
				  unsigned int errctlr_bit)
{
...
test_result_t test_ras_corrected(void)
{
	...
	for_each_cpu(cpu_node) {
		...
		ret = tftf_cpu_on(cpu_mpid, (uintptr_t)test_corrected_errors,
				  0);
	...
	for_each_cpu(cpu_node) {
		...
		do {
			/*
			 * clean+invalidate cache lines before reading the
			 * global flag populated by another CPU
			 */
