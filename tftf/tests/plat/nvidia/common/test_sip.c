...
#define TEGRA_SIP_GET_SMMU_PER			0xC200FF00
...
{
	uint32_t size_in_bytes = (4 << 20);
	uint32_t offset = (8 << 20);
	...
	mem = vidmem_base;
	for (unsigned int i = 0; i < (size_in_bytes / 8); i++, mem+=8) {
...
/**
 * @Test_Aim@ Force an TEGRA_SIP_GET_SMMU_PER return
 */
test_result_t test_get_smmu_per(void)
{
	smc_args tegra_sip_smc = { TEGRA_SIP_GET_SMMU_PER, 0, 0, 0, 0 };
	smc_ret_values ret;
	...
	if (ret.ret0 != SMC_OK) {
		tftf_testcase_printf("get smmu per test Fail, got %ld\n",
				(long int) ret.ret0);
		return TEST_RESULT_FAIL;
	}

	tftf_testcase_printf("GET_SMMU_PER per[0] = 0x%lx\n", ret.ret1);
	tftf_testcase_printf("GET_SMMU_PER per[1] = 0x%lx\n", ret.ret2);
	tftf_testcase_printf("GET_SMMU_PER per[2] = 0x%lx\n", ret.ret3);
