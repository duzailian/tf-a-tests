...
inline void set_tftf_mailbox(struct mailbox_buffers mb)
{
	test_mb = mb;
}

inline bool get_tftf_mailbox(struct mailbox_buffers *mb)
{
	...
	const struct ffa_uuid *ffa_uuids, size_t ffa_uuids_size)
{
	...
	SKIP_TEST_IF_FFA_VERSION_LESS_THAN(ffa_version_major, ffa_version_minor);

	/**********************************************************************
	 * If OPTEE is SPMC skip the current test.
	 **********************************************************************/
	...
	for (unsigned int i = 0; i < ffa_uuids_size; i++) {
