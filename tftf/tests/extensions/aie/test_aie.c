...
test_result_t test_aie_support(void)
{
	...
	if (EL2_IS_IN_HOST()) {
		write_mair2_el12(TEST_PATTERN_1);
		write_amair2_el12(TEST_PATTERN_1);
	} else {
	...
	if (!check_reg(EL2_IS_IN_HOST() ? read_mair2_el12() : read_mair2_el1(),
		       TEST_PATTERN_1, "MAIR2_EL1")) {
		return TEST_RESULT_FAIL;
	}
	if (!check_reg(EL2_IS_IN_HOST() ? read_amair2_el12() : read_amair2_el1(),
		       TEST_PATTERN_1, "AMAIR2_EL1")) {
		return TEST_RESULT_FAIL;
	}
