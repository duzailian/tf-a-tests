...
/*
 * TF is expected to allow access to key registers from lower EL's,
...
/*
 * Makes a call to psci version, and checks that the pauth keys are not
...
test_result_t test_pauth_leakage(void)
{
	...
	if (memcmp(pauth_keys_before, pauth_keys_after, LENGTH_KEYS * sizeof(uint64_t)) != 0)
...
/*
 * Makes a call to TSP ADD, and checks that the pauth keys are not
 * leaked when it returns
 */
#if __GNUC__ > 7 || (__GNUC__ == 7 && __GNUC_MINOR__ > 0)
__attribute__((target("sign-return-address=all")))
