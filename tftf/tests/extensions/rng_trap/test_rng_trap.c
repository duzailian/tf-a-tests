...
/*
 * This test ensures that a trap instructions causes a trap to EL3 and generates
 ...
 */
static test_result_t test_rng_trap(enum trap_instruction instruct)
{
...
{
	return test_rng_trap(RNG_RNDR_INSTRUCT);
}
...
{
	return test_rng_trap(RNG_RNDRRS_INSTRUCT);
}
