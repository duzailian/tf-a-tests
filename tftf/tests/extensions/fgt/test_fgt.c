...
static bool is_init_val_set(u_register_t reg, u_register_t init_val, u_register_t feat_mask)
{
...
test_result_t test_fgt_enabled(void)
{
	...
	hfgitr_el2 = read_hfgitr_el2();
	hfgrtr_el2 = read_hfgrtr_el2();
	hfgwtr_el2 = read_hfgwtr_el2();
	read_hdfgrtr_el2();
	read_hdfgwtr_el2();
	...
	if (!is_init_val_set(hfgitr_el2, HFGITR_EL2_INIT_VAL,
		HFGITR_EL2_NON_FEAT_DEPENDENT_MASK)) {
		return TEST_RESULT_FAIL;
	}
	...
	if (!is_init_val_set(hfgrtr_el2, HFGRTR_EL2_INIT_VAL,
		HFGRTR_EL2_NON_FEAT_DEPENDENT_MASK)) {
		return TEST_RESULT_FAIL;
	}
	...
	if (!is_init_val_set(hfgwtr_el2, HFGWTR_EL2_INIT_VAL,		\
		HFGWTR_EL2_NON_FEAT_DEPENDENT_MASK)) {
		return TEST_RESULT_FAIL;
	}
