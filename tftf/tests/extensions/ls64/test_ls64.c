...
test_result_t test_ls64_instructions(void)
{
	 ...
	 */
	uint64_t *store_address = (uint64_t *)DEVICE2_BASE;
	 ...
	 *
	 * NOTE: As Device Memory Region (0x1d000000 - 0x1d001111), can be accessed
	 * only via st64b/ld64b instructions, we cannot pre-write into this memory
	 * via other instructions. Henceforth, we first load the data from a normal
	 * input buffer into the consecutive registers and then copy them in one
	 * atomic operation via st64b to Device memory(DEVICE2_BASE).
	 * Further we load the data from the same device memory into a noraml
