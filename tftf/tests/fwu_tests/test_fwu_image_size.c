...
test_result_t test_fwu_image_size(void)
{
	...
	bl2_entry = find_fiptoc_entry_t(fip_base, &bl2);
	//bl2_entry_address = (uintptr_t)bl2_entry + FLASH_BASE;
	bl2_entry_address = (uintptr_t)bl2_entry;

//(uintptr_t)bl2_entry + FLASH_BASE;// so this is

	bl2u_entry = find_fiptoc_entry_t(fwu_fip_base, &bl2u);
	//bl2u_entry_address = (intptr_t)bl2u_entry + FLASH_BASE;
	bl2u_entry_address = (intptr_t)bl2u_entry; //so good so far I like this
	
	if(!tftf_is_rebooted()){

	tftf_testcase_printf("bl2_entry size value is (%llx) \n", bl2_entry->size);
	tftf_testcase_printf("bl2_entry offset value is (%llx) \n", bl2_entry->offset_address);

		/* Reboot has not occured yet */
		...
		*/
		...
		//corrupt bl2 image
		//status = fwu_nvm_write(bl2_entry->offset_address, &corrupt_size, sizeof(uint64_t));
		//status = fwu_nvm_write(bl2_entry_address, &corrupt_size, sizeof(uint64_t));
		int offset = bl2_entry_address + sizeof(uuid_t) + sizeof(uint64_t)- FLASH_BASE;
		...
		//int offset = bl2u_entry_address + sizeof(uuid_t) + sizeof(uint64_t);
		//offset = bl2u_entry_address - FLASH_BASE;
		...
		//read buffer to confirm
		int buffer;
		status = fwu_nvm_read(offset, &buffer, sizeof(uint64_t));
		tftf_testcase_printf("offset value is %x \n", offset);
		tftf_testcase_printf("post nvm write+read %x \n", buffer);

		if(status != STATUS_SUCCESS){
			tftf_testcase_printf("STATUS NOT SUCCESS ERROR %d \n",status);
		}

		//add printstatetment for expcet script is board is not expected to be recovered
		tftf_testcase_printf("NVM READs");
		tftf_testcase_printf("bl2_entry size value is (%llx) \n", bl2_entry->size);
		tftf_testcase_printf("bl2_entry offset value is (%llx) \n", bl2_entry->offset_address);
