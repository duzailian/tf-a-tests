...
void fpu_state_set(fpu_reg_state_t *vec, uint8_t regs_val, uint32_t fpcr_val,
			uint32_t fpsr_val)
{
...
bool fpu_state_cmp(fpu_reg_state_t *current, fpu_reg_state_t *read)
{
	(void)memset((void *)read, 0, sizeof(fpu_reg_state_t));
	read_fpu_state_registers(read);
	if (memcmp((uint8_t *)current,
			(uint8_t *)read,
			sizeof(fpu_reg_state_t)) != 0U) {
		ERROR("fpu_state_cmp failed\n");
		fpu_state_print(read);
		return false;
	} else {
		return true;
	}
