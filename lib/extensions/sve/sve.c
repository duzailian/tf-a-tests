/*
 * Copyright (c) 2022, Arm Limited. All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include <arch_helpers.h>
#include <debug.h>
#include <lib/extensions/sve.h>
#include <lib/extensions/fpu.h>

/*
 * sve_template: used as temporary buffer to store template values
 * sve_read: used as temporary buffer for SVE state read from registers
 */
struct sve_state sve_template, sve_read;

void read_sve_state(struct sve_state *sve)
{
	read_sve_zcr_fpu_state(sve->zcr_fpu);
	read_sve_z_state((unsigned char *)sve->z);
	read_sve_ffr_state(sve->ffr);
	read_sve_p_state(sve->p);
}

void write_sve_state(struct sve_state *sve)
{
	write_sve_zcr_fpu_state(sve->zcr_fpu);
	write_sve_z_state((unsigned char *)sve->z);
	write_sve_p_state(sve->p);
	write_sve_ffr_state(sve->ffr);
}

void sve_state_set(struct sve_state *sve,
			uint8_t z_regs,
			uint8_t p_regs,
			uint32_t zcr_el1,
			uint32_t zcr_el2,
			uint8_t ffr_regs)
{
	uint32_t max_vl_z, index;

	memset(sve, 0x0, sizeof(struct sve_state));
	max_vl_z = IS_IN_EL2() ? zcr_el2 : zcr_el1;
	for (index = 0U; index < SVE_VLA_Z_REGS_MAX; index++) {
		memset((uint8_t *)&sve->z[index*SVE_VLA_VL_Z(max_vl_z)],
		z_regs++, SVE_VLA_VL_Z(max_vl_z));
	}
	for (index = 0U; index < SVE_VLA_P_REGS_MAX; index++) {
		memset((uint8_t *)&sve->p[index*SVE_VLA_VL_P(max_vl_z)],
		p_regs++, SVE_VLA_VL_P(max_vl_z));
	}
	memset((uint8_t *)&sve->ffr, ffr_regs, SVE_VLA_VL_P(max_vl_z));
	read_fpu_state_registers_fpcr_fpsr((char *)sve + offsetof(struct sve_state, fpsr));
	sve->zcr_el1 = zcr_el1;
	sve->zcr_el2 = zcr_el2;
}

struct sve_state sve_state_write_template(uint8_t z_regs,
		uint8_t p_regs,
		uint32_t zcr_el1,
		uint32_t zcr_el2,
		uint8_t ffr_regs)
{
	sve_state_set(&sve_template, z_regs, p_regs, zcr_el1,
			zcr_el2, ffr_regs);
	write_sve_zcr_fpu_state(sve_template.zcr_fpu);
	write_sve_z_state((unsigned char *)sve_template.z);
	write_sve_p_state(sve_template.p);
	write_sve_ffr_state(sve_template.ffr);
	return sve_template;
}

sve_compare_result_t sve_read_compare(const struct sve_state *sve_compare,
		struct sve_state *sve_read)
{
	int cmp = -1;
	uint32_t max_vl_z, i;
	sve_compare_result_t result = SVE_STATE_ERROR;

	/* read SVE state registers values.*/
	memset(sve_read, 0, sizeof(struct sve_state));
	read_sve_state(sve_read);
	/* compare.*/
	if (sve_compare->zcr_el1 != sve_read->zcr_el1) {
		ERROR("\n comparison of SVE/ZCR_EL1 failed\n");
		result = ZCR_ELR1_ERROR;
	}
	if (sve_compare->fpcr != sve_read->fpcr) {
		ERROR("\n comparison of SVE/fpcr failed\n");
		result = FPCR_REG_ERROR;
	}
	if (sve_compare->fpsr != sve_read->fpsr) {
		ERROR("\n comparison of SVE/fpsr failed\n");
		result = FPSR_REG_ERROR;
	}
	if (IS_IN_EL2()) {
		if (sve_compare->zcr_el2 != sve_read->zcr_el2) {
			ERROR("\n comparison of SVE/ZCR_EL2 failed\n");
			result = ZCR_ELR2_ERROR;
		}
	}
	max_vl_z = IS_IN_EL2() ? sve_compare->zcr_el2 : sve_compare->zcr_el1;
	cmp = memcmp(sve_compare->ffr, sve_read->ffr, SVE_VLA_VL_P(max_vl_z));
	if (cmp) {
		ERROR("\n comparison of SVE/FFR failed\n");
		result = FFR_REG_ERROR;
	}
	for (i = 0U; i < SVE_VLA_Z_REGS_MAX; i++) {
		cmp = memcmp((uint8_t *)&sve_compare->z[i*SVE_VLA_VL_Z(max_vl_z)],
				(uint8_t *)&sve_read->z[i*SVE_VLA_VL_Z(max_vl_z)],
				SVE_VLA_VL_Z(max_vl_z));
		if (cmp) {
			result = Z_REG_ERROR;
			ERROR("\n comparison of SVE/Z failed in index:%d\n", i);
		}
	}
	for (i = 0U; i < SVE_VLA_P_REGS_MAX; i++) {
		cmp = memcmp((uint8_t *)&sve_compare->p[i*SVE_VLA_VL_P(max_vl_z)],
				(uint8_t *)&sve_read->p[i*SVE_VLA_VL_P(max_vl_z)],
				SVE_VLA_VL_P(max_vl_z));
		if (cmp) {
			result = P_REG_ERROR;
			ERROR("\n comparison of SVE/P failed in index:%d\n", i);
		}
	}
	result = SVE_STATE_SUCCESS;
	return result;
}

bool sve_state_compare_template(const struct sve_state *sve_template)
{
	return (sve_read_compare(sve_template,
			&sve_read) == SVE_STATE_SUCCESS);
}
