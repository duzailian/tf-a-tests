...
static inline __attribute__((unused)) void xlat_clean_dcache_range(
 ...
 */
static inline unsigned int xlat_tables_va_to_index(
	const uintptr_t table_base_va, const uintptr_t va,
	const unsigned int level)
{
					...
					unsigned int level)
{
	...
	while (table_idx < table_entries) {
		...
		if (action == ACTION_WRITE_BLOCK_ENTRY) {
			...
			/* Recurse to write into subtable */
			end_va = xlat_tables_map_region(
				ctx, mm, table_idx_va, subtable,
	...
	if (ctx->initialized) {
		...
		if (end_va != (mm_cursor->base_va + mm_cursor->size - 1U)) {
			...
			xlat_tables_unmap_region(
				ctx, &unmap_mm, 0U, ctx->base_table,
				ctx->base_table_entries, ctx->base_level);
#if !(HW_ASSISTED_COHERENCY || WARMBOOT_ENABLE_DCACHE_EARLY)
			xlat_clean_dcache_range(
				(uintptr_t)ctx->base_table,
