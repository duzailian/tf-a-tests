...
void tftf_wait_for_event(event_t *event)
{
	...
	while (!event_received) {
		...
		if (!event->cnt) {
			...
			/*
			 * No memory barrier is needed here because
			 * spin_unlock() issues a Store-Release instruction,
			 * which guarantees that loads and stores appearing in
			 * program order before the Store-Release are observed
			 * before the Store-Release itself.
			 */
			spin_unlock(&event->lock);
		}
	}
