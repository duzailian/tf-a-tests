...
static void dump_guid_hob_data(struct efi_hob_guid_type *guid_hob)
{
	...
	union uuid_helper_t mmram_mem_resv_guid = {.efi_guid = (struct efi_guid)MM_PEI_MMRAM_MEMORY_RESERVE_GUID};
	union uuid_helper_t ns_buffer_guid = {.efi_guid = (struct efi_guid)MM_NS_BUFFER_GUID};
	union uuid_helper_t shared_buffer_guid = {.efi_guid = (struct efi_guid)MM_COMM_BUFFER_GUID};

	/* Dump GUID HOB data according to GUID type. */
	efi_physical_address_t guid_data = (efi_physical_address_t)(&guid_hob->name + 1);

	if(uuid_equal(&uuid_name.uuid_struct, &mmram_mem_resv_guid.uuid_struct)){
		struct efi_mmram_hob_descriptor_block * mmram_desc_block = (struct efi_mmram_hob_descriptor_block *)guid_data;
		INFO("    MM_PEI_MMRAM_MEMORY_RESERVE_GUID with %u regions \n", mmram_desc_block->number_of_mm_reserved_regions);
		for (uint32_t i = 0; i <
				mmram_desc_block->number_of_mm_reserved_regions;
				i++){
			INFO("    MMRAM_DESC[%u]: \n", i);
			dump_efi_mmram_descriptor(&mmram_desc_block->descriptor[i]);
		}
	}else if(uuid_equal(&uuid_name.uuid_struct, &ns_buffer_guid.uuid_struct)){
		INFO("    MM_NS_BUFFER_GUID\n");
		dump_efi_mmram_descriptor((struct efi_mmram_descriptor*)guid_data);
	}else if(uuid_equal(&uuid_name.uuid_struct, &shared_buffer_guid.uuid_struct)){
		INFO("    MM_COMM_BUFFER_GUID\n");
		struct mm_comm_buffer_desc * shared_buff_desc = (struct
				mm_comm_buffer_desc *)guid_data;
		INFO("        Physical_start: 0x%llx\n", shared_buff_desc->physical_start);
		INFO("        Number of pages: %llu\n", shared_buff_desc->number_of_pages);
	}
