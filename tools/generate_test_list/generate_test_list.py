...
def RemoveSkippedTests(testsuites, skip_tests_filename):
  ...
  with open(skip_tests_filename) as skipped_file:
    ...
    for i, l in enumerate(skipped_file_lines):
      ...
      print(
          f"INFO: Testcase '{testsuite_name}/{testcase_name}' will be skipped."
      )
...
if __name__ == '__main__':
  ...
  for ts in testsuites:
    if '/' in ts.name:
      raise ValueError(
          f'ERROR: {args.xml_test_filename}: Invalid test suite name {ts.name}'
      )

    if ts.name in testsuite_name_set:
      raise ValueError(
          f"ERROR: {args.xml_test_filename}: Can't have 2 test suites named "
          f'{ts.name}'
      )
    ...
    for tc in ts.testcases:
      if tc.name in testcase_name_set:
        raise ValueError(
            f"ERROR: {args.xml_test_filename}: Can't have 2 tests named "
            f'{testcase_name}'
        )
  ...
  with open(args.testlist_hdr_filename, 'w') as hdr_file:
    file_hdr_contents = h_tpl.replace(
  ...
  all_function_prototypes = [
      f'test_result_t {t.function}(void);' for t in combined_testcases
  ]
