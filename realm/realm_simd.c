 ...
 *
 * If the access causes a plane exit, Plane 0 receives the following return
 * values:
...
bool test_realm_sve_plane_n(void)
{
	 ...
	 * SIMD_TRAP_ATTEMPTS to access SIMD registers and no plane exit
	 * should happen.
	 */
	if (!test_realm_enter_plane_n(&run, 0UL)) {
		return false;
	}

	if ((run.exit.exit_reason != RSI_EXIT_SYNC) ||
	    (EC_BITS(run.exit.esr) != EC_AARCH64_HVC)) {
		ERROR("Invalid exit reason from PN: 0x%lu, EC: 0x%lu\n",
			run.exit.exit_reason, EC_BITS(run.exit.esr));
		return false;
	}

	attempted = realm_shared_data_get_plane_n_val(1U, REC_IDX(read_mpidr_el1()),
						      HOST_ARG1_INDEX);
	success = realm_shared_data_get_plane_n_val(1U, REC_IDX(read_mpidr_el1()),
						    HOST_ARG2_INDEX);
	...
	/*
	 * Restart P1 again, this time with TRAP_SIMD enabled. That should
	 * cause a plane exit when attempting SIMD instructions.
	 */
	if (!test_realm_enter_plane_n(&run, RSI_PLANE_ENTRY_FLAG_TRAP_SIMD)) {
		return false;
	}
	 ...
	 * due to SIMD access.
	 */
	if (!test_realm_resume_plane_n(&run, 1U, 0U)) {
		return false;
	}
