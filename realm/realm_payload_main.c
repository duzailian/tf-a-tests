...
void realm_configure_mmu(void)
{
	...
	realm_printf("REALM_TEXT_START=0x%lx REALM_TEXT_END=0x%lx\n REALM_RODATA_START=0x%lx REALM_RODATA_END=0x%lx\n REALM_DATA_START=0x%lx REALM_DATA_END=0x%lx\n REALM_BSS_START=0x%lx REALM_BSS_END=0x%lx\n",
			REALM_TEXT_START, REALM_TEXT_END, REALM_RODATA_START, REALM_RODATA_END, REALM_DATA_START, REALM_DATA_END, REALM_BSS_START, REALM_BSS_END);
	init_xlat_tables();
	enable_mmu_el1(0);
}
