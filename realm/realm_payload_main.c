	...
	if (ripas == RSI_EMPTY) {
		...
		if (ret == RSI_SUCCESS && response == RSI_ACCEPT) {
			...
			if (ret == RSI_SUCCESS && ripas == RSI_RAM) {
				realm_printf("rsi_ipa_state_set passed ripas = %d\n", ripas);
				return true;
			} else {
				realm_printf("rsi_ipa_state_set failed ripas = %d\n", ripas);
				return false;
			}
		} else {
			return false;
...
void realm_payload_main(void)
{
	...
	if (realm_get_shared_structure(read_mpidr_el1() & MPID_MASK) != NULL) {
