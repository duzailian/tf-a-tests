	...
	if (ripas == RSI_EMPTY) {
		...
		if (ret == RSI_SUCCESS) {
			...
			if (ripas == RSI_DESTROYED) {
				// change ripas to RAM if not destroyed
				// expect error due to RSI_NO_CHANGE_DESTROYED
				ret = rsi_ipa_state_set(base, base + PAGE_SIZE, RSI_RAM,
						RSI_NO_CHANGE_DESTROYED, &new_base, &response);
				if (new_base != base) {
					realm_printf("rsi_ipa_state_set should have failed"
							"response = %d new_base=0x%lx\n",
							response, new_base);
					return false;
				}
				ret = rsi_ipa_state_get(base, &ripas);
