...
static bool test_realm_inst_fetch_destroyed_cmd(void)
{
	u_register_t ret, base, new_base;
	void (*func_ptr)(void);
	rsi_ripas_respose_type response;
	...
	if (ripas == RSI_EMPTY) {
		...
		if (ret == RSI_SUCCESS) {
			...
			if (ripas == RSI_DESTROYED) {
				/* Causes instruction abort as RIPAS = destroyed */
...
static bool test_realm_inst_fetch_unassigned_cmd(void)
{
	...
	realm_printf("Initial ripas=0x%lx\n", ripas);
	if (ripas == RSI_RAM) {
		/* Causes instruction abort as HIPAS = unassigned */
		realm_printf("Generate Instruction Abort\n");
		func_ptr = (void (*)(void))base;
		func_ptr();
		/* should not return */
...
void realm_payload_main(void)
{
	...
	if (realm_get_my_shared_structure() != NULL) {
		...
		case REALM_INSTR_FETCH_CMD:
			test_succeed = test_realm_inst_fetch_destroyed_cmd();
			break;
