...
/* Get inst id for a known Realm device id RDEV_BDF_ID */
...
/*
 * If the the Realm supports DA feature, this function calls series of RSI RDEV
...
bool test_realm_da_rsi_calls(void)
{
	...
	rsi_rc = realm_rdev_init(rdev, RDEV_BDF_ID);
	if (rdev == NULL) {
