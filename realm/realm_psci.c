...
secondary_ep_t entrypoint[MAX_REC_COUNT];
u_register_t context_id[MAX_REC_COUNT];
...
void realm_secondary_entrypoint(void)
{
	u_register_t my_mpidr;
	secondary_ep_t ep;

	my_mpidr = read_mpidr_el1() & MPID_MASK;
	realm_printf("Booting CPU = 0x%lx\n", my_mpidr);
	...
	realm_printf("Powering off CPU = 0x%lx\n", my_mpidr);
	realm_cpu_off();
}
