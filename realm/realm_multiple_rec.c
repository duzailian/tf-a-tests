...
bool test_realm_multiple_rec_multiple_cpu_cmd(void)
{
	...
	rec_count = realm_shared_data_get_host_val(0U, HOST_NUM_REC_INDEX);
	...
	for (unsigned int j = 1U; j < rec_count; j++) {
		ret = realm_cpu_on(j, (uintptr_t)secondary_cpu, CXT_ID_MAGIC + j);
		if (ret != PSCI_E_SUCCESS) {
	...
	if (is_secondary_cpu_booted != rec_count-1) {
		waitms(500);
	}
