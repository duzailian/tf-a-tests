...
struct rand_smc_node {
	...
	struct rand_smc_node *treenodes;	// Selection of nodes that are farther down in the tree that reference further rand_smc_node objects
	int *norcall;				// Specifies whether a particular node is a leaf node or tree node
	int entries;				// Number of nodes in object
	int biasent;				// Number that gives the total number of entries in biasarray based on all biases of the nodes
				    ...
				    struct memmod *mmod)
{
	...
	while (dtdone == 0) {
		...
		/*
		  ...
		  * 2 	Node termination of current hiearchy.  Could indicate end of tree or preparation for another branch
		  * 3 	Leaf node indication where a bias with a function name should be found for the current node
