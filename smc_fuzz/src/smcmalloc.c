 ...
 */
void *smcmalloc(unsigned int rsize, struct memmod *mmod)
{
	...
	/*
	 * Is size on a power of 2 boundary?  if not select next largest power
	 * of 2 to place the memory request in
	 */
	...
	for (unsigned int i = 0U; i < mmod->nmemblk; i++) {
		...
		/*
		 * Searching sizes and alignments of memory blocks to find a
		 * candidate that will accept the size
		 */
		if ((rsize <= (mmod->memptr->size - aladd)) &&
		    (mmod->memptr->size > aladd) &&
		    (mmod->memptr->valid == 1)) {
			foundmem = true;
			...
			/*
			 * Determining if the size adheres to power of 2
			 * boundary and if a retired malloc block can be
			 * utilized from the malloc table
			 */
			if (modval == 0U) {
				if (mmod->ptrmemblkqueue > 0U) {
					newblk = mmod->memblkqueue
							 [mmod->ptrmemblkqueue -
							  1U];
					mmod->ptrmemblkqueue--;
				} else {
				...
				/*
				 * Setting memory block parameters for newly
				 * created memory
				 */
				newblk->size = 0U;
				...
				/*
				 * Scrolling through the malloc attribute table
				 * to find entries that have values that match
				 * the newly created block and replace them with
				 * it
				 */
				unsigned int fadd =
					newblk->address + newblk->size;
				for (unsigned int j = 0U; j < mmod->cntdeladd;
				     j++) {
					if ((fadd == mmod->mallocdeladd[j]) &&
					    (mmod->mallocdeladd_valid[j] ==
					     1U)) {
				...
				mmod->trailblock[mallocdeladd_pos] =
					mmod->memptr;
				mmod->mallocdeladd_valid[mallocdeladd_pos] = 1U;
				mmod->memptr = (void *)mmod->memory;

				/*
				 * Removing entries from malloc table that can
				 * be merged with other blocks
				 */
				for (unsigned int j = 0U; j < mmod->nmemblk;
				     j++) {
				...
				/*
				 * Allocating memory that is aligned with power
				 * of 2
				 */
				unsigned int nblksize = mmod->memptr->size -
							rsize -
							(alignnum - modval);
				if (mmod->ptrmemblkqueue > 0U) {
					newblk = mmod->memblkqueue
				...
				unsigned int fadd =
					newblk->address + newblk->size;
				for (unsigned int i = 0U; i < mmod->cntdeladd;
				     i++) {
					if ((fadd == mmod->mallocdeladd[i]) &&
					    (mmod->mallocdeladd_valid[i] ==
					     1U)) {
				...
				mmod->mallocdeladd[mallocdeladd_pos] =
					mmod->memptr->address +
					mmod->memptr->size;
				mmod->precblock[mallocdeladd_pos] =
					mmod->memptr;
				mmod->mallocdeladd_valid[mallocdeladd_pos] = 1U;
			}
			if (incrcntdeladd == 1) {
				...
				if (mmod->cntdeladd >= mmod->maxmemblk) {
					printf("ERROR: size of GENMALLOC table "
					       "exceeded\n");
					mmod->memerror = 2U;
				}
			}
	...
	if (incrnmemblk == 1) {
		...
		if (mmod->nmemblk >=
			...
			printf("SMC GENMALLOC exceeded block limit of %ld\n",
			       ((TOTALMEMORYSIZE / BLKSPACEDIV) /
				sizeof(struct memblk)));
			mmod->memerror = 3U;
		}
	}
	if (foundmem == false) {
		printf("ERROR: SMC GENMALLOC did not find memory region, size "
		       "is %u\n",
		       rsize);
		mmod->memerror = 4U;
	}
		...
		for (unsigned int i = 0U; i < mmod->checknumentries; i++) {
			if (((mmod->mallocdeladd[mallocdeladd_pos] >
			    ...
			    (((mmod->mallocdeladd[mallocdeladd_pos] + rsize) >
			     ...
			     ((mmod->mallocdeladd[mallocdeladd_pos] + rsize) <
			      mmod->checkea[i]))) {
				printf("ERROR: found overlap with previous "
				       "addressin smc GENMALLOC\n");
				printf("New address %u size %u\n",
				       mmod->mallocdeladd[mallocdeladd_pos],
				       rsize);
				printf("Conflicting address %u size %u\n",
				       mmod->checksa[i],
				       (mmod->checkea[i] - mmod->checksa[i]));
				mmod->memerror = 5U;
			}
		}
				...
				if (((mmod->mallocdeladd[mallocdeladd_pos] >
				     ...
				     (mmod->mallocdeladd[mallocdeladd_pos] <
				      (mmod->memptr->address +
				       mmod->memptr->size))) ||
				    (((mmod->mallocdeladd[mallocdeladd_pos] +
				       rsize) > mmod->memptr->address) &&
				     ((mmod->mallocdeladd[mallocdeladd_pos] +
				       rsize) < (mmod->memptr->address +
						 mmod->memptr->size)))) {
					printf("ERROR: found overlap with "
					       "GENFREE memory region in smc "
					       "GENMALLOC\n");
					printf("New address %u size %u\n",
					       mmod->mallocdeladd
						       [mallocdeladd_pos],
					       rsize);
					printf("Conflicting address %u size "
					       "%u\n",
					       mmod->memptr->address,
					       mmod->memptr->size);
					mmod->memerror = 7U;
	...
	return (void *)mmod->memory + ((TOTALMEMORYSIZE / BLKSPACEDIV)) +
	       mmod->mallocdeladd[mallocdeladd_pos];
#ifdef DEBUG_SMC_MALLOC
	return (void *)mmod->memory + 0x100U +
	       mmod->mallocdeladd[mallocdeladd_pos];
#endif
}
...
int smcfree(void *faddptr, struct memmod *mmod)
{
	...
	/*
	 * Scrolling through the malloc attribute table to find entries that
	 * match the user supplied address
	 */
	...
	for (unsigned int i = 0U; i < mmod->cntdeladd; i++) {
		if ((fadd == mmod->mallocdeladd[i]) &&
			...
			if (mmod->trailblock[i] != NULL) {
				if ((mmod->precblock[i]->address +
				     mmod->precblock[i]->size) == fadd) {
					/*
					 * Found matching attribute block and
					 * then proceed to merge with
					 * surrounding blocks
					 */
					...
					/*
					 * Scrolling through the malloc
					 * attribute table to find entries that
					 * have values that match the newly
					 * merged block and replace them with it
					 */

					for (unsigned int j = 0U;
					     j < mmod->nmemblk; j++) {
						if (mmod->memptr->valid == 1) {
							if ((mmod->trailblock[i]
								     ->address +
							     mmod->trailblock[i]
								     ->size) ==
							    mmod->memptr
			...
			mmod->mallocdeladd_queue[mmod->mallocdeladd_queue_cnt] =
				i;
			mmod->mallocdeladd_queue_cnt++;
			if (mmod->mallocdeladd_queue_cnt >= mmod->maxmemblk) {
				printf("ERROR: GENMALLOC reuse queue size "
				       "exceeded\n");
				exit(1);
			}

			/*
			 * Scrolling through the malloc attribute table to find
			 * entries that have values that match the newly merged
			 * block and replace them with it
			 */

			unsigned int faddGENFREE =
				newblk->address + newblk->size;
