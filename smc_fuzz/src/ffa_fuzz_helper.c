...
void run_ffa_fuzz(int funcid, struct memmod *mmod)
{
	if (funcid == ffa_id_get_funcid) {
		...
		setconstraint(FUZZER_CONSTRAINT_SVALUE, sender_id, 1,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG1_SENDER_ID, mmod,
			      FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_VECTOR, receiver_ids, 3,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG1_RECEIVER_ID,
			      mmod, FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_VECTOR, message_type, 2,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG2_MESSAGE_TYPE,
			      mmod, FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_VECTOR, frmwrk_msg_type, 2,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG2_FRMWK_MSG_TYPE,
			      mmod, FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_VECTOR, msg_0_input, 2,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG3_MSG_0, mmod,
			      FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_RANGE, msg_1_input, 2,
			      FFA_MSG_SEND_DIRECT_REQ_CALL_ARG4_MSG_1, mmod,
			      FUZZER_CONSTRAINT_EXCMODE);

		ip = generate_args(FFA_MSG_SEND_DIRECT_REQ_CALL, SMC_FUZZ_SANITY_LEVEL);
		ret = ffa_call_with_params(ip, FFA_MSG_SEND_DIRECT_REQ_SMC64);
		...
		setconstraint(FUZZER_CONSTRAINT_SVALUE, sender_id, 1,
			      FFA_MSG_SEND_DIRECT_RESP_CALL_ARG1_SENDER_ID, mmod,
			      FUZZER_CONSTRAINT_EXCMODE);
		setconstraint(FUZZER_CONSTRAINT_VECTOR, receiver_ids, 3,
			      FFA_MSG_SEND_DIRECT_RESP_CALL_ARG1_RECEIVER_ID,
			      mmod, FUZZER_CONSTRAINT_EXCMODE);

		ip = generate_args(FFA_MSG_SEND_DIRECT_RESP_CALL, SMC_FUZZ_SANITY_LEVEL);
		ret = ffa_call_with_params(ip, FFA_MSG_SEND_DIRECT_RESP_SMC64);
		...
		ip = generate_args(FFA_FEATURES_FEAT_ID_CALL, SMC_FUZZ_SANITY_LEVEL);
		ret = ffa_call_with_params(ip, FFA_FEATURES);
		printf("ret.fid: 0x%lx\n", ret.fid);
	}else if (funcid == ffa_features_func_id_funcid) {
		uint64_t ffa_funcid_range[] = {FFA_FUNC_ID_MIN, FFA_FUNC_ID_MAX};
		struct inputparameters ip;
		struct ffa_value ret;
		...
		ip = generate_args(FFA_FEATURES_FUNC_ID_CALL, SMC_FUZZ_SANITY_LEVEL);
		...
		funcid_gen_input = get_generated_value(FFA_FEATURES_FUNC_ID_CALL_ARG1_FUNC_ID, ip);
		...
		ip = generate_args(FFA_NOTIFICATION_BITMAP_CREATE_CALL, SMC_FUZZ_SANITY_LEVEL);
		ret = ffa_call_with_params(ip, FFA_NOTIFICATION_BITMAP_CREATE);

		gen_vm_id = get_generated_value(FFA_NOTIFICATION_BITMAP_CREATE_CALL_ARG1_VM_ID, ip);
		gen_n_vcpus = get_generated_value(FFA_NOTIFICATION_BITMAP_CREATE_CALL_ARG2_N_VCPUS, ip);
		...
		ip = generate_args(FFA_NOTIFICATION_BIND_CALL, SMC_FUZZ_SANITY_LEVEL);
		...
		gen_vm_id = get_generated_value(FFA_NOTIFICATION_BITMAP_DESTROY_CALL_ARG1_VM_ID, ip);
		...
		ip = generate_args(FFA_NOTIFICATION_BITMAP_DESTROY_CALL, SMC_FUZZ_SANITY_LEVEL);
