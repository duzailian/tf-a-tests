...
void run_ffa_fuzz(int funcid, struct memmod* mmod)
{
	if (funcid == ffa_id_get_funcid) {
		...
		/*TODO a way to pair the x1/x2 args so that UUID input is
		 * cohesive? */
	...
	} else if (funcid == ffa_msg_send_direct_req_32_funcid || funcid ==
			ffa_msg_send_direct_req_64_funcid) {
		...
		if (funcid == ffa_msg_send_direct_req_32_funcid){
			...
			sender_arg = FFA_MSG_SEND_DIRECT_REQ_32_CALL_ARG1_SENDER_ID;
			receiver_arg = FFA_MSG_SEND_DIRECT_REQ_32_CALL_ARG1_RECEIVER_ID;
			msg_type_arg = FFA_MSG_SEND_DIRECT_REQ_32_CALL_ARG2_MESSAGE_TYPE;
			frmwk_msg_type_arg = FFA_MSG_SEND_DIRECT_REQ_32_CALL_ARG2_FRMWK_MSG_TYPE;
			...
			sender_arg = FFA_MSG_SEND_DIRECT_REQ_64_CALL_ARG1_SENDER_ID;
			receiver_arg = FFA_MSG_SEND_DIRECT_REQ_64_CALL_ARG1_RECEIVER_ID;
			msg_type_arg = FFA_MSG_SEND_DIRECT_REQ_64_CALL_ARG2_MESSAGE_TYPE;
			frmwk_msg_type_arg = FFA_MSG_SEND_DIRECT_REQ_64_CALL_ARG2_FRMWK_MSG_TYPE;
		...
		ip = generate_args(call_id,
				   SMC_FUZZ_SANITY_LEVEL);
		ret = ffa_call_with_params(ip, req_function_id);

		if (ret.fid == FFA_MSG_SEND_DIRECT_RESP_SMC64 || ret.fid ==
				FFA_MSG_SEND_DIRECT_RESP_SMC32) {
			printf("Received direct response, ret.arg4 = %ld\n",
			       ret.arg4);
		} else if (ret.fid == FFA_ERROR) {
			printf("Direct request returned with FFA_ERROR code "
			       "%d\n",
			       ffa_error_code(ret));
		} else if (ret.fid == FFA_MSG_YIELD) {
			printf("FFA_MSG_SEND_DIRECT_REQ returned with "
			       "FFA_MSG_YIELD\n");
		} else if (ret.fid == FFA_INTERRUPT) {
			printf("FFA_MSG_SEND_DIRECT_REQ returned with "
			       "FFA_INTERRUPT\n");
		} else {
			printf("FAIL FFA_MSG_SEND_DIRECT_REQ returned with "
			       "0x%lx\n",
			       ret.fid);
		}
	} else if (funcid == ffa_msg_send_direct_resp_32_funcid || funcid ==
			ffa_msg_send_direct_resp_64_funcid) {
		uint64_t receiver_ids[] = {SP_ID(1), SP_ID(2), SP_ID(3)};
		uint64_t sender_id[] = {HYP_ID};
		struct inputparameters ip;
		struct ffa_value ret;
		...
		if (funcid == ffa_msg_send_direct_resp_32_funcid){
			...
			sender_arg = FFA_MSG_SEND_DIRECT_RESP_32_CALL_ARG1_SENDER_ID;
			receiver_arg = FFA_MSG_SEND_DIRECT_RESP_32_CALL_ARG1_RECEIVER_ID;
		}else{
			resp_function_id = FFA_MSG_SEND_DIRECT_RESP_SMC64;
			call_id = FFA_MSG_SEND_DIRECT_RESP_64_CALL;
			sender_arg = FFA_MSG_SEND_DIRECT_RESP_64_CALL_ARG1_SENDER_ID;
			receiver_arg = FFA_MSG_SEND_DIRECT_RESP_64_CALL_ARG1_RECEIVER_ID;
		}
		...
		if (ret.fid == FFA_ERROR) {
			printf("Direct response returned with FFA_ERROR code "
			       "%d\n",
			       ffa_error_code(ret));
		} else {
			printf("FAIL FFA_MSG_SEND_DIRECT_RESP returned with "
			       "0x%lx\n",
			       ret.fid);
		}
		...
		if (ret.fid == FFA_SUCCESS_SMC32 ||
			...
			printf("FAIL FFA_NOTIFICATION_BITMAP_CREATE returned "
			       "with 0x%lx\n",
			       ret.fid);
		}
		...
		if (ret.fid == FFA_SUCCESS_SMC32 ||
			...
			printf("FFA_NOTIFICATION_BIND returned with FFA_ERROR "
			       "code %d\n",
			       ffa_error_code(ret));
		} else {
			printf("FAIL FFA_NOTIFICATION_BIND returned with "
			       "0x%lx\n",
			       ret.fid);
		}
		...
		if (ret.fid == FFA_SUCCESS_SMC32 ||
		    ret.fid == FFA_SUCCESS_SMC64) {
			printf("FFA_NOTIFICATION_BITMAP_DESTROY succeeded for "
			       "VM %lld\n",
			       gen_vm_id);
		} else if (ret.fid == FFA_ERROR) {
			printf("FFA_NOTIFICATION_BITMAP_CREATE returned with "
