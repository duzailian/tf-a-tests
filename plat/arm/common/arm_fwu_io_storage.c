...
static const io_block_spec_t fwu_fip_block_spec = {
	.offset = PLAT_ARM_FWU_FIP_BASE, .length = PLAT_ARM_FWU_FIP_SIZE};
...
static const struct plat_io_policy policies[] = {
	[FWU_FIP_IMAGE_ID] = {&memmap_dev_handle,
			      (uintptr_t)&fwu_fip_block_spec, open_memmap},
	[FWU_CERT_ID] = {&fwu_fip_dev_handle, (uintptr_t)&fwu_cert_uuid_spec,
			 open_fwu_fip},
	[SCP_BL2U_IMAGE_ID] = {&fwu_fip_dev_handle,
			       (uintptr_t)&scp_bl2u_uuid_spec, open_fwu_fip},
	[BL2U_IMAGE_ID] = {&fwu_fip_dev_handle, (uintptr_t)&bl2u_uuid_spec,
			   open_fwu_fip},
	[NS_BL2U_IMAGE_ID] = {&fwu_fip_dev_handle,
			      (uintptr_t)&ns_bl2u_uuid_spec, open_fwu_fip},
};
...
static int open_fwu_fip(const uintptr_t spec)
{
	...
	result = io_dev_init(fwu_fip_dev_handle, (uintptr_t)FWU_FIP_IMAGE_ID);
	if (result == IO_SUCCESS) {
		result = io_open(fwu_fip_dev_handle, spec, &local_image_handle);
		if (result == IO_SUCCESS) {
			VERBOSE("Using FIP\n");
