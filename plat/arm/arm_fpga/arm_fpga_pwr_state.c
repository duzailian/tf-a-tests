...
/*
 * State IDs for local power states on the ARM_FPGA
 */
#define ARM_FPGA_RUN_STATE_ID		0	/* Valid for CPUs and Clusters */
#define ARM_FPGA_RETENTION_STATE_ID	1	/* Valid for only CPUs */
#define ARM_FPGA_OFF_STATE_ID		2	/* Valid for CPUs and Clusters */

/*
 * Suspend depth definitions for each power state
 */
typedef enum {
	ARM_FPGA_RUN_DEPTH = 0,
	ARM_FPGA_RETENTION_DEPTH,
	ARM_FPGA_OFF_DEPTH,
} suspend_depth_t;

/* The state property array with details of idle state possible for the core */
static const plat_state_prop_t core_state_prop[] = {
	{ARM_FPGA_RETENTION_DEPTH, ARM_FPGA_RETENTION_STATE_ID, PSTATE_TYPE_STANDBY},
	{ARM_FPGA_OFF_DEPTH, ARM_FPGA_OFF_STATE_ID, PSTATE_TYPE_POWERDOWN},
	{0},
};
