...
#define mc_smmu_bypass_cfg                                   \
	{                                                    \
		.reg = TEGRA_SMMU0_BASE, .val = 0x00000000U, \
	}
...
void tegra_pwr_mgmt_setup(void)
{
	...
	for (unsigned int i = 1U; i < ARRAY_SIZE(tegra194_mc_context) - 1U;
	     i++) {
		tegra194_mc_context[i].val =
			mmio_read_32(tegra194_mc_context[i].reg);
	}
