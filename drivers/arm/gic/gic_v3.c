...
#ifdef __aarch64__
#define MPIDR_AFFLVL3_MASK \
	((unsigned long long)MPIDR_AFFLVL_MASK << MPIDR_AFF3_SHIFT)
#define gic_typer_affinity_from_mpidr(mpidr) \
	(((mpidr) & (~MPIDR_AFFLVL3_MASK)) | \
	 (((mpidr)&MPIDR_AFFLVL3_MASK) >> 8))
#else
#define gic_typer_affinity_from_mpidr(mpidr) \
 ...
 *****************************************************************************/
static void gicd_write_irouter(uintptr_t base, unsigned int interrupt_id,
...
static void gicr_write_ipriorityr(uintptr_t base, unsigned int id,
				  unsigned int val)
{
...
unsigned int gicv3_get_ipriorityr(unsigned int interrupt_id)
{
	...
	if (interrupt_id < MIN_SPI_ID) {
		...
		return mmio_read_8(gicd_base_addr + GICD_IPRIORITYR +
				   interrupt_id);
	}
}
...
unsigned int gicv3_get_isenabler(unsigned int interrupt_id)
{
	...
	if (interrupt_id < MIN_SPI_ID) {
		core_pos = platform_get_core_pos(read_mpidr_el1());
		assert(rdist_pcpu_base[core_pos]);
		return gicr_get_isenabler0(rdist_pcpu_base[core_pos],
					   interrupt_id);
	} else
...
void gicv3_probe_redistif_addr(void)
{
	...
	do {
		...
		if (affinity ==
		    ((typer_val >> TYPER_AFF_VAL_SHIFT) & TYPER_AFF_VAL_MASK)) {
