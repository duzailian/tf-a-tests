...
void ffa_tests(void) {
	...
	if (spm_version == FFA_ERROR_INVALID_PARAMETER) {
		NOTICE("FFA_VERSION Invalid Parameter\n");
	} else if(spm_version == FFA_ERROR_NOT_SUPPORTED) {
		NOTICE("FFA_VERSION not supported\n");
	} else {
   	...
   	announce_test_section_end(test_ffa_version);
