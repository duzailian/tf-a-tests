...
static void __dead2 message_loop(spci_vm_id_t vm_id)
{
	...
	for (;;) {
		...
		if (spci_ret.ret0 != SPCI_MSG_SEND_DIRECT_REQ_SMC32) {
			/* TODO: reply with SPCI_ERROR */
			for(;;);
		}

		/* TODO check target vm id in x1 == vm_id */

		/* TODO check source vm id in x2 == HYP */
...
void __dead2 cactus_main(void)
{
	...
	/* Get number of VMs */
	spci_vm_count_t vm_cnt = spm_vm_get_count();
	NOTICE("VM count: %u\n", vm_cnt);

	/* Get virtual CPU count for current VM */
	spci_vcpu_count_t vcpu_cnt = spm_vcpu_get_count(spci_id);
	NOTICE("vCPU count: %u\n", vcpu_cnt);
