...
CACTUS_CMD_HANDLER(notifications_bind, CACTUS_NOTIFICATION_BIND_CMD)
{
	...
	ffa_vm_id_t receiver = cactus_notification_get_receiver(*args);
	ffa_vm_id_t sender = cactus_notification_get_sender(*args);
	...
	ret = ffa_notification_bind(sender, receiver, 0, notifications);
...
CACTUS_CMD_HANDLER(notifications_get, CACTUS_NOTIFICATION_GET_CMD)
{
	ffa_vm_id_t source = ffa_dir_msg_source(*args);
	ffa_vm_id_t vm_id = ffa_dir_msg_dest(*args);
	ffa_vm_id_t notification_receiver =
...
CACTUS_CMD_HANDLER(notifiactions_set, CACTUS_NOTIFICATIONS_SET_CMD)
{
	ffa_vm_id_t source = ffa_dir_msg_source(*args);
	ffa_vm_id_t vm_id = ffa_dir_msg_dest(*args);
	ffa_vm_id_t receiver = cactus_notification_get_receiver(*args);
	ffa_vm_id_t sender = cactus_notification_get_sender(*args);
	uint64_t notifications = cactus_notification_get_notifications2(*args);
	uint32_t flags = cactus_notification_get_flags(*args);
	smc_ret_values ret;
