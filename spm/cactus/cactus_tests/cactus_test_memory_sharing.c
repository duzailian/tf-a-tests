...
static bool data_abort_gpf_handler(void)
{
	...
	VERBOSE("%s count %u esr_el1 %llx elr_el1 %lx\n", __func__,
		data_abort_gpf_triggered, esr_el1, read_elr_el1());
...
static bool cactus_mem_unmap_and_relinquish(
	struct ffa_composite_memory_region *composite, void *send,
	ffa_memory_handle_t handle, ffa_id_t vm_id)
{
	int ret;
	...
	if (!memory_relinquish((struct ffa_mem_relinquish *)send, handle,
			       vm_id)) {
		return false;
	}
...
CACTUS_CMD_HANDLER(mem_send_cmd, CACTUS_MEM_SEND_CMD)
{
	...
	for (uint32_t i = 0; i < composite->constituent_count; i++) {
		...
		ret = mmap_add_dynamic_region(base_address, base_address, size,
					      mem_attrs);
	...
	for (uint32_t i = 0; i < composite->constituent_count; i++) {
		...
		for (uint32_t j = 0; j < words_to_write; j++) {
			 ...
			 * before using it.
			 */
			if ((retrv_flags & FFA_MEMORY_REGION_FLAG_CLEAR) !=
			    0U) {
				...
				/*
				 * In RME enabled systems, the memory is
				 * expected to be scrubbed on PAS updates from S
				 * to NS. As well, it is likely that the memory
				...
				VERBOSE("Check memory contents. Expect %u "
					"words of %x\n",
					words_to_write, mem_func + 0xFFA);

				/* SPs writing `mem_func` + 0xFFA. */
				if (ptr[i] != mem_func + 0xFFA) {
					ERROR("Memory content NOT as "
					      "expected!\n");
					cactus_mem_unmap_and_relinquish(
						composite, mb->send, handle,
						vm_id);
	...
	return cactus_success_resp(vm_id, source, data_abort_gpf_triggered);
}
...
CACTUS_CMD_HANDLER(req_mem_send_cmd, CACTUS_REQ_MEM_SEND_CMD)
{
	...
	ret = mmap_add_dynamic_region((uint64_t)constituents[0].address,
				      (uint64_t)constituents[0].address,
