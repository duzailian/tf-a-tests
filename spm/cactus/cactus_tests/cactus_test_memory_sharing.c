...
static bool cactus_mem_relinquish(
...
CACTUS_CMD_HANDLER(mem_send_cmd, CACTUS_MEM_SEND_CMD)
{
	...
	for (uint32_t i = 0; i < composite->constituent_count; i++) {
		...
		for (uint32_t j = 0; j < words_to_write; j++) {
			...
			if ((retrv_flags & FFA_MEMORY_REGION_FLAG_CLEAR) != 0U) {
				...
				/*
				 * Currently validating memory contents for
				 * operations from NWd, due to implications
				 * with RME system architecture:
				 * - Update to PAS address space from NS->S
				 *   should preserve memory content.
				 */
				if (IS_SP_ID(m->sender)) {
					continue;
				}
	...
	if (mem_func != FFA_MEM_DONATE_SMC32 &&
	    !cactus_mem_relinquish(composite, mb->send, handle, vm_id)) {
