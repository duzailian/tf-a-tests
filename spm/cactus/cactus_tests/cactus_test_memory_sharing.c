...
CACTUS_CMD_HANDLER(mem_send_cmd, CACTUS_MEM_SEND_CMD)
{
	...
	struct ffa_memory_access receiver = init_receiver(mem_func, vm_id);
...
CACTUS_CMD_HANDLER(req_mem_send_cmd, CACTUS_REQ_MEM_SEND_CMD)
{
	...
	struct ffa_memory_access receiver =
		init_receiver(mem_func, receiver_id);
	...
	ffa_ret = memory_init_and_send(mb, PAGE_SIZE, vm_id, &receiver, 1,
				       constituents, constituents_count,
				       mem_func);
	handle = ffa_mem_success_handle(ffa_ret);
