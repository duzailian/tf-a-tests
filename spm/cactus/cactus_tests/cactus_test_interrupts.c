...
CACTUS_CMD_HANDLER(sleep_fwd_cmd, CACTUS_FWD_SLEEP_CMD)
{
	...
	ffa_id_t fwd_dest = (ffa_id_t)(*args).ret5;
	uint32_t sleep_ms = (uint32_t)(*args).ret4;


	INFO("VM%x requested %x to sleep for value %u\n",
	...
	if( (uint32_t)(ffa_ret).ret3 != sleep_ms) {
		ERROR("Request returned: %u ms!\n", (uint32_t)(ffa_ret).ret3);
		return cactus_error_resp(vm_id, ffa_dir_msg_source(*args),
					 CACTUS_ERROR_TEST);
