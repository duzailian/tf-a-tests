...
struct ffa_value cactus_handle_framework_msg(struct ffa_value args)
{
	...
	/*
	 * FF-A spec states that SPs are prohibited from invoking any ABI other
	 * than a framework direct response to the ongoing power management
	 * framework message. Make the Cactus SP intentionally invoke prohibited
	 * ABIs and attest that SPMC should deny such invocations.
	 */
