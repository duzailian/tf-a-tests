...
///
///    * if [begin & ~7ull, end_incl | 7ull] == [0, ~0ull],
...
{
	*storage_ = (*storage_ * 0x0005deecE66Dull + 0xB) & 0xffffFFFFffffull;
...
struct user_frame_t {
	...
	// -- 2 --
	// If operating under PCIe then msiaddress should be either 1 (send
	// MSI-X) or 0 (don't send).  The MSI-X to send is in msidata.
	uint64_t msiaddress;
	...
	//     15:14  shareability 0..2 (nsh/ish/osh) (ACE encoding), ignored if
	//     a device type 13     outer transient, ignored unless outer ACACHE
	//     is cacheable 12     inner transient, ignored unless inner ACACHE
	//     is cacheable 10:8   APROT (AMBA encoding)
	...
	//      1111 -- rawaWB
	//
	// NOTE that the meaning of the ACACHE encodings are dependent on if it
	// is a read or a write.  AMBA can't encode directly the 'no-allocate
	// cacheable' and you have to set the 'other' allocation hint.  So for
	// example, a read naWB has to be encoded as waWB.  A write naWB has to
	// be encoded as raWB, etc.
	//
	// Lowest halfword are 'source' attributes.
	// Highest halfword are 'destination' attributes.
	//
	// NOTE that you can make an non-secure stream output a secure
	// transaction
	// -- the SMMU should sort it out.
	//

	// -- 4 --
	// Under PCIe then a real Function does not have control over the
	// attributes of the transactions that it makes.  However, for testing
	// purposes of the SMMU then we allow its attributes to be specified
	// (and magically transport them over PCIe).
	uint32_t attributes;
	uint32_t seed;
...
struct privileged_frame_t {
	...
	uint32_t streamid;
	uint32_t substreamid;  // ~0u means no substreamid, otherwise must be a
			       // 20 bit number or ENGINE_FRAME_MISCONFIGURED
...
enum cmd_t {
	...
	// The engine memcpy's from region [begin, end_incl] to address
	// udata[0].
	//
	// If stride is 0 then ENGINE_ERROR is produced, udata[2] contains the
	// error address.  No MSI is generated.
	//
	// If stride is 1 then this is a normal memcpy().  If stride is larger
	// then not all the data will be copied.
	//
	// The order and size of the transactions used are determined randomly
	// using seed.  If seed is:
	...
	//
	// This models a work-load where we start with some reads and then do
	// some writes.
	ENGINE_MEMCPY = 2,
	...
	//
	// A separate random number generator per transaction is then used based
	// on seed and the address:
	//
	//       seed_per_transaction = seed ^ (address >> 32) ^ (address &
	//       0xFFFFffff);
	//
	// This seed is then used to seed a random number generator to fill the
	// required space.   The data used should be:
	//          uint64_t storage;
	//          for (uint8_t* p = (uintptr_t)begin; p !=
	//          (uintptr_t)end_incl; ++ p)
	//          {
	...
	//             ++ p;
	//          }
	// This isn't the most efficient way of doing it as it throws away a lot
	// of entropy from the call to testengine_random() but then we aren't
	// aiming for good random numbers.
	//
	// If stride is 0 then ENGINE_ERROR is produced, data[2] contains the
	// error address. (NOTE that data[1] is not used).
	//
	// If stride is 1 then this fills the entire buffer.  If stride is
	// larger then not all the data will be randomized.
	//
	...
	// using seed.
	//
	// The begin must be 64 bit aligned (begin & 7) == 0 and the end_incl
	// must end at the end of a 64 bit quantitity (end_incl & 7) == 7,
	// otherwise ENGINE_FRAME_MISCONFIGURED is generated.
	//
...
{
	unsigned const t =
		t_;  // compensate for bad MSVC treating t_ as signed!
...
{
	return t_ == ENGINE_NO_FRAME || t_ == ENGINE_HALTED ||
...
///
///       16-31 -- RATE   -- some ill-defined metric for how fast to do the
///       work!
///
