...
static bool run_smmuv3_rand48(uintptr_t start_address, size_t size, uint32_t options)
{
	return run_testengine(ENGINE_RAND48, start_address, 0, size / FRAME_COUNT, options);
}
...
CACTUS_CMD_HANDLER(smmuv3_cmd, CACTUS_DMA_SMMUv3_CMD)
{
	...
	case ENGINE_MEMCPY:
		...
		break;
	case ENGINE_RAND48:
		if (run_smmuv3_rand48(start_address, size, options)) {
			return cactus_success_resp(vm_id, source, 0);
		}
		break;
	default:
