...
bool run(uint32_t operation, uintptr_t source_addr, uintptr_t target_addr, size_t transfer_size)
{
...
static bool run_smmuv3_memcpy(uintptr_t start_address, size_t size)
{
	...
	ret = run(ENGINE_MEMCPY, start_address, target_address,
		cpy_range / FRAME_COUNT);
	...
	if (ret) {
		...
		for (size_t i = 0U; i < (cpy_range / 8U); i++) {
			if (mmio_read_64(start_address + 8 * i) != mmio_read_64(target_address + 8 * i)) {
...
{
	return run(ENGINE_RAND48, start_address, 0, (size >> 1) / FRAME_COUNT);
}
