			   ...
			   uint32_t attributes)
{
	...
	for (f = 0U; f < FRAME_COUNT; f++) {
		...
		mmio_write32_offset(PRIV_BASE_FRAME + F_IDX(f),
				    DOWNSTREAM_PORT_OFF, 0);
		mmio_write32_offset(PRIV_BASE_FRAME + F_IDX(f), STREAM_ID_OFF,
				    streamID_list[f % 2]);
		mmio_write32_offset(PRIV_BASE_FRAME + F_IDX(f),
				    SUBSTREAM_ID_OFF, NO_SUBSTREAMID);

		mmio_write32_offset(USR_BASE_FRAME + F_IDX(f), UCTRL_OFF, 0);
		mmio_write32_offset(USR_BASE_FRAME + F_IDX(f), ATTR_OFF,
				    attributes);

		if (operation == ENGINE_RAND48) {
			mmio_write32_offset(USR_BASE_FRAME + F_IDX(f), SEED_OFF,
					    (f + 1) * 42);
		}

		mmio_write64_offset(USR_BASE_FRAME + F_IDX(f), BEGIN_OFF,
				    begin_addr);
		mmio_write64_offset(USR_BASE_FRAME + F_IDX(f), END_CTRL_OFF,
				    end_addr);

		/* Legal values for stride: 1 and any multiples of 8 */
		mmio_write64_offset(USR_BASE_FRAME + F_IDX(f), STRIDE_OFF, 1);
		mmio_write64_offset(USR_BASE_FRAME + F_IDX(f), UDATA_OFF,
				    dest_addr);

		mmio_write32_offset(USR_BASE_FRAME + F_IDX(f), CMD_OFF,
				    operation);
		VERBOSE("SMMUv3TestEngine: waiting completion for frame: %u\n",
			f);

		/*
		 * It is guaranteed that a read of "cmd" fields after writing to
		 * it will immediately return ENGINE_FRAME_MISCONFIGURED if the
		 * command was invalid.
		 */
		if (mmio_read32_offset(USR_BASE_FRAME + F_IDX(f), CMD_OFF) ==
		    ENGINE_MIS_CFG) {
		...
		while (attempts++ < LOOP_COUNT) {
			status = mmio_read32_offset(USR_BASE_FRAME + F_IDX(f),
						    CMD_OFF);
		...
		if (attempts == LOOP_COUNT) {
			ERROR("SMMUv3: test failed, exceeded max. wait "
			      "loop.\n");
			return false;
		}
...
static bool run_smmuv3_memcpy(uintptr_t start_address, size_t size,
			      uint32_t attributes)
{
	...
	if (ret) {
		/*
		 * Invalidate cached entries to force the CPU to fetch the data
		 * from Main memory
		 */
...
static bool run_smmuv3_rand48(uintptr_t start_address, size_t size,
			      uint32_t attributes)
{
	return run_testengine(ENGINE_RAND48, start_address, 0,
			      size / FRAME_COUNT, attributes);
}
