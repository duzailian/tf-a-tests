...
static const struct ffa_uuid sp_uuids[] = {{PRIMARY_UUID},
					   {SECONDARY_UUID},
					   {TERTIARY_UUID},
					   {IVY_UUID},
					   {EL3_SPMD_LP_UUID}};
	...
	{.id = SP_ID(1),
	 ...
	 .properties =
		 (FFA_PARTITION_AARCH64_EXEC | FFA_PARTITION_DIRECT_REQ_RECV |
		  FFA_PARTITION_DIRECT_REQ_SEND | FFA_PARTITION_NOTIFICATION),
	 .uuid = {PRIMARY_UUID}},
...
static void ffa_partition_info_get_regs_test(void)
{
	...
	if (ffa_func_id(ret) != FFA_SUCCESS_SMC32) {
		ERROR("FFA_PARTITION_INFO_GET_REGS not supported skipping "
		      "tests.\n");
		return;
	}

	expect(ffa_partition_info_regs_helper(
		       sp_uuids[3], &ffa_expected_partition_info[3], 1),
	       true);
	expect(ffa_partition_info_regs_helper(
		       sp_uuids[2], &ffa_expected_partition_info[2], 1),
	       true);
	expect(ffa_partition_info_regs_helper(
		       sp_uuids[1], &ffa_expected_partition_info[1], 1),
	       true);
	expect(ffa_partition_info_regs_helper(
		       sp_uuids[0], &ffa_expected_partition_info[0], 1),
	       true);
	...
	if ((ffa_func_id(ret) == FFA_ERROR) &&
		...
		INFO("Skipping register based EL3 SPMD Logical partition"
		     " discovery\n");
		expect(ffa_partition_info_regs_helper(
			       NULL_UUID, ffa_expected_partition_info,
			       (ARRAY_SIZE(ffa_expected_partition_info) - 1)),
		       true);
	} else {
		expect(ffa_partition_info_regs_helper(
			       sp_uuids[4], &ffa_expected_partition_info[4], 1),
		       true);
		expect(ffa_partition_info_regs_helper(
			       NULL_UUID, ffa_expected_partition_info,
...
void ffa_version_test(void)
{
	struct ffa_value ret =
		ffa_version(MAKE_FFA_VERSION(FFA_MAJOR, FFA_MINOR));
	...
	INFO("Test FFA_VERSION. Return %u.%u; Compatible: %i\n",
	     ...
	     spm_version & FFA_VERSION_MINOR_MASK, (int)ffa_version_compatible);
