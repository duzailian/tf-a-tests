...
void helium_message_handler(struct sprt_queue_entry_message *message)
{
	...
	if (message->type == SPRT_MSG_TYPE_SERVICE_REQUEST) {
		...
		case HELIUM_GET_MAGIC:
			ret1 = HELIUM_MAGIC_NUMBER;
			ret0 = SPRT_SUCCESS;
			break;
...
void __dead2 helium_main(void)
{
	/*
	 * Handle secure service requests.
	 */
	sprt_initialize_queues((void *)HELIUM_SPM_BUF_BASE);
		...
		while (1) {
			...
			if (err == -ENOENT) {
				...
				if (err == -ENOENT) {
					...
					assert(err == 0);
					helium_message_handler(&message);
				}
			} else {
				assert(err == 0);
				helium_message_handler(&message);
