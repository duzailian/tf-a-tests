...
/*
 * Returns the next message to be processed by the SPRT client. Each message can
 * be retrieved only once. The message pointer must point to a valid memory
 * owned by the caller. A zero return value indicates there is a msessage for
 * the SP, -ENOENT means there are no messages.
 */
