...
{
	return 	ffa_msg_send_direct_req(source, dest, cmd, val0, val1, val2,
					val3);
}
...
static inline smc_ret_values cactus_send_response(
	...
	uint64_t val1, uint64_t val2, uint64_t val3)
{
	return ffa_msg_send_direct_resp(source, dest, resp, val0, val1,
					val2, val3);
}
...
{
	return ffa_msg_send_direct_resp(source, dest, response, 0, 0, 0, 0);
}
