...
static inline smc_ret_values cactus_send_response(
	ffa_vm_id_t source, ffa_vm_id_t dest, uint32_t resp, uint64_t val0,
	uint64_t val1, uint64_t val2, uint64_t val3)
{
...
/*
 * Request to SP bind a notification to a FF-A endpoint. In case of error
 ...
 * are propagated throught the command's arguments, to allow the test of
 * erroneous uses of the FFA_NOTIFICATION_BIND interface.
 *
 * The command id is the hex representation of the tring "bind".
 */
#define CACTUS_NOTIFICATION_BIND_CMD U(0x62696e64)

static inline smc_ret_values cactus_notification_bind_send_cmd(
	ffa_vm_id_t source, ffa_vm_id_t dest, ffa_vm_id_t receiver,
	ffa_vm_id_t sender, uint64_t notifications, uint32_t flags)
{
...
static inline smc_ret_values cactus_notification_unbind_send_cmd(
	ffa_vm_id_t source, ffa_vm_id_t dest, ffa_vm_id_t receiver,
	ffa_vm_id_t sender, uint64_t notifications)
{
	return cactus_send_cmd(source, dest, CACTUS_NOTIFICATION_UNBIND_CMD,
			       receiver, sender, notifications, 0);
}

static inline ffa_vm_id_t cactus_notification_get_receiver(
	smc_ret_values ret)
{
	return (ffa_vm_id_t)ret.ret4;
}

static inline ffa_vm_id_t cactus_notification_get_sender(
	smc_ret_values ret)
{
	return (ffa_vm_id_t)ret.ret5;
}

static inline uint64_t cactus_notification_get_notifications(
	smc_ret_values ret)
{
...
static inline smc_ret_values cactus_notifications_get_success_resp(
	ffa_vm_id_t source, ffa_vm_id_t dest, uint64_t from_sp,
	uint64_t from_vm)
{
...
static inline smc_ret_values cactus_notifications_set_send_cmd(
	ffa_vm_id_t source, ffa_vm_id_t dest, ffa_vm_id_t receiver,
	ffa_vm_id_t sender, uint32_t flags, uint64_t notifications)
{
	return cactus_send_cmd(source, dest, CACTUS_NOTIFICATIONS_SET_CMD,
			       receiver, sender, notifications, flags);
}

/** Can't reuse defined getter for the 'notifications' argument */
static inline uint64_t cactus_notification_get_notifications2(
	smc_ret_values ret)
{
	return (uint64_t)ret.ret6;
}
