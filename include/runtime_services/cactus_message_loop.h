...
struct cactus_cmd_handler {
	const uint64_t id;
	ffa_value (*fn)(const ffa_value *args,
...
#define CACTUS_HANDLER_FN(name)						\
	static ffa_value CACTUS_HANDLER_FN_NAME(name)(		\
		const ffa_value *args, struct mailbox_buffers *mb)
...
bool cactus_handle_cmd(ffa_value *cmd_args, ffa_value *ret,
		       struct mailbox_buffers *mb);
