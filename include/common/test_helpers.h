...
#define SKIP_TEST_IF_MM_NOT_PRESENT()						\
	do {									\
		struct sc_value version_sc = { MM_VERSION_AARCH32 };		\
		uint32_t version = sp_service_call(&version_sc);		\
										\
...
#define SKIP_TEST_IF_FFA_VERSION_LESS_THAN(major, minor)			\
	do {									\
		struct sc_value sc_ret = ffa_version(				\
					 MAKE_FFA_VERSION(major, minor));	\
		uint32_t version = sc_ret.fid;					\
										\
...
#define SKIP_TEST_IF_FFA_ENDPOINT_NOT_DEPLOYED(mb, ffa_uuid)			\
	do {									\
		struct sc_value sc_ret = ffa_partition_info_get(ffa_uuid);	\
		ffa_rx_release();						\
