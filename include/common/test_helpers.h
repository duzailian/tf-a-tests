...
#define SKIP_TEST_IF_TSP_NOT_PRESENT()                                     \
	do {                                                               \
		...
		if (!uuid_equal(&tos_uuid, &tsp_uuid)) {                   \
			tftf_testcase_printf(                              \
				"Trusted OS is not the TSP, its UUID is: " \
				"%s\n",                                    \
...
#define SKIP_TEST_IF_DIT_NOT_SUPPORTED()                             \
	do {                                                         \
		if (!is_armv8_4_dit_present()) {                     \
			tftf_testcase_printf("DIT not supported\n"); \
			return TEST_RESULT_SKIPPED;                  \
		}                                                    \
	} while (0)
...
#define SKIP_TEST_IF_MM_VERSION_LESS_THAN(major, minor)                 \
	do {                                                            \
		...
		if (version < MM_VERSION_FORM(major, minor)) {          \
			tftf_testcase_printf(                           \
				"MM_VERSION returned %u.%u\n"           \
				"The required version is %u.%u\n",      \
				version >> MM_VERSION_MAJOR_SHIFT,      \
				version & MM_VERSION_MINOR_MASK, major, \
				minor);                                 \
			return TEST_RESULT_SKIPPED;                     \
		}                                                       \
                                                                        \
...
#define SKIP_TEST_IF_PA_SIZE_LESS_THAN(n)                                      \
	do {                                                                   \
		static const unsigned int pa_range_bits_arr[] =                \
			{                                                      \
				PARANGE_0000, PARANGE_0001, PARANGE_0010,      \
				PARANGE_0011, PARANGE_0100, PARANGE_0101,      \
				PARANGE_0110};                                 \
...
#define is_psci_sys_susp_supported() \
	(tftf_get_psci_feature_info(SMC_PSCI_SYSTEM_SUSPEND) == PSCI_E_SUCCESS)

/* Helper macro to verify if PSCI_STAT_COUNT API is supported */
#define is_psci_stat_count_supported() \
	(tftf_get_psci_feature_info(SMC_PSCI_STAT_COUNT) == PSCI_E_SUCCESS)
...
/*
 * Helper function to get a sentinel address that can be used to test
 * mem_protect
 */
