...
-  More tests are made available in this release to help validate the
   ...
   -  SVE test scenarios.
   -  TRNG test scenarios.
   -  v8.6 AMU enhancements test scenarios.

TFTF
~~~~

-  Fix an include issue with the AMU header files.
...
-  Use MTE instructions in 'test_mte_instructions' test.
...
-  Enable 8 cores support for Theodul.
...
-  Leverage INTERRUPT_ENABLE hvc call through SP to let SPMC(S-EL2) know
...
-  Enable managed exit for primary cactus.
...
-  FFA_MSG_DIRECT_RESP call using 5 registers.
...
-  Add & handle 'CACTUS_REQ_ECHO_CMD' and 'CACTUS_ECHO_CMD'.

-  Add commands CACTUS_ECHO_CMD and CACTUS_REQ_ECHO_CMD.
...
-  Add uart2 to device-regions node.
    -  first SP no longer have an open access to the full system peripheral 
       range and devices must be explicitelly declared in the SP manifest.
