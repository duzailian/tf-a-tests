...
-  NVIDIA Tegra194:
   ...
   -  Skips the "Create all power states and validate EL3 power state parsing"
...
-  Update memory/device region nodes in manifest. Memory region has 3
   entries such as RX buffer, TX buffer and dummy. These memory regions
   entries are mapped with following Attributes:
      -  RX buffer: read-only
...
-  Adjust the number of EC context to max number of PEs according as per
   the FF-A specification mandating that a SP must either:
      -  Implement as many ECs as the number of PEs (in case of a
         "multi-processor" SP with pinned contexts).
      -  Implement a single EC (in case of a migratable "uni-processor" SP).
