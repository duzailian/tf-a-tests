...
-  FF-A testing:
    ...
    - FF-A v1.1 Secure interrupts
        ...
        - Cactus now does not accept direct request message from any endpoint
          when in WAITING state
        - New direct request message command to resume after managed exit
        - Now allowing the destination SP of a sleep command to check if the
          forward destination SP has been preempted
        - Removed explicit enabling virtual maintenance interrupts for SPs as
