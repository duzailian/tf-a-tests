...
More features and tests are introduced in this release to enhance validation and support in the following areas:

- Realm Management Extension
- FF-A and SPM Testing
- Fuzzing Framework
- System Functionality Tests (e.g., PointerAuth, RAS registers, SMCCC feature availability, FPMR access)
- Firmware Handoff
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    - Added testcase to enter all planes.
...
- SPM/FF-A Testing:
    ...
    - Added support PPI timer interrupts.
    ...
    - Refactored `get_ffa_feature_test_target`.
...
- New Tests
    ...
    - Fuzzing Enhancements
        ...
        - Fixed single-feature testing behavior in fuzzing.
        - Introduced fuzzing support for all SDEI calls, and the FF-A interface.
...
- Platforms:
    ...
    - Versal
        ...
        - Added platform specific fucntion to return custom entry address.
...
- Miscellaneous
    ...
    - Added Cortex-X925 and updateed Cortex-X4 to Errata ABI.
    ...
    - Refactored code to use a macro for checking the SMCCC version.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    - Refactored Cactus by mapping boot information regions.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    - Resolved compile error in `test_irq_spurious_gicv2.c`.
