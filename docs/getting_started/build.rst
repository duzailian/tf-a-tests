...
The SPMC runs at S-EL2 and acts as a partition manager for multiple guest
S-EL1 partitions (`TF-A Secure Partition Manager (FF-A)`_):
...
- Cactus is a sample FF-A compliant S-EL1 partition. As a matter of providing
  ...
  corresponding each to a different secure virtual machine). Each partition
  instance has a separate manifest (cactus.dts, cactus-secondary.dts,
  cactus-tertiary.dts). First two instances are MP SPs. Third instance is a
  UP SP. Each instance runs a set of built-in tests at boot time. They exercise
  SP to SPMC FF-A interfaces contained in the secure world. The partition
  interacts with the SPMC through SMC. Once the NWd and TFTF are started,
  another set of run-time tests exercise the normal world to secure world
  primitives.
