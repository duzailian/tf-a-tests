...
implementation based on MM, while Cactus and Ivy can test the SPM implementation
based on the SPCI and SPRT draft specifications.
...
-  Test communication with SPM through either the `ARM Management Mode
   Interface`_, or both SPCI and SPRT.
...
For SPM-MM, the following commands can be used to run the tests:
